...
**Status (2026-07-14):** M1–M8 + M7-E + M10 **DONE**; M9 **DEFERRED**; M11 **DONE** — Phase-A engine wiring + REV-1 committed (`main@b2c82ab`); the flag-on e2e lanes are green on both platforms and promoted into per-commit CI (`b8ee978`); the **Phase-B flip has landed** — `liveSyncEnabled` now defaults **true**, so production ships the persistent live-sync engine. Two non-code residuals remain before tagging a release: one clean per-commit CI run (the first was infra-cancelled mid-emulator-boot, not a scenario failure) and an owner/physical-device foreground↔background socket sanity check (the iOS simulator cannot reproduce real-device suspension). **Amended P4 (power efficiency):** on iOS there is no longer a background socket to sanity-check — the engine is PAUSED between publishes and each publish tick opens one bounded burst — so that owner item is now "does the CoreLocation keep-alive keep the process executable long enough for the next burst to run", tracked in `docs/M7_BACKGROUND_SHARING.md` §6. Self-commit was **REMOVED** in M5 (kill-switch + lint guard); the concurrent-self-update fork this migration targets is CLOSED by M5 (remove the cause) + M3/M4 (converge the rest). The retained short-poll receive path stays behind the `false` branch ≥1 release as the one-line rollback.
...
1. **Globals use `RwLock<Option<Arc<…>>>`, NOT `OnceLock`.** Both `SESSION` and `CIRCLE_MANAGER` match the existing `TILE_CACHE` precedent (`api.rs`). `OnceLock` is write-once and cannot be reset on logout/identity-switch (a verified blocker). The `Option` wrapper lets `stop_session`/wipe-on-logout tear down inner state and re-login rebuild it. Async fallible init runs under the write-lock (or `tokio::sync::OnceCell` for the inner build) — never `std::sync::OnceLock::get_or_init` (sync + infallible, unfit for async relay connect).
...
4. **`FfiRelayEvent` = ONE tagged enum** (Dart sealed class): `Location`, `GroupUpdate`, `Welcome`, `Unprocessable`, `Status`. Rust decrypts ONCE, emits decrypted fields (no Dart re-decrypt). Every leaf gets a redacting `Debug` (group-id/coordinates/secret bytes), mirroring `DecryptResultFfi` (`api.rs`). The `Status`/error variant carries ONLY a **closed enum of generic reasons — never a raw String** (Security Rule 8). FRB enum-with-data codegen verified in the regen smoke step (fallback = struct-of-options discriminant).
...
6. **Decrypt contract = explicit outcome at the FFI.** Replace the two `Ok(None)` decrypt returns in `api.rs` by surfacing the EXISTING core `LocationMessageResult` variants through `DecryptOutcomeFfi { Location | GroupUpdate | Unprocessable | PreviouslyFailed }`. Cursor advances ONLY on `Location`/`GroupUpdate`; `Unprocessable`/`PreviouslyFailed` leave the cursor put so the event replays.
...
Streams: `'group_445'`, `'inbox_1059'`. Single-identity Haven (an `account_pubkey` column is additive if multi-identity ever lands). **Advance is an atomic conditional UPDATE at the SQL layer** — `UPDATE sync_cursors SET last_synced_ms=:ms WHERE :ms > last_synced_ms` — NOT read-modify-write in Rust, so concurrent foreground/background writers across the App-Group DB cannot lose an update (WAL serializes file-level writes). **`since` derivation:** group `since = cursor_secs - 60` (resubscribe) / `- 10` (initial); ~~**inbox `since = cursor_secs - 604800` (7-day gift-wrap lookback) applied to EVERY kind-1059 REQ**, not just first subscription~~ **CORRECTED (power-efficiency P1): the 7-day window is the COLD-START one only.** `since_for_stream`'s inbox branch matches on `SubscribePhase`: `Initial` (no persisted inbox cursor) keeps `INBOX_GIFTWRAP_LOOKBACK_SECS` (7 d), and every `Resubscribe` — every engine start against a persisted cursor, every re-anchor, every `CLOSED` repair and every background burst open — asks for `INBOX_RESUBSCRIBE_LOOKBACK_SECS` = 2 d + 1 h, sized to NIP-59's 48 h backdating plus an hour of clock skew. Narrowing it further is not available: a wrap below the floor is lost silently. Cite the constants in `haven-core/src/relay/cursor.rs`, never a value. All `since` capped to `now`. **Seed-if-unset = `now - 24h`** for field circles (no full-history avalanche). **Bootstrap order is mandatory: seed cursor (M1) BEFORE starting any subscription (M3)**, so a `since=None` "send all history" window can never open.
...
- **T3:** ONE standing multiplexed `#h` REQ per relay-set REPLACES three jittered poll loops (30/60/3600s) → net fewer REQ ops + no connect/disconnect churn. Scope to own relay set + `nostr_group_id` only (never real MLS group id); sub-ids HMAC-hashed with a **per-session ephemeral salt (not persisted)**. Honest delta in SECURITY.md: a persistent socket creates a continuous online-presence signal + a long-lived (socket ↔ set-of-nostr_group_ids) correlation for the session — a metadata change vs short polls that cannot be eliminated while holding a live connection. **Amended P1/P4: that is now a FOREGROUND cost.** Android stops the engine at every pause; iOS with background sharing off stops it too, and iOS with sharing on PAUSES it between publishes, so backgrounded there the standing REQ becomes one bounded burst per publish tick — presence at EVERY burst instant, not only at publish instants: `LiveSyncCore::resume_burst` re-anchors every circle's REQ and dials the whole relay union while `BackgroundBurstCoordinator._publishPass` sends only to the circles whose own decorrelated tick came due, so a circle relay's REQ/CLOSE pair carries this device's `kind:445` for its circle on a fraction of bursts, and on none at all when the publish window refuses (no identity, disclosure unaccepted, permission revoked, GPS timeout) or consent flips inside the pass; an inbox-only relay sees the same shape one plane over (`INV-R-BACKGROUND-PRESENCE-ONLY-AT-PUBLISH`); mitigations = own-relays-only + Tor/VPN (Mullvad) onboarding guidance.
...
**Explicitly NOT a WN port** — WN has no adopt-winner/created_at-tiebreak/clear-and-adopt logic (it clears pending only on publish failure and delegates concurrent-commit handling to MDK). The sole precedent is Haven's own test in `circle/manager.rs`, which does manual (not deterministic-winner) convergence. M4 builds the deterministic primitive from scratch.
...
- **Background, CLLocationManager session active + background-sharing ON:** BEST-EFFORT, one bounded receive BURST per publish tick (every 72–168 s — the 120 s nominal cadence, jittered ±40 %), only while the OS keeps the location session warm and the process executable. *(SUPERSEDED TWICE — corrected P4: this bullet's "~90s cadence cursor-anchored receive timer (M7)" was replaced first by M11's standing engine socket and then by P4's burst plane, which holds no standing REQ and no socket between ticks and arms no receive timer of its own. Movement is not required — a stationary device publishes on the jittered cadence and each publish IS the receive. Two bounds worth stating here: the inbox REQ rides only every `INBOX_BURSTS_PER_REQ`-th burst, and an account with nothing publish-eligible has no ticks, so it receives nothing until the next foreground.)*
...
### Android
- **Foreground OR location-FGS running (background-sharing ON):** NEAR-GUARANTEED real-time via live StreamSink inside the FGS, **not bounded by any platform FGS timeout**. *(CORRECTED 2026-08-30, P0 doc-drift sweep — this bullet previously claimed an "Android 15 (targetSdk 35) 6-hour FGS timeout" applied. It does not. Android's own page says: "Currently, this restriction only applies to `dataSync` and `mediaProcessing` foreground service types" — developer.android.com, "Foreground service timeouts", https://developer.android.com/develop/background-work/services/fgs/timeout, verified 2026-08-30; Android 16/API 36 adds nothing to that list, https://developer.android.com/about/versions/16/behavior-changes-16, verified 2026-08-30. `foregroundServiceType=location` is not on it, so there is no ~6 h platform cap on Haven's FGS. Two hedges so this retraction is not itself over-broad: the quoted "Currently" is Google's own — the list can widen, so re-check the page before relying on it; and the separate `shortService` type does carry its own ~3-minute cap, which is a different restriction Haven does not use.)* What does still end the service is unchanged: the user dismissing it, an OEM battery killer, or process death. RebootReceiver (M7) re-arms on boot; the WorkManager 15-min floor + FCM/UnifiedPush wake (M9) provide best-effort catch-up until the FGS restarts or the app reopens.
...
- **Honest user-facing line:** "With background sharing on, Haven keeps a live relay connection and delivers in real time (Android can still end the service — you dismiss it, an OEM battery manager kills it, or the process dies — and it restarts and catches up from a saved sync point). GrapheneOS/de-Googled devices use UnifiedPush instead of Google push."
...
- `api.rs`'s `Unprocessable`→`Ok(None)` and `PreviouslyFailed`→`Ok(None)` decrypt returns — the silent-drop / cursor-skip bug. **CONFIRMED.**
- **The 4-way decrypt distinction ALREADY EXISTS in core:** `haven-core/src/nostr/mls/types.rs` `LocationMessageResult` has `Location|GroupUpdate|Unprocessable|PreviouslyFailed` with redacting `Debug`. The defect is PURELY at the FFI boundary (`api.rs` flattens two variants to `Ok(None)`). M1 surfaces existing variants; it does NOT re-architect core decrypt.
- `haven-core/src/relay/manager.rs` — one bare `Client::builder().build()`, no `verify_subscriptions`. **CONFIRMED.** the same file's dead-from-Flutter `subscribe()` using auto-id `subscribe_to`. **CONFIRMED.**
- `self_update_provider.dart` — hourly `groupsNeedingSelfUpdate(3600)`; its doc AND `groups_needing_self_update`'s own doc BOTH say the query returns groups "where the post-join rotation is incomplete." **=> Disabling this timer ALSO disables the MIP-02 post-join self-update. CONFIRMED** (corrected throughout — see M5/D2).
- **`finalize_pending_commit` production sites (full inventory):** `NostrCircleService`'s on-demand/self-update publish, its `_publishEvolutionEvent` (membership commit), `LocationSharingService`'s receiver auto-commit, and that service's evolution-poller site (DELETED by M6). **CONFIRMED — the original 2-site citation was incomplete; M4 wires all three live paths.**
...
- **MDK rev `93ae324` (parres-hq/mdk) — the SAME fork Haven already pins — DOES contain `mip05`** at `crates/mdk-core/src/mip05/` with `encrypt_push_token`/`decrypt_push_token` (`crypto.rs:18`), `build_notification_batches` (`notifications.rs:129`), kinds 446/447/448/449 (`types.rs:188-219`). `mip05` is a **declared-but-not-enabled feature** (`haven-core Cargo.toml` has NO `features=["mip05"]`). **=> M9's prerequisite is to ADD `features=["mip05"]` to the existing pin, NOT migrate to a different fork.**
...
- **M4 finalize-site inventory completed:** all live sites verified (the two `NostrCircleService` sites + `LocationSharingService`'s receiver auto-commit; its evolution-poller site deleted by M6). Added a CI grep-guard. The original 2-site citation was incomplete.
...
- **Android 6-hour FGS timeout corrected:** `foregroundServiceType=location` is NOT timeout-exempt on Android 15 (targetSdk=35); the matrix was downgraded from unbounded "NEAR-GUARANTEED" to "NEAR-GUARANTEED up to ~6h" with WorkManager/FCM/RebootReceiver recovery. **RETRACTED 2026-08-30 (P0 doc-drift sweep): the "correction" was itself wrong.** The Android 15 timeout applies — in Google's own words "**Currently**", so the scope can widen — only to `dataSync` and `mediaProcessing`; `location` was never subject to it, and there is no exempt-list to be off (https://developer.android.com/develop/background-work/services/fgs/timeout, verified 2026-08-30). The separate `shortService` type does have its own ~3-minute cap; Haven does not use it. The matrix bullet is restored to unbounded-by-the-platform.
...
- **Inbox 7-day gift-wrap lookback** now applied to EVERY kind-1059 REQ (cursor stores raw rumor timestamp; 7d subtract live in `since_for_stream`) — closes the dormant-device missed-gift-wrap gap. *(SUPERSEDED by power-efficiency P1: the 7 d window is the cold-start phase only; every `Resubscribe` asks for `INBOX_RESUBSCRIBE_LOOKBACK_SECS` = 2 d + 1 h. See the `since`-derivation correction above.)*
...
- **Buffer constants corrected:** 10s initial / 60s resubscribe / 7d gift-wrap (was conflated to 60s). *(SUPERSEDED by power-efficiency P1: the gift-wrap lookback is now phase-dependent — 7 d on `Initial`, 2 d + 1 h on `Resubscribe`.)*
...
- **Avatar epoch re-share side-effect** (`evolution_poller_provider.dart`) explicitly preserved in M6's `GroupUpdate` routing — closes the silent avatar-delivery regression.
...
- MDK `process_message` behavior under a held pending commit is documented (Haven's own test in `circle/manager.rs`) as VARIABLE across {`Err(OwnCommitPending)`, `Unprocessable`, silent-advance}. The M4 prerequisite test pins it for rev `93ae324`; a future MDK bump could change it — M4's tolerate-all-three design mitigates but does not eliminate this coupling.
...
```rust
// haven/rust_builder/src/api.rs — mirror TILE_CACHE
...
- **NEVER `.unwrap()` a global guard at the FFI boundary.** Every access: `SESSION.write().map_err(|_| "session lock poisoned".to_string())?` — the `current_cache` precedent (api.rs). A poisoned lock returns `Err`, never panics.
...
live_sync/
  ...
  config.rs     // BUS_CAP=8192, POOL_NOTIF_CAP=8192, WORKER_QUEUE_CAP, COMMIT_SETTLE_WINDOW_SECS=8,
                //   MAX_SETTLE_COMMITS=16, BACKOFF_MIN=1s/MAX=30s, SUB_ID_PREFIX_BYTES=8
                //   (HEALTH_CHECK_SECS=900 / RELAY_LIST_SECS=1800 were planned here and
                //   deleted 2026-08-30: they never gained a reader — the live maintenance
                //   timers are Dart: `subscriptionHealthInterval` and
                //   `relayListMaintenanceInterval` in `maintenance_scheduler_provider.dart`)
...
| Settle / fork safety | Regime split; regime-2 gate at the ROUTING layer BEFORE decrypt (sibling arrives as `Ok(Commit)`, NOT `Err`). | MDK process.rs:66-88; `circle/manager.rs` |
| NIP-42 | `automatic_authentication(false)` on the engine Client (default true). | sdk options.rs:93 |
| PSI-6 wording | "single SERIALIZED MLS writer" + `MlsWriteGate` per-circle mutex; the engine IS a writer. | `circle/manager.rs` |
| `FfiRelayEvent` shape | Struct-of-discriminant is PRIMARY (matches DecryptOutcomeFfi); tagged enum was the gated experiment (not needed — regen clean). | api.rs |
...
| settle JSON→Event | Re-parse in Rust helper; malformed = hard error, never silent drop. | `circle/manager.rs` |
| start idempotency | Hold SESSION write lock through the whole build; receiver+spawn before first REQ. | api.rs |
| Global lock access | `.map_err(...)?`, never `.unwrap()`. | api.rs |
...
```rust
// ───────── Globals (near TILE_CACHE in api.rs) — accessed via .map_err(...)?, never .unwrap() ─────────
static SESSION:        RwLock<Option<Arc<LiveSyncCore>>>      = RwLock::new(None);
...
// ───────── The streamed event — struct-of-discriminant (matches DecryptOutcomeFfi in api.rs) ─────────
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
// Hand-written Debug (mirrors DecryptOutcomeFfi Debug in api.rs): presence-bools + competing_commits_count only.
...
pub async fn run_catchup_all_circles(
    ...
    nostr_group_ids: &[[u8;32]],   // empty = all visible circles (get_visible_circles)
...
A persistent live socket is a continuous foreground online-presence signal plus a long-lived (this-socket ↔ set-of-`#h`) correlation. **Amended P1/P4: both residuals below are FOREGROUND costs now.** The engine holds no socket while backgrounded on either platform — stopped at every pause except iOS-with-sharing-on, where it is paused between publishes and each publish tick opens one bounded burst. The linkage in (2) is therefore per-burst there rather than per-session, and what remains is named in full under `INV-R-BACKGROUND-PRESENCE-ONLY-AT-PUBLISH` and in `haven-core/SECURITY.md`, "iOS background sharing: presence only at publish instants (P4)" — including what the narrowing does NOT cover: a circle relay reading a REQ/CLOSE pair on every burst whether or not its own `kind:445` rides that one, and an inbox-only relay reading the burst cadence itself as "this pubkey is background-sharing right now". Vs polling: M3 WINS on PCS-window (toward seconds) and connect/disconnect churn; NEUTRAL on content (still E2E, no key/secret/real-group-id on the wire); introduces TWO honestly-disclosed residuals:
...
- `converge_commit(&self, mls_group_id, our_commit, staged_epoch, competing_commits, intent) -> CommitConvergence{Merged | AdoptedWinner{intent_still_pending} | RolledBack}` (`circle/manager.rs`). Sync. Does NOT publish. Never leaves a dangling pending commit. Uses ungated `group_epoch_internal` (release-safe).
...
- **Amended P4: the tick is FOREGROUND-ONLY on every branch, gated at both ends.** It used to stay
  armed on the one branch a background Dart timer could reach — iOS with background sharing on, where
  the process stays executable — because a standing subscription was exactly what it existed to
  repair. P4 removes the thing being repaired, and the carve-out became a defect: `maintain_
  subscription_health` short-circuits only when `paused` is true ON ENTRY, and a burst clears `paused`
  for its whole duration, so a tick landing MID-BURST read a pool in the middle of `connect()` as
  dropped and repaired it through `resume_after_background()` — the FOREGROUND entry, which carries
  the inbox REQ at any `k`. One such tick leaves standing REQs, an open socket and a 49 h gift-wrap
  replay keyed on this npub at an instant that is not a publish, and it persists to the next burst's
  pause. The fix gates it where it is ARMED (the single arming site refuses while backgrounded, and
  the pause cancels the three timers gated at arming time — the fourth, the public-profile sweep,
  keeps its timer and gates inside its own tick) and where it FIRES (a foreground re-read, for a
  timer armed in the foreground that fires after the pause). On the burst branch the BURST is the
  repair, at 72-168 s rather than 15 min.
...
- **`SESSION` guard discipline (rust CRITICAL):** `live_session_core()` snapshots the `Arc` and
  DROPS the `std::sync` guard BEFORE any `.await` (mirror the existing global-guard accessors in `api.rs`), else `await_holding_lock`
...
- Files: `relay/live_sync/health.rs` (pure `health_needs_resubscribe` + presence-only
  ...
  `EngineOff`). Dart: `maintainSubscriptionHealth` forward (NO secret/circle) + the scheduler's health
  timer (foreground-only since P4 — the pause cancels it and `_armHealth` refuses to re-arm while
  backgrounded; the FFI still answers `engineOff` whenever the engine is down). No new deps (existing
  nostr-sdk `RelayStatus`).
