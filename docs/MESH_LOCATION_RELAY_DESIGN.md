...
| `tags[expiration]` | NIP-40, `created_at + 228 s` — a FIXED group constant, not jittered: the engine stamps it from the group's `message-retention.v1` component (0x8005) as `LOCATION_MESSAGE_RETENTION_SECS` (`haven-core/src/location/ttl.rs:85`), application messages only | **outer plaintext** |
...
Inside the NIP-44 layer is the serialized MLS `MLSMessage`/`PrivateMessage`, whose own payload (the inner kind-25442 rumor with the sender's **real** Nostr pubkey, no tags, and the `LocationMessage` JSON of `lat`/`lon`/geohash/timestamps) is AEAD-encrypted under the **MLS ciphersuite AEAD — AES-128-GCM** for Haven's `MLS_128_…_AES128GCM_…` suite (*not* ChaCha20). Privacy fields are `#[serde(skip)]`.
...
- **Persistent activity indicator** while active: the map's own sharing control is always visible, and the Android foreground-service notification signals it too. **On iOS the OS-level signal is the status-bar arrow, not a blue bar** — since power-plan phase P3 a positively confirmed "Always" grant releases the `CLBackgroundActivitySession` and clears `showsBackgroundLocationIndicator`, so those users get the arrow plus the Location Services listing and no blue bar; When-In-Use, a provisional Always and iOS 17 Always still show the bar. Do not design mesh disclosure around a bar being present on iOS.
...
The profile-pictures plan **deliberately** made a photo event and a location event **byte- and cadence-indistinguishable to any non-decrypting observer** (same outer **kind 445**, same inner application kind, same `["h", nostr_group_id]` tag, and a deliberately-collided avatar/location padding bucket). A BLE relay is exactly such a non-decrypting observer, so the mesh **cannot** classify a kind-445 blob as location-vs-photo by size, kind, tag, or padding bucket.

> **Void since 2026-07-16 (kept because the reasoning below is what survives).** Photos left the MLS tunnel entirely at the public-profile migration (`docs/PUBLIC_PROFILE_MIGRATION_PLAN.md`): avatars are public kind-0 + Blossom, `docs/PROFILE_PICTURES_PLAN.md` no longer exists, and the only inner application kind Haven emits is **25442** (location). There is no photo 445 left to be indistinguishable from — which changes nothing, because the paragraph below already refuses to rest on this premise.
...
- **Packet:** inner kind-25442 JSON ~180–260 B → inner MLS ciphertext + frame ~600–850 B → **pad to 1024 B block** → **2–3 fragments** at ~469 B chunk.
...
- **Freshness gate (mesh-frame field, no decrypt):** the mesh-frame expiry compared with `RECEIVER_EXPIRATION_GRACE_SECS = 60` (`haven-core/src/location/ttl.rs:49`). **CORRECTED 2026-08-30 (P0 doc-drift sweep).** This bullet used to derive the window from `compute_jittered_ttl_secs` (`[interval, 2*interval]` over `[72,168] s` = `[72,336] s`). Per-send TTL jitter was retired at the Dark Matter cutover (2026-08-13; `haven-core/SECURITY.md`, "Outer kind:445 metadata"): the helper has no production caller (`ttl.rs:11-16`) and the engine derives the NIP-40 `expiration` deterministically from the group's `message-retention.v1` component (0x8005) as the fixed `LOCATION_MESSAGE_RETENTION_SECS = 228 s` (`ttl.rs:85`). The real horizon is therefore a CONSTANT 228 s, not a range; with the +60 s receiver grace a beacon lives in the mesh **≤ 288 s (~4.8 min)** worst case, not ~6.6 min. The privacy consequence is recorded rather than hidden: a deterministic TTL *is* the constant-TTL fingerprint the retired sampling existed to prevent (`ttl.rs:17-20`) — but one shared with every Dark Matter client using the same engine derivation, rather than a per-send Haven quirk. Mesh-local receive time = coarse transport ordering only; semantic LWW is member-only on the inner timestamp.

### 11.5 Store-and-forward buffer
| Parameter | Default | Basis |
|---|---|---|
| Retention TTL | **align to frame expiry, hard cap ≈ 300 s** | beyond 228+60 s the beacon is useless (corrected 2026-08-30 with the fixed retention above; was ≈ 400 s off the retired jittered window) |
...
- **BLE platform landscape (conducted research; confirm on hardware in P2):** CoreBluetooth backgrounded service-UUID overflow-area substitution; Android 14 `FOREGROUND_SERVICE_CONNECTED_DEVICE` + runtime `BLUETOOTH_SCAN/CONNECT/ADVERTISE` (+ `neverForLocation`, a privacy win); Android 15 FGS-timeout scope (`dataSync`/`mediaProcessing` only, "currently"; `shortService` has its own ~3-min cap; `connectedDevice` is on neither list — https://developer.android.com/develop/background-work/services/fgs/timeout, verified 2026-08-30); Wi-Fi Aware (NAN) cross-platform immaturity; peripheral-role plugin capability (`bluetooth_low_energy` vs central-only `flutter_blue_plus`); MLS RFC 9420/9750; PADME/PURBs (PETS 2019); Signal sealed-sender break (Martiny et al., NDSS 2021).
...
- Location types/TTL: `haven-core/src/location/types.rs` (`LocationMessage`), `haven-core/src/location/ttl.rs` (**`LOCATION_MESSAGE_RETENTION_SECS = 228`** — the engine-stamped NIP-40 expiration, `:85`; `RECEIVER_EXPIRATION_GRACE_SECS = 60`, `:49`; `compute_jittered_ttl_secs` is RETIRED with no production caller, `:11-16` — corrected 2026-08-30; stable-`h`-tag leak note).
- Circle types: `haven-core/src/circle/types.rs` (`nostr_group_id`, MLS-id redaction `Debug`, `LastKnownLocation`).
- Kinds: `haven-core/src/nostr/event.rs` (`KIND_GROUP_MESSAGE=445`, `KIND_LOCATION_UPDATE=25442` — the inner location kind, stamped by `location_rumor`/`send_location` and gated on receive by `is_location_rumor` in `haven-core/src/nostr/mls/manager.rs`).
