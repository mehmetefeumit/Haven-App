...
**Owner decisions / open questions.** OD-P2-1, OD-P2-2, OD-P2-3. V-P2-1 (`dumpsys location` grammar on the CI API-34 image — pre-stated from `LocationRequest.toString()`, closed by the fixture, ~~confirmed on the first run~~ **DISPROVED on the first run that could see it, CI 34488512808 (2026-09-10)**: the fixtures had been written in the same unpadded form the parser searched for, so they agreed with each other and not with the device — `logcat -v threadtime` pads the tag to 8 columns (`b1power : SAMPLE`, not `b1power: SAMPLE`), AOSP pads the wake-lock level to 30 columns, and prints a sub-second duration as `-326ms` with no seconds field. All three are now parsed from the real capture and pinned by fixtures copied from it; a pre-stated grammar is a hypothesis until a lane has matched a real line against it); V-P2-2 (first FGS listen after `onStart` receives events — assert via a `[BackgroundTask] registration armed` marker followed by a delivery in B1); V-P2-3 → V (`PowerManager.WakeLock.acquire(timeout)` on a held non-refcounted lock re-posts the timer, `PowerManager.java:3930-3950, 3984-4000`); V-P2-4 → V (`addTaskLifecycleListener` from `Application.onCreate` precedes the boot-restart engine — `ForegroundService.kt:45-54,173-178`, `ForegroundTask.kt:47-70`, `RebootReceiver.kt:43-46`; confirm on `adb reboot` in M7 runbook step 7); I-P2-1 (HAL `CAPABILITY_SCHEDULING` on the owner's phones — informational, also decides the indoor residual); I-P2-2 (GMS fused duty-cycling); U-P2-1 (delivery→Dart wake window — P2b only; consequence stated in D4).
...
`INV-W-445-EXPIRATION-WINDOW.status` was DELETED from `ratchet_override.items`, which is why the block held **8** items (the whole block was then removed on 2026-09-10, post-merge, as P6's stale-override row requires)
...
| Stale overrides | ~~delete any `ratchet_override` left by P5 (and any phase that needed one); `check_privacy_invariants.sh --baseline-ref origin/main` must report 0 declared overrides.~~ **DONE 2026-09-10, in the first commit after the epic landed as `a59d0b9`.** All eight items became stale the moment the epic was `HEAD`: each allowed a weakening relative to the PRE-epic base, and against the new base none of them names a change. CI run 34488512808 stayed green on this only because it still diffed against the old base, so the NEXT push would have redded `repo-guards`. The whole block is removed (the gate treats an absent `ratchet_override` as no allowances, and only demands a reason when items exist); `--baseline-ref HEAD` now reports **0 declared override(s)**. |
...
| V-P2-1 | V (pre-stated) | `dumpsys location` `Request[…]` / `dumpsys power` `ACQ=` grammar on the CI API-34 image — derived from `LocationRequest.toString()` / `WakeLock.toString()` (§2.2) | P2a-6: shell fixtures in the SAME commit; ~~the first run confirms~~ **the first run that could see it DISPROVED three grammars (CI 34488512808) — tag padding, 30-column lock-level padding, sub-second `Nms` durations — because the fixtures shared the parser's wrong form; now re-derived from that run's capture** |
