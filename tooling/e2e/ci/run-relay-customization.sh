...
run_self_test() {
  ...
  echo "run-relay-customization.sh: self-test passed (the secret-leak gate removes exactly the *.log files it scanned on a leak, leaves a labels-only LEAK.marker, and touches nothing on rc 3 or rc 0)."
  return 0
}
...
# more relay/identity chatter than the single-relay lanes.
#
