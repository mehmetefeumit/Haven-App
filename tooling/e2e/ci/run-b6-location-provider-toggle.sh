...
run_self_test() {
  ...
  local extra_lit='readonly -a SEAL_EXTRA=(--host-decl "coordinate=${GEO_LAT},${GEO_LON}" --floor drive=79 --floor relay=7)'
...
#
# drive=79. The policy's 100 is sized for the Android core flow's 394-line
# transcript; this lane drives one scenario whose complete transcript is
# 159 lines (flutter-drive.log) in run 35376588206, the first green run after the
# gate's rollout. 79 is half of that and far above the ~17 lines `flutter
...
readonly -a SEAL_EXTRA=(--host-decl "coordinate=${GEO_LAT},${GEO_LON}" --floor drive=79 --floor relay=7)
