#!/usr/bin/env bash
#
# Runs a single-instance Haven E2E scenario on the action-booted
# emulator-5554. The architecture matches the two-AVD path
# (`run-two-avd-scenario.sh`):
#
#   Phase 1 — `flutter build apk`     (bake the APK once)
...
#
# `flutter test integration_test/foo.dart` builds + installs + drives
# in a single command, but it gives no opportunity to pre-grant
# runtime permissions before the production UI requests them. Android
# 12+ shows the permission dialog modally on top of Haven; the
# integration_test runner cannot dismiss it, and any test that
# interacts with the UI after that point fails because all taps land
# on the dialog instead of Haven. We hit this on scenario_01 when
# `Geolocator` requested `ACCESS_FINE_LOCATION` shortly after the
# `MapShell` mount, blocking Alice's drag-sheet → create-circle flow.
#
# Splitting the lifecycle lets us call `adb shell pm grant` between
# `adb install` and the first frame, which suppresses the dialog
...
# PIDs). Wrapping the entire flow in a single `bash <path>` invocation
# avoids that.
#
# Usage:
...
#
# ACCESS_BACKGROUND_LOCATION cannot be granted via `pm grant` on API
# 30+ — it requires the user to navigate through a Settings flow.
# Haven only requests it via the background-share opt-in, which no
# scenario currently exercises. We skip it deliberately rather than
# silently ignoring a failure.
# -----------------------------------------------------------------
