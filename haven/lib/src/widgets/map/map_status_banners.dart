...
///
/// ## Why one slot rather than several independent banners
///
/// Three surfaces exist that say "your location sharing is not working, and
/// here is why": [LocationAccessBanner] (the device cannot produce a fix, or
/// Haven is not allowed to read one), [ClockSkewBanner] (fixes are produced and
/// sent, but this device's clock makes them useless) and [SharingHealthBanner]
/// (fixes are produced, timestamped correctly, and still nothing is being
/// delivered). Each renders nothing while its own condition is fine, so all
/// three are individually safe to place unconditionally — which makes stacking
/// them the tempting thing to do, and the wrong one. They can be true at the
/// same time, and full-width error cards over the map would eat the viewport
/// and force the user to work out which one to act on first.
///
...
///
/// [ClockSkewBanner] comes next, ahead of [SharingHealthBanner], for the same
/// upstream reason one level down. A wrong clock IS a cause of the delivery
/// silence the health banner measures — a publisher lagging past the retention
/// window has every event discarded by correctly-clocked peers while relays
/// still ACK it — so with skew present the health banner would be reporting the
/// symptom while the banner underneath it names the cause. Worse, its remedy
/// would be actively wrong: "Repair" reconnects and republishes, which a skewed
/// clock makes fail again identically, whereas the clock banner sends the user
/// to the setting that actually fixes it.
///
/// ## Why only the LOWER-precedence banners are conditional
///
...
/// two drift apart later. So the winner is built unconditionally and left to
/// hide itself, and the ONE thing this widget adds is the precedence: each
/// lower banner is suppressed while any banner above it has something to say.
///
...
///
/// The clock and sharing-health banners are the conditional ones because
/// neither has such an edge, and because not building them while a
/// higher-precedence banner is up also keeps them from announcing the recovery
/// of a warning the user was never shown.
library;
...
class MapStatusBanners extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final clockSkewed = ref.watch(clockSkewProvider).isSkewed;
    ...
    // the caller supplies and put the banners back to running off the
    ...
    return Stack(
      children: [
        ...
        if (!accessBlocked && !clockSkewed) const SharingHealthBanner(),
      ],
    );
  }
}
