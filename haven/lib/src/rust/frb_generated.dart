// This file is automatically generated, so please do not edit it.
// @generated by `flutter_rust_bridge`@ 2.11.1.

// ignore_for_file: unused_import, unused_element, unnecessary_import, duplicate_ignore, invalid_use_of_internal_member, annotate_overrides, non_constant_identifier_names, curly_braces_in_flow_control_structures, prefer_const_literals_to_create_immutables, unused_field

import 'api.dart';
import 'dart:async';
import 'dart:convert';
import 'frb_generated.dart';
import 'frb_generated.io.dart'
    if (dart.library.js_interop) 'frb_generated.web.dart';
import 'package:flutter_rust_bridge/flutter_rust_bridge_for_generated.dart';

/// Main entrypoint of the Rust API
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  @internal
  static final instance = RustLib._();

  RustLib._();

  /// Initialize flutter_rust_bridge
  static Future<void> init({
    RustLibApi? api,
    BaseHandler? handler,
    ExternalLibrary? externalLibrary,
    bool forceSameCodegenVersion = true,
  }) async {
    await instance.initImpl(
      api: api,
      handler: handler,
      externalLibrary: externalLibrary,
      forceSameCodegenVersion: forceSameCodegenVersion,
    );
  }

  /// Initialize flutter_rust_bridge in mock mode.
  /// No libraries for FFI are loaded.
  static void initMock({required RustLibApi api}) {
    instance.initMockImpl(api: api);
  }

  /// Dispose flutter_rust_bridge
  ///
  /// The call to this function is optional, since flutter_rust_bridge (and everything else)
  /// is automatically disposed when the app stops.
  static void dispose() => instance.disposeImpl();

  @override
  ApiImplConstructor<RustLibApiImpl, RustLibWire> get apiImplConstructor =>
      RustLibApiImpl.new;

  @override
  WireConstructor<RustLibWire> get wireConstructor =>
      RustLibWire.fromExternalLibrary;

  @override
  Future<void> executeRustInitializers() async {
    await api.crateApiInitApp();
  }

  @override
  ExternalLibraryLoaderConfig get defaultExternalLibraryLoaderConfig =>
      kDefaultExternalLibraryLoaderConfig;

  @override
  String get codegenVersion => '2.11.1';

  @override
  int get rustContentHash => -234967442;

  static const kDefaultExternalLibraryLoaderConfig =
      ExternalLibraryLoaderConfig(
        stem: 'rust_lib_haven',
        ioDirectory: 'rust_builder/target/release/',
        webPrefix: 'pkg/',
      );
}

abstract class RustLibApi extends BaseApi {
  Future<void> crateApiCircleManagerFfiAbandonCircleLocalOnly({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<CircleWithMembersFfi> crateApiCircleManagerFfiAcceptInvitation({
    required CircleManagerFfi that,
    required List<int> giftWrapId,
  });

  Future<AddMembersResultFfi> crateApiCircleManagerFfiAddMembersToCircle({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<int> mlsGroupId,
    required List<MemberKeyPackageFfi> members,
    required List<String> creatorFallbackRelays,
  });

  Future<void> crateApiCircleManagerFfiAddUserRelay({
    required CircleManagerFfi that,
    required String url,
    required RelayTypeFfi relayType,
  });

  Future<BuiltRelayListEventFfi> crateApiCircleManagerFfiBuildRelayListPublish({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  });

  Future<BuiltUnpublishFfi> crateApiCircleManagerFfiBuildRelayRemovalScrub({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
    required List<String> droppedRelays,
  });

  Future<BuiltUnpublishFfi> crateApiCircleManagerFfiBuildUnpublishRelayList({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  });

  Future<CircleHealthFfi> crateApiCircleManagerFfiCircleHealth({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
  });

  Future<void> crateApiCircleManagerFfiCompleteLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<void> crateApiCircleManagerFfiConfirmPublished({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
  });

  Future<CircleCreationResultFfi> crateApiCircleManagerFfiCreateCircle({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<MemberKeyPackageFfi> members,
    required String name,
    String? description,
    required String circleType,
    required List<String> relays,
    required List<String> creatorFallbackRelays,
  });

  Future<PlatformInt64?> crateApiCircleManagerFfiCursorGet({
    required CircleManagerFfi that,
    required String stream,
  });

  Future<void> crateApiCircleManagerFfiCursorReset({
    required CircleManagerFfi that,
    required String stream,
  });

  Future<void> crateApiCircleManagerFfiDeclineInvitation({
    required CircleManagerFfi that,
    required List<int> giftWrapId,
  });

  Future<List<LocationMessageResultFfi>>
  crateApiCircleManagerFfiDecryptLocation({
    required CircleManagerFfi that,
    required String eventJson,
  });

  Future<DecryptLocationOutcomeFfi>
  crateApiCircleManagerFfiDecryptLocationCollectingCommits({
    required CircleManagerFfi that,
    required String eventJson,
  });

  Future<void> crateApiCircleManagerFfiDeleteContact({
    required CircleManagerFfi that,
    required String pubkey,
  });

  Future<void> crateApiCircleManagerFfiDeleteMyPublicProfile({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  });

  Future<void> crateApiCircleManagerFfiDownloadMemberPicture({
    required CircleManagerFfi that,
    required String pubkeyHex,
  });

  Future<List<String>> crateApiCircleManagerFfiDownloadMemberPictures({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
  });

  Future<EncryptLocationOutcomeFfi> crateApiCircleManagerFfiEncryptLocation({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String senderPubkeyHex,
    required double latitude,
    required double longitude,
    required BigInt updateIntervalSecs,
  });

  Future<List<ProfileMetadataFfi>> crateApiCircleManagerFfiFetchMemberProfiles({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
    required PlatformInt64 maxAgeSecs,
  });

  Future<ProfileMetadataFfi> crateApiCircleManagerFfiFetchMyProfile({
    required CircleManagerFfi that,
    required String pubkeyHex,
  });

  Future<void> crateApiCircleManagerFfiFinalizeRelayUpdate({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
    required List<int> mlsGroupId,
  });

  Future<List<ContactFfi>> crateApiCircleManagerFfiGetAllContacts({
    required CircleManagerFfi that,
  });

  Future<ProfileMetadataFfi?> crateApiCircleManagerFfiGetCachedProfile({
    required CircleManagerFfi that,
    required String pubkeyHex,
  });

  Future<List<ProfileMetadataFfi>> crateApiCircleManagerFfiGetCachedProfiles({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
  });

  Future<CircleWithMembersFfi?> crateApiCircleManagerFfiGetCircle({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<List<CircleWithMembersFfi>> crateApiCircleManagerFfiGetCircles({
    required CircleManagerFfi that,
  });

  Future<ContactFfi?> crateApiCircleManagerFfiGetContact({
    required CircleManagerFfi that,
    required String pubkey,
  });

  Future<List<CircleMemberFfi>> crateApiCircleManagerFfiGetMembers({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<List<InvitationFfi>> crateApiCircleManagerFfiGetPendingInvitations({
    required CircleManagerFfi that,
  });

  Future<Uint8List?> crateApiCircleManagerFfiGetProfilePicture({
    required CircleManagerFfi that,
    required String pubkeyHex,
  });

  Future<Uint8List?> crateApiCircleManagerFfiGetProfileThumbnail({
    required CircleManagerFfi that,
    required String pubkeyHex,
  });

  Future<bool> crateApiCircleManagerFfiGetPublishRelayList({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  });

  Future<List<CircleWithMembersFfi>> crateApiCircleManagerFfiGetVisibleCircles({
    required CircleManagerFfi that,
  });

  Future<BigInt> crateApiCircleManagerFfiGroupEpoch({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<List<String>> crateApiCircleManagerFfiListUserRelays({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  });

  Future<CircleManagerFfi> crateApiCircleManagerFfiNew({
    required String dataDir,
    required List<int> identitySecretBytes,
  });

  Future<void> crateApiCircleManagerFfiNotePeerEvent({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  });

  Future<void> crateApiCircleManagerFfiNotePublishAcked({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  });

  Future<BigInt> crateApiCircleManagerFfiOutgoingLocationExpirySecs({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<LeavePlanFfi> crateApiCircleManagerFfiPlanLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String selfPubkeyHex,
  });

  Future<InvitationFfi?> crateApiCircleManagerFfiProcessGiftWrappedInvitation({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required String giftWrapEventJson,
  });

  Future<ProfilePendingStateFfi> crateApiCircleManagerFfiProfilePendingState({
    required CircleManagerFfi that,
  });

  Future<ProfilePoolStatusFfi> crateApiCircleManagerFfiProfilePoolStatus({
    required CircleManagerFfi that,
  });

  Future<CommitToPublishFfi> crateApiCircleManagerFfiProposeAdminHandoff({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String successorHex,
  });

  Future<String> crateApiCircleManagerFfiProposeLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<CommitToPublishFfi> crateApiCircleManagerFfiProposeSelfDemote({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  });

  Future<int> crateApiCircleManagerFfiPruneExpiredLastKnown({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  });

  Future<BigInt> crateApiCircleManagerFfiPruneProcessedGiftWraps({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  });

  Future<void> crateApiCircleManagerFfiPublishFailed({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
  });

  Future<List<DirectoryEntryFfi>>
  crateApiCircleManagerFfiRankedDirectoryMembers({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  });

  Future<bool> crateApiCircleManagerFfiReconcileMemberDirectory({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  });

  Future<void> crateApiCircleManagerFfiRecordPublishedRelayList({
    required CircleManagerFfi that,
    required String identityPubkeyHex,
    required int kind,
    required String eventIdHex,
    required PlatformInt64 publishedAtSecs,
  });

  Future<List<String>> crateApiCircleManagerFfiRelayPublishTargets({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  });

  Future<void> crateApiCircleManagerFfiRemoveLastKnownCircle({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
  });

  Future<void> crateApiCircleManagerFfiRemoveLastKnownMember({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required String senderPubkey,
  });

  Future<CommitToPublishFfi> crateApiCircleManagerFfiRemoveMembers({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required List<String> memberPubkeys,
  });

  Future<ProfileMetadataFfi> crateApiCircleManagerFfiRemoveMyProfilePicture({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  });

  Future<bool> crateApiCircleManagerFfiRemoveUserRelay({
    required CircleManagerFfi that,
    required String url,
    required RelayTypeFfi relayType,
  });

  Future<RepairRotationOutcomeFfi> crateApiCircleManagerFfiRepairEpochRotation({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required BigInt nowSecs,
  });

  Future<void> crateApiCircleManagerFfiResetAllSyncCursors({
    required CircleManagerFfi that,
  });

  Future<ProfileMetadataFfi?> crateApiCircleManagerFfiResolveStrangerProfile({
    required CircleManagerFfi that,
    required String pubkey,
    required PlatformInt64 maxAgeSecs,
  });

  Future<void> crateApiCircleManagerFfiRestoreDefaultProfileRelays({
    required CircleManagerFfi that,
  });

  Future<void> crateApiCircleManagerFfiRestoreDefaultsFor({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  });

  Future<ProfileMetadataFfi> crateApiCircleManagerFfiSaveMyProfileLocal({
    required CircleManagerFfi that,
    String? displayName,
    String? about,
  });

  Future<ProfilePictureRefFfi>
  crateApiCircleManagerFfiSaveMyProfilePictureLocal({
    required CircleManagerFfi that,
    required List<int> raw,
  });

  Future<bool> crateApiCircleManagerFfiSeedRelayDefaultsIfUnseeded({
    required CircleManagerFfi that,
  });

  Future<ContactFfi> crateApiCircleManagerFfiSetContact({
    required CircleManagerFfi that,
    required String pubkey,
    String? displayName,
    String? notes,
  });

  void crateApiCircleManagerFfiSetLocalNickname({
    required CircleManagerFfi that,
    required String pubkeyHex,
    String? nickname,
  });

  Future<void> crateApiCircleManagerFfiSetPublishRelayList({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
    required bool value,
  });

  String crateApiCircleManagerFfiSignDeletionEvent({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<String> eventIds,
  });

  Future<List<LastKnownLocationFfi>>
  crateApiCircleManagerFfiSnapshotLastKnownForCircle({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 nowUnixSecs,
  });

  Future<bool> crateApiCircleManagerFfiStillAMember({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String pubkeyHex,
  });

  Future<ConvergenceSweepFfi> crateApiCircleManagerFfiSweepUnresolvableInputs({
    required CircleManagerFfi that,
    required BigInt nowSecs,
  });

  Future<ProfileSyncResultFfi> crateApiCircleManagerFfiSyncMyProfile({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  });

  Future<CommitToPublishFfi> crateApiCircleManagerFfiUpdateCircleRelays({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required List<String> newRelays,
  });

  Future<void> crateApiCircleManagerFfiUpsertLastKnownLocation({
    required CircleManagerFfi that,
    required LastKnownLocationFfi location,
  });

  Future<void> crateApiCircleManagerFfiWipeAllLastKnownLocations({
    required CircleManagerFfi that,
  });

  Future<void> crateApiCircleManagerFfiWipeAndResetDefaultsFor({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  });

  Future<HavenCore> crateApiHavenCoreDefault();

  LocationSettings crateApiHavenCoreGetLocationSettings({
    required HavenCore that,
  });

  Future<void> crateApiHavenCoreInitialize({required HavenCore that});

  bool crateApiHavenCoreIsInitialized({required HavenCore that});

  Future<HavenCore> crateApiHavenCoreNew();

  void crateApiHavenCoreSetLocationSettings({
    required HavenCore that,
    required LocationSettings settings,
  });

  LocationMessage crateApiHavenCoreUpdateLocation({
    required HavenCore that,
    required double latitude,
    required double longitude,
  });

  Future<int> crateApiLiveSyncFfiInFlightPublishes({required LiveSyncFfi that});

  bool crateApiLiveSyncFfiIsPaused({required LiveSyncFfi that});

  bool crateApiLiveSyncFfiIsRunning({required LiveSyncFfi that});

  Stream<FfiRelayEvent> crateApiLiveSyncFfiLiveEvents({
    required LiveSyncFfi that,
  });

  Future<LiveSyncFfi> crateApiLiveSyncFfiNewInstance({
    required CircleManagerFfi circle,
    required String ownPubkeyHex,
  });

  Future<void> crateApiLiveSyncFfiOpenBackgroundBurst({
    required LiveSyncFfi that,
  });

  Future<void> crateApiLiveSyncFfiPauseSubscriptions({
    required LiveSyncFfi that,
  });

  Future<int> crateApiLiveSyncFfiPoolSubscriptionCount({
    required LiveSyncFfi that,
  });

  Future<void> crateApiLiveSyncFfiResumeAfterBackground({
    required LiveSyncFfi that,
  });

  Future<void> crateApiLiveSyncFfiSettleBeforePause({
    required LiveSyncFfi that,
  });

  Future<void> crateApiLiveSyncFfiStartSession({
    required LiveSyncFfi that,
    required List<FfiGroupSpec> groups,
    required List<String> inboxRelays,
  });

  Future<void> crateApiLiveSyncFfiStopSession({required LiveSyncFfi that});

  Future<void> crateApiLiveSyncFfiSubscribeCircle({
    required LiveSyncFfi that,
    required FfiGroupSpec spec,
  });

  Future<void> crateApiLiveSyncFfiUnsubscribeCircle({
    required LiveSyncFfi that,
    required List<int> nostrGroupId,
  });

  Future<BacklogOutcomeFfi> crateApiLiveSyncFfiWaitBacklogSettled({
    required LiveSyncFfi that,
  });

  Future<LocationEventService> crateApiLocationEventServiceDefault();

  BigInt crateApiLocationEventServiceJitteredPublishIntervalSecs({
    required LocationEventService that,
    required BigInt nominalSecs,
  });

  LocationEventService crateApiLocationEventServiceNew();

  bool crateApiLocationEventServiceVerifySignature({
    required LocationEventService that,
    required SignedLocationEventFfi event,
  });

  PlatformInt64 crateApiLocationMessageExpiresAt({
    required LocationMessage that,
  });

  String crateApiLocationMessageGeohash({required LocationMessage that});

  bool crateApiLocationMessageIsExpired({required LocationMessage that});

  double crateApiLocationMessageLatitude({required LocationMessage that});

  double crateApiLocationMessageLongitude({required LocationMessage that});

  PlatformInt64 crateApiLocationMessageTimestamp({
    required LocationMessage that,
  });

  Future<LocationSettings> crateApiLocationSettingsNew({
    required int updateIntervalMinutes,
  });

  int crateApiLocationSettingsUpdateIntervalMinutes({
    required LocationSettings that,
  });

  Future<void> crateApiNostrIdentityManagerClearCache({
    required NostrIdentityManager that,
  });

  Future<PublicIdentity> crateApiNostrIdentityManagerCreateIdentity({
    required NostrIdentityManager that,
  });

  Future<NostrIdentityManager> crateApiNostrIdentityManagerDefault();

  Future<void> crateApiNostrIdentityManagerDeleteIdentity({
    required NostrIdentityManager that,
  });

  Future<String> crateApiNostrIdentityManagerExportNsec({
    required NostrIdentityManager that,
  });

  PublicIdentity? crateApiNostrIdentityManagerGetIdentity({
    required NostrIdentityManager that,
  });

  Future<Uint8List> crateApiNostrIdentityManagerGetSecretBytes({
    required NostrIdentityManager that,
  });

  bool crateApiNostrIdentityManagerHasIdentity({
    required NostrIdentityManager that,
  });

  Future<PublicIdentity> crateApiNostrIdentityManagerImportFromNsec({
    required NostrIdentityManager that,
    required String nsec,
  });

  Future<PublicIdentity> crateApiNostrIdentityManagerLoadFromBytes({
    required NostrIdentityManager that,
    required List<int> secretBytes,
  });

  Future<NostrIdentityManager> crateApiNostrIdentityManagerNew();

  String crateApiNostrIdentityManagerPubkeyHex({
    required NostrIdentityManager that,
  });

  Future<String> crateApiNostrIdentityManagerSign({
    required NostrIdentityManager that,
    required List<int> messageHash,
  });

  Future<RelayEventCheckFfi> crateApiRelayManagerFfiCheckEventOnRelay({
    required RelayManagerFfi that,
    required String relayUrl,
    required String authorPubkey,
    required int eventKind,
  });

  Future<void> crateApiRelayManagerFfiDisconnectRelay({
    required RelayManagerFfi that,
    required String url,
  });

  Future<List<String>> crateApiRelayManagerFfiFetchGiftWraps({
    required RelayManagerFfi that,
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  });

  Future<List<RelayGiftWrapFetchFfi>>
  crateApiRelayManagerFfiFetchGiftWrapsPerRelay({
    required RelayManagerFfi that,
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  });

  Future<List<String>> crateApiRelayManagerFfiFetchGroupMessages({
    required RelayManagerFfi that,
    required List<int> nostrGroupId,
    required List<String> relays,
    PlatformInt64? since,
    int? limit,
  });

  Future<String?> crateApiRelayManagerFfiFetchKeypackage({
    required RelayManagerFfi that,
    required String pubkey,
  });

  Future<List<String>> crateApiRelayManagerFfiFetchKeypackageRelays({
    required RelayManagerFfi that,
    required String pubkey,
  });

  Future<MemberKeyPackageFfi?> crateApiRelayManagerFfiFetchMemberKeypackage({
    required RelayManagerFfi that,
    required String pubkey,
  });

  Future<List<String>> crateApiRelayManagerFfiFetchNip65Relays({
    required RelayManagerFfi that,
    required String pubkey,
  });

  Future<List<RelayConnectionStatusFfi>> crateApiRelayManagerFfiGetRelayStatus({
    required RelayManagerFfi that,
  });

  Future<KpMaintenanceOutcomeFfi> crateApiRelayManagerFfiMaintainKeyPackage({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  });

  Future<RelayListMaintenanceOutcomeFfi>
  crateApiRelayManagerFfiMaintainRelayList({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  });

  Future<RelayManagerFfi> crateApiRelayManagerFfiNewInstance();

  Future<PublishResultFfi> crateApiRelayManagerFfiPublishEvent({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  });

  Future<void> crateApiRelayManagerFfiPublishEventFireAndForget({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  });

  Future<PublishResultFfi> crateApiRelayManagerFfiPublishLocationEvent({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  });

  Future<LegacyRetractionOutcomeFfi>
  crateApiRelayManagerFfiRetractLegacyKeyMaterial({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  });

  Future<CatchupResultFfi> crateApiRelayManagerFfiRunCatchupAllCircles({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required String ownPubkeyHex,
    required BigInt maxDurationSecs,
  });

  Future<void> crateApiRelayManagerFfiShutdown({required RelayManagerFfi that});

  void crateApiAllowPrivateBlossomForTest();

  void crateApiAllowWsLoopbackForTest();

  Future<CircleHealthFfi> crateApiCircleHealthFfiDefault();

  Future<ConvergenceSweepFfi> crateApiConvergenceSweepFfiDefault();

  List<String> crateApiDefaultRelays();

  Future<void> crateApiDestroyLegacyMlsState({required String dataDir});

  List<String> crateApiDiscoveryRelays();

  String crateApiFoldForSearch({required String query});

  Future<ForceReleaseOutcomeFfi> crateApiForceReleaseLiveSession();

  Future<void> crateApiInitApp();

  Future<void> crateApiInitKeyringStore();

  Future<bool> crateApiIsSessionLive({required String dataDir});

  Future<LegacyRetractionOutcomeFfi>
  crateApiLegacyRetractionOutcomeFfiDefault();

  String crateApiLogAlias({
    required LogAliasClassFfi class_,
    required String value,
  });

  Future<SubscriptionHealthOutcomeFfi> crateApiMaintainSubscriptionHealth();

  Future<DecryptedLocationFfi> crateApiParseEngineLocation({
    required String contentJson,
    required String senderPubkey,
  });

  void crateApiRotateLogAliasSalt();

  void crateApiSetBlossomServerForTest({required String url});

  void crateApiSetDefaultRelaysForTest({required List<String> relays});

  void crateApiSetDiscoveryRelaysForTest({required List<String> relays});

  void crateApiSetProfileRelaysForTest({required List<String> relays});

  Future<BigInt> crateApiTileCacheEvict({
    required PlatformInt64 maxBytes,
    required PlatformInt64 idleAgeSecs,
    required PlatformInt64 maxRetentionSecs,
  });

  Future<TileCacheEntryFfi?> crateApiTileCacheGet({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
  });

  Future<void> crateApiTileCacheInit({required String dataDir});

  Future<void> crateApiTileCachePut({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
    required List<int> bytes,
    required PlatformInt64 staleAtMs,
    PlatformInt64? lastModifiedMs,
    String? etag,
  });

  Future<void> crateApiTileCachePutMetadata({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
    required PlatformInt64 staleAtMs,
    PlatformInt64? lastModifiedMs,
    String? etag,
  });

  Future<void> crateApiTileCacheWipe();

  Future<void> crateApiUseInMemoryKeyringForTest();

  Future<void> crateApiWipeAllMlsState({required String dataDir});

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_CircleManagerFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_CircleManagerFfi;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_CircleManagerFfiPtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_HavenCore;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_HavenCore;

  CrossPlatformFinalizerArg get rust_arc_decrement_strong_count_HavenCorePtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LiveSyncFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LiveSyncFfi;

  CrossPlatformFinalizerArg get rust_arc_decrement_strong_count_LiveSyncFfiPtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationEventService;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationEventService;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_LocationEventServicePtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationMessage;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationMessage;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_LocationMessagePtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationSettings;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationSettings;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_LocationSettingsPtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_NostrIdentityManager;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_NostrIdentityManager;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_NostrIdentityManagerPtr;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_RelayManagerFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_RelayManagerFfi;

  CrossPlatformFinalizerArg
  get rust_arc_decrement_strong_count_RelayManagerFfiPtr;
}

class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  RustLibApiImpl({
    required super.handler,
    required super.wire,
    required super.generalizedFrbRustBinding,
    required super.portManager,
  });

  @override
  Future<void> crateApiCircleManagerFfiAbandonCircleLocalOnly({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 1,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiAbandonCircleLocalOnlyConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiAbandonCircleLocalOnlyConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_abandon_circle_local_only",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<CircleWithMembersFfi> crateApiCircleManagerFfiAcceptInvitation({
    required CircleManagerFfi that,
    required List<int> giftWrapId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(giftWrapId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 2,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_circle_with_members_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiAcceptInvitationConstMeta,
        argValues: [that, giftWrapId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiAcceptInvitationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_accept_invitation",
        argNames: ["that", "giftWrapId"],
      );

  @override
  Future<AddMembersResultFfi> crateApiCircleManagerFfiAddMembersToCircle({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<int> mlsGroupId,
    required List<MemberKeyPackageFfi> members,
    required List<String> creatorFallbackRelays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_list_member_key_package_ffi(members, serializer);
          sse_encode_list_String(creatorFallbackRelays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 3,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_add_members_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiAddMembersToCircleConstMeta,
        argValues: [
          that,
          identitySecretBytes,
          mlsGroupId,
          members,
          creatorFallbackRelays,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiAddMembersToCircleConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_add_members_to_circle",
        argNames: [
          "that",
          "identitySecretBytes",
          "mlsGroupId",
          "members",
          "creatorFallbackRelays",
        ],
      );

  @override
  Future<void> crateApiCircleManagerFfiAddUserRelay({
    required CircleManagerFfi that,
    required String url,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(url, serializer);
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 4,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiAddUserRelayConstMeta,
        argValues: [that, url, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiAddUserRelayConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_add_user_relay",
        argNames: ["that", "url", "relayType"],
      );

  @override
  Future<BuiltRelayListEventFfi> crateApiCircleManagerFfiBuildRelayListPublish({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 5,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_built_relay_list_event_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiBuildRelayListPublishConstMeta,
        argValues: [that, identitySecretBytes, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiBuildRelayListPublishConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_build_relay_list_publish",
        argNames: ["that", "identitySecretBytes", "relayType"],
      );

  @override
  Future<BuiltUnpublishFfi> crateApiCircleManagerFfiBuildRelayRemovalScrub({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
    required List<String> droppedRelays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_relay_type_ffi(relayType, serializer);
          sse_encode_list_String(droppedRelays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 6,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_built_unpublish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiBuildRelayRemovalScrubConstMeta,
        argValues: [that, identitySecretBytes, relayType, droppedRelays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiBuildRelayRemovalScrubConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_build_relay_removal_scrub",
        argNames: ["that", "identitySecretBytes", "relayType", "droppedRelays"],
      );

  @override
  Future<BuiltUnpublishFfi> crateApiCircleManagerFfiBuildUnpublishRelayList({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 7,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_built_unpublish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiBuildUnpublishRelayListConstMeta,
        argValues: [that, identitySecretBytes, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiBuildUnpublishRelayListConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_build_unpublish_relay_list",
        argNames: ["that", "identitySecretBytes", "relayType"],
      );

  @override
  Future<CircleHealthFfi> crateApiCircleManagerFfiCircleHealth({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 8,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_circle_health_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiCircleHealthConstMeta,
        argValues: [that, nostrGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiCircleHealthConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_circle_health",
        argNames: ["that", "nostrGroupId"],
      );

  @override
  Future<void> crateApiCircleManagerFfiCompleteLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 9,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiCompleteLeaveConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiCompleteLeaveConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_complete_leave",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<void> crateApiCircleManagerFfiConfirmPublished({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_box_autoadd_pending_state_ref_ffi(pending, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 10,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiConfirmPublishedConstMeta,
        argValues: [that, pending],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiConfirmPublishedConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_confirm_published",
        argNames: ["that", "pending"],
      );

  @override
  Future<CircleCreationResultFfi> crateApiCircleManagerFfiCreateCircle({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<MemberKeyPackageFfi> members,
    required String name,
    String? description,
    required String circleType,
    required List<String> relays,
    required List<String> creatorFallbackRelays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_list_member_key_package_ffi(members, serializer);
          sse_encode_String(name, serializer);
          sse_encode_opt_String(description, serializer);
          sse_encode_String(circleType, serializer);
          sse_encode_list_String(relays, serializer);
          sse_encode_list_String(creatorFallbackRelays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 11,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_circle_creation_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiCreateCircleConstMeta,
        argValues: [
          that,
          identitySecretBytes,
          members,
          name,
          description,
          circleType,
          relays,
          creatorFallbackRelays,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiCreateCircleConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_create_circle",
        argNames: [
          "that",
          "identitySecretBytes",
          "members",
          "name",
          "description",
          "circleType",
          "relays",
          "creatorFallbackRelays",
        ],
      );

  @override
  Future<PlatformInt64?> crateApiCircleManagerFfiCursorGet({
    required CircleManagerFfi that,
    required String stream,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(stream, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 12,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_i_64,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiCursorGetConstMeta,
        argValues: [that, stream],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiCursorGetConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_cursor_get",
        argNames: ["that", "stream"],
      );

  @override
  Future<void> crateApiCircleManagerFfiCursorReset({
    required CircleManagerFfi that,
    required String stream,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(stream, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 13,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiCursorResetConstMeta,
        argValues: [that, stream],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiCursorResetConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_cursor_reset",
        argNames: ["that", "stream"],
      );

  @override
  Future<void> crateApiCircleManagerFfiDeclineInvitation({
    required CircleManagerFfi that,
    required List<int> giftWrapId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(giftWrapId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 14,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDeclineInvitationConstMeta,
        argValues: [that, giftWrapId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDeclineInvitationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_decline_invitation",
        argNames: ["that", "giftWrapId"],
      );

  @override
  Future<List<LocationMessageResultFfi>>
  crateApiCircleManagerFfiDecryptLocation({
    required CircleManagerFfi that,
    required String eventJson,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(eventJson, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 15,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_location_message_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDecryptLocationConstMeta,
        argValues: [that, eventJson],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDecryptLocationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_decrypt_location",
        argNames: ["that", "eventJson"],
      );

  @override
  Future<DecryptLocationOutcomeFfi>
  crateApiCircleManagerFfiDecryptLocationCollectingCommits({
    required CircleManagerFfi that,
    required String eventJson,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(eventJson, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 16,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_decrypt_location_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta:
            kCrateApiCircleManagerFfiDecryptLocationCollectingCommitsConstMeta,
        argValues: [that, eventJson],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiDecryptLocationCollectingCommitsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_decrypt_location_collecting_commits",
        argNames: ["that", "eventJson"],
      );

  @override
  Future<void> crateApiCircleManagerFfiDeleteContact({
    required CircleManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 17,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDeleteContactConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDeleteContactConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_delete_contact",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<void> crateApiCircleManagerFfiDeleteMyPublicProfile({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 18,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDeleteMyPublicProfileConstMeta,
        argValues: [that, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDeleteMyPublicProfileConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_delete_my_public_profile",
        argNames: ["that", "identitySecretBytes"],
      );

  @override
  Future<void> crateApiCircleManagerFfiDownloadMemberPicture({
    required CircleManagerFfi that,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 19,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDownloadMemberPictureConstMeta,
        argValues: [that, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDownloadMemberPictureConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_download_member_picture",
        argNames: ["that", "pubkeyHex"],
      );

  @override
  Future<List<String>> crateApiCircleManagerFfiDownloadMemberPictures({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_String(pubkeysHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 20,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiDownloadMemberPicturesConstMeta,
        argValues: [that, pubkeysHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiDownloadMemberPicturesConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_download_member_pictures",
        argNames: ["that", "pubkeysHex"],
      );

  @override
  Future<EncryptLocationOutcomeFfi> crateApiCircleManagerFfiEncryptLocation({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String senderPubkeyHex,
    required double latitude,
    required double longitude,
    required BigInt updateIntervalSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_String(senderPubkeyHex, serializer);
          sse_encode_f_64(latitude, serializer);
          sse_encode_f_64(longitude, serializer);
          sse_encode_u_64(updateIntervalSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 21,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_encrypt_location_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiEncryptLocationConstMeta,
        argValues: [
          that,
          mlsGroupId,
          senderPubkeyHex,
          latitude,
          longitude,
          updateIntervalSecs,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiEncryptLocationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_encrypt_location",
        argNames: [
          "that",
          "mlsGroupId",
          "senderPubkeyHex",
          "latitude",
          "longitude",
          "updateIntervalSecs",
        ],
      );

  @override
  Future<List<ProfileMetadataFfi>> crateApiCircleManagerFfiFetchMemberProfiles({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
    required PlatformInt64 maxAgeSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_String(pubkeysHex, serializer);
          sse_encode_i_64(maxAgeSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 22,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiFetchMemberProfilesConstMeta,
        argValues: [that, pubkeysHex, maxAgeSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiFetchMemberProfilesConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_fetch_member_profiles",
        argNames: ["that", "pubkeysHex", "maxAgeSecs"],
      );

  @override
  Future<ProfileMetadataFfi> crateApiCircleManagerFfiFetchMyProfile({
    required CircleManagerFfi that,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 23,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiFetchMyProfileConstMeta,
        argValues: [that, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiFetchMyProfileConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_fetch_my_profile",
        argNames: ["that", "pubkeyHex"],
      );

  @override
  Future<void> crateApiCircleManagerFfiFinalizeRelayUpdate({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_box_autoadd_pending_state_ref_ffi(pending, serializer);
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 24,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiFinalizeRelayUpdateConstMeta,
        argValues: [that, pending, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiFinalizeRelayUpdateConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_finalize_relay_update",
        argNames: ["that", "pending", "mlsGroupId"],
      );

  @override
  Future<List<ContactFfi>> crateApiCircleManagerFfiGetAllContacts({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 25,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_contact_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetAllContactsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetAllContactsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_all_contacts",
        argNames: ["that"],
      );

  @override
  Future<ProfileMetadataFfi?> crateApiCircleManagerFfiGetCachedProfile({
    required CircleManagerFfi that,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 26,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetCachedProfileConstMeta,
        argValues: [that, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetCachedProfileConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_cached_profile",
        argNames: ["that", "pubkeyHex"],
      );

  @override
  Future<List<ProfileMetadataFfi>> crateApiCircleManagerFfiGetCachedProfiles({
    required CircleManagerFfi that,
    required List<String> pubkeysHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_String(pubkeysHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 27,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetCachedProfilesConstMeta,
        argValues: [that, pubkeysHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetCachedProfilesConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_cached_profiles",
        argNames: ["that", "pubkeysHex"],
      );

  @override
  Future<CircleWithMembersFfi?> crateApiCircleManagerFfiGetCircle({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 28,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_circle_with_members_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetCircleConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetCircleConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_circle",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<List<CircleWithMembersFfi>> crateApiCircleManagerFfiGetCircles({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 29,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_circle_with_members_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetCirclesConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetCirclesConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_circles",
        argNames: ["that"],
      );

  @override
  Future<ContactFfi?> crateApiCircleManagerFfiGetContact({
    required CircleManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 30,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_contact_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetContactConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetContactConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_contact",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<List<CircleMemberFfi>> crateApiCircleManagerFfiGetMembers({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 31,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_circle_member_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetMembersConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetMembersConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_members",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<List<InvitationFfi>> crateApiCircleManagerFfiGetPendingInvitations({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 32,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_invitation_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetPendingInvitationsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetPendingInvitationsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_pending_invitations",
        argNames: ["that"],
      );

  @override
  Future<Uint8List?> crateApiCircleManagerFfiGetProfilePicture({
    required CircleManagerFfi that,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 33,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_list_prim_u_8_strict,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetProfilePictureConstMeta,
        argValues: [that, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetProfilePictureConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_profile_picture",
        argNames: ["that", "pubkeyHex"],
      );

  @override
  Future<Uint8List?> crateApiCircleManagerFfiGetProfileThumbnail({
    required CircleManagerFfi that,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 34,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_list_prim_u_8_strict,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetProfileThumbnailConstMeta,
        argValues: [that, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetProfileThumbnailConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_profile_thumbnail",
        argNames: ["that", "pubkeyHex"],
      );

  @override
  Future<bool> crateApiCircleManagerFfiGetPublishRelayList({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 35,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetPublishRelayListConstMeta,
        argValues: [that, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetPublishRelayListConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_publish_relay_list",
        argNames: ["that", "relayType"],
      );

  @override
  Future<List<CircleWithMembersFfi>> crateApiCircleManagerFfiGetVisibleCircles({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 36,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_circle_with_members_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGetVisibleCirclesConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGetVisibleCirclesConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_get_visible_circles",
        argNames: ["that"],
      );

  @override
  Future<BigInt> crateApiCircleManagerFfiGroupEpoch({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 37,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiGroupEpochConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiGroupEpochConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_group_epoch",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<List<String>> crateApiCircleManagerFfiListUserRelays({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 38,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiListUserRelaysConstMeta,
        argValues: [that, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiListUserRelaysConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_list_user_relays",
        argNames: ["that", "relayType"],
      );

  @override
  Future<CircleManagerFfi> crateApiCircleManagerFfiNew({
    required String dataDir,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(dataDir, serializer);
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 39,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiNewConstMeta,
        argValues: [dataDir, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiNewConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_new",
        argNames: ["dataDir", "identitySecretBytes"],
      );

  @override
  Future<void> crateApiCircleManagerFfiNotePeerEvent({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          sse_encode_i_64(atMs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 40,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiNotePeerEventConstMeta,
        argValues: [that, nostrGroupId, atMs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiNotePeerEventConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_note_peer_event",
        argNames: ["that", "nostrGroupId", "atMs"],
      );

  @override
  Future<void> crateApiCircleManagerFfiNotePublishAcked({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          sse_encode_i_64(atMs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 41,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiNotePublishAckedConstMeta,
        argValues: [that, nostrGroupId, atMs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiNotePublishAckedConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_note_publish_acked",
        argNames: ["that", "nostrGroupId", "atMs"],
      );

  @override
  Future<BigInt> crateApiCircleManagerFfiOutgoingLocationExpirySecs({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 42,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiOutgoingLocationExpirySecsConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiOutgoingLocationExpirySecsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_outgoing_location_expiry_secs",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<LeavePlanFfi> crateApiCircleManagerFfiPlanLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String selfPubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_String(selfPubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 43,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_leave_plan_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiPlanLeaveConstMeta,
        argValues: [that, mlsGroupId, selfPubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiPlanLeaveConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_plan_leave",
        argNames: ["that", "mlsGroupId", "selfPubkeyHex"],
      );

  @override
  Future<InvitationFfi?> crateApiCircleManagerFfiProcessGiftWrappedInvitation({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required String giftWrapEventJson,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_String(giftWrapEventJson, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 44,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_invitation_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta:
            kCrateApiCircleManagerFfiProcessGiftWrappedInvitationConstMeta,
        argValues: [that, identitySecretBytes, giftWrapEventJson],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiProcessGiftWrappedInvitationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_process_gift_wrapped_invitation",
        argNames: ["that", "identitySecretBytes", "giftWrapEventJson"],
      );

  @override
  Future<ProfilePendingStateFfi> crateApiCircleManagerFfiProfilePendingState({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 45,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_pending_state_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiProfilePendingStateConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiProfilePendingStateConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_profile_pending_state",
        argNames: ["that"],
      );

  @override
  Future<ProfilePoolStatusFfi> crateApiCircleManagerFfiProfilePoolStatus({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 46,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_pool_status_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiProfilePoolStatusConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiProfilePoolStatusConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_profile_pool_status",
        argNames: ["that"],
      );

  @override
  Future<CommitToPublishFfi> crateApiCircleManagerFfiProposeAdminHandoff({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String successorHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_String(successorHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 47,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_commit_to_publish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiProposeAdminHandoffConstMeta,
        argValues: [that, mlsGroupId, successorHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiProposeAdminHandoffConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_propose_admin_handoff",
        argNames: ["that", "mlsGroupId", "successorHex"],
      );

  @override
  Future<String> crateApiCircleManagerFfiProposeLeave({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 48,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiProposeLeaveConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiProposeLeaveConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_propose_leave",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<CommitToPublishFfi> crateApiCircleManagerFfiProposeSelfDemote({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 49,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_commit_to_publish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiProposeSelfDemoteConstMeta,
        argValues: [that, mlsGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiProposeSelfDemoteConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_propose_self_demote",
        argNames: ["that", "mlsGroupId"],
      );

  @override
  Future<int> crateApiCircleManagerFfiPruneExpiredLastKnown({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_i_64(nowUnixSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 50,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_32,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiPruneExpiredLastKnownConstMeta,
        argValues: [that, nowUnixSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiPruneExpiredLastKnownConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_prune_expired_last_known",
        argNames: ["that", "nowUnixSecs"],
      );

  @override
  Future<BigInt> crateApiCircleManagerFfiPruneProcessedGiftWraps({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_i_64(nowUnixSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 51,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiPruneProcessedGiftWrapsConstMeta,
        argValues: [that, nowUnixSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiPruneProcessedGiftWrapsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_prune_processed_gift_wraps",
        argNames: ["that", "nowUnixSecs"],
      );

  @override
  Future<void> crateApiCircleManagerFfiPublishFailed({
    required CircleManagerFfi that,
    required PendingStateRefFfi pending,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_box_autoadd_pending_state_ref_ffi(pending, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 52,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiPublishFailedConstMeta,
        argValues: [that, pending],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiPublishFailedConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_publish_failed",
        argNames: ["that", "pending"],
      );

  @override
  Future<List<DirectoryEntryFfi>>
  crateApiCircleManagerFfiRankedDirectoryMembers({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_i_64(nowUnixSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 53,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_directory_entry_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRankedDirectoryMembersConstMeta,
        argValues: [that, nowUnixSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRankedDirectoryMembersConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_ranked_directory_members",
        argNames: ["that", "nowUnixSecs"],
      );

  @override
  Future<bool> crateApiCircleManagerFfiReconcileMemberDirectory({
    required CircleManagerFfi that,
    required PlatformInt64 nowUnixSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_i_64(nowUnixSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 54,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiReconcileMemberDirectoryConstMeta,
        argValues: [that, nowUnixSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiReconcileMemberDirectoryConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_reconcile_member_directory",
        argNames: ["that", "nowUnixSecs"],
      );

  @override
  Future<void> crateApiCircleManagerFfiRecordPublishedRelayList({
    required CircleManagerFfi that,
    required String identityPubkeyHex,
    required int kind,
    required String eventIdHex,
    required PlatformInt64 publishedAtSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(identityPubkeyHex, serializer);
          sse_encode_u_16(kind, serializer);
          sse_encode_String(eventIdHex, serializer);
          sse_encode_i_64(publishedAtSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 55,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRecordPublishedRelayListConstMeta,
        argValues: [that, identityPubkeyHex, kind, eventIdHex, publishedAtSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiRecordPublishedRelayListConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_record_published_relay_list",
        argNames: [
          "that",
          "identityPubkeyHex",
          "kind",
          "eventIdHex",
          "publishedAtSecs",
        ],
      );

  @override
  Future<List<String>> crateApiCircleManagerFfiRelayPublishTargets({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 56,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRelayPublishTargetsConstMeta,
        argValues: [that, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRelayPublishTargetsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_relay_publish_targets",
        argNames: ["that", "relayType"],
      );

  @override
  Future<void> crateApiCircleManagerFfiRemoveLastKnownCircle({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 57,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRemoveLastKnownCircleConstMeta,
        argValues: [that, nostrGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRemoveLastKnownCircleConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_remove_last_known_circle",
        argNames: ["that", "nostrGroupId"],
      );

  @override
  Future<void> crateApiCircleManagerFfiRemoveLastKnownMember({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required String senderPubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          sse_encode_String(senderPubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 58,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRemoveLastKnownMemberConstMeta,
        argValues: [that, nostrGroupId, senderPubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRemoveLastKnownMemberConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_remove_last_known_member",
        argNames: ["that", "nostrGroupId", "senderPubkey"],
      );

  @override
  Future<CommitToPublishFfi> crateApiCircleManagerFfiRemoveMembers({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required List<String> memberPubkeys,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_list_String(memberPubkeys, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 59,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_commit_to_publish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRemoveMembersConstMeta,
        argValues: [that, mlsGroupId, memberPubkeys],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRemoveMembersConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_remove_members",
        argNames: ["that", "mlsGroupId", "memberPubkeys"],
      );

  @override
  Future<ProfileMetadataFfi> crateApiCircleManagerFfiRemoveMyProfilePicture({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 60,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRemoveMyProfilePictureConstMeta,
        argValues: [that, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRemoveMyProfilePictureConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_remove_my_profile_picture",
        argNames: ["that", "identitySecretBytes"],
      );

  @override
  Future<bool> crateApiCircleManagerFfiRemoveUserRelay({
    required CircleManagerFfi that,
    required String url,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(url, serializer);
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 61,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRemoveUserRelayConstMeta,
        argValues: [that, url, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRemoveUserRelayConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_remove_user_relay",
        argNames: ["that", "url", "relayType"],
      );

  @override
  Future<RepairRotationOutcomeFfi> crateApiCircleManagerFfiRepairEpochRotation({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required BigInt nowSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_u_64(nowSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 62,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_repair_rotation_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRepairEpochRotationConstMeta,
        argValues: [that, mlsGroupId, nowSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRepairEpochRotationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_repair_epoch_rotation",
        argNames: ["that", "mlsGroupId", "nowSecs"],
      );

  @override
  Future<void> crateApiCircleManagerFfiResetAllSyncCursors({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 63,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiResetAllSyncCursorsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiResetAllSyncCursorsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_reset_all_sync_cursors",
        argNames: ["that"],
      );

  @override
  Future<ProfileMetadataFfi?> crateApiCircleManagerFfiResolveStrangerProfile({
    required CircleManagerFfi that,
    required String pubkey,
    required PlatformInt64 maxAgeSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          sse_encode_i_64(maxAgeSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 64,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiResolveStrangerProfileConstMeta,
        argValues: [that, pubkey, maxAgeSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiResolveStrangerProfileConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_resolve_stranger_profile",
        argNames: ["that", "pubkey", "maxAgeSecs"],
      );

  @override
  Future<void> crateApiCircleManagerFfiRestoreDefaultProfileRelays({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 65,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta:
            kCrateApiCircleManagerFfiRestoreDefaultProfileRelaysConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiRestoreDefaultProfileRelaysConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_restore_default_profile_relays",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiCircleManagerFfiRestoreDefaultsFor({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 66,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiRestoreDefaultsForConstMeta,
        argValues: [that, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiRestoreDefaultsForConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_restore_defaults_for",
        argNames: ["that", "relayType"],
      );

  @override
  Future<ProfileMetadataFfi> crateApiCircleManagerFfiSaveMyProfileLocal({
    required CircleManagerFfi that,
    String? displayName,
    String? about,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_opt_String(displayName, serializer);
          sse_encode_opt_String(about, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 67,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_metadata_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSaveMyProfileLocalConstMeta,
        argValues: [that, displayName, about],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSaveMyProfileLocalConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_save_my_profile_local",
        argNames: ["that", "displayName", "about"],
      );

  @override
  Future<ProfilePictureRefFfi>
  crateApiCircleManagerFfiSaveMyProfilePictureLocal({
    required CircleManagerFfi that,
    required List<int> raw,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(raw, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 68,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_picture_ref_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSaveMyProfilePictureLocalConstMeta,
        argValues: [that, raw],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiSaveMyProfilePictureLocalConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_save_my_profile_picture_local",
        argNames: ["that", "raw"],
      );

  @override
  Future<bool> crateApiCircleManagerFfiSeedRelayDefaultsIfUnseeded({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 69,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta:
            kCrateApiCircleManagerFfiSeedRelayDefaultsIfUnseededConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiSeedRelayDefaultsIfUnseededConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_seed_relay_defaults_if_unseeded",
        argNames: ["that"],
      );

  @override
  Future<ContactFfi> crateApiCircleManagerFfiSetContact({
    required CircleManagerFfi that,
    required String pubkey,
    String? displayName,
    String? notes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          sse_encode_opt_String(displayName, serializer);
          sse_encode_opt_String(notes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 70,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_contact_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSetContactConstMeta,
        argValues: [that, pubkey, displayName, notes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSetContactConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_set_contact",
        argNames: ["that", "pubkey", "displayName", "notes"],
      );

  @override
  void crateApiCircleManagerFfiSetLocalNickname({
    required CircleManagerFfi that,
    required String pubkeyHex,
    String? nickname,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkeyHex, serializer);
          sse_encode_opt_String(nickname, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 71)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSetLocalNicknameConstMeta,
        argValues: [that, pubkeyHex, nickname],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSetLocalNicknameConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_set_local_nickname",
        argNames: ["that", "pubkeyHex", "nickname"],
      );

  @override
  Future<void> crateApiCircleManagerFfiSetPublishRelayList({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
    required bool value,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          sse_encode_bool(value, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 72,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSetPublishRelayListConstMeta,
        argValues: [that, relayType, value],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSetPublishRelayListConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_set_publish_relay_list",
        argNames: ["that", "relayType", "value"],
      );

  @override
  String crateApiCircleManagerFfiSignDeletionEvent({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
    required List<String> eventIds,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          sse_encode_list_String(eventIds, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 73)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSignDeletionEventConstMeta,
        argValues: [that, identitySecretBytes, eventIds],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSignDeletionEventConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_sign_deletion_event",
        argNames: ["that", "identitySecretBytes", "eventIds"],
      );

  @override
  Future<List<LastKnownLocationFfi>>
  crateApiCircleManagerFfiSnapshotLastKnownForCircle({
    required CircleManagerFfi that,
    required List<int> nostrGroupId,
    required PlatformInt64 nowUnixSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          sse_encode_i_64(nowUnixSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 74,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_last_known_location_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSnapshotLastKnownForCircleConstMeta,
        argValues: [that, nostrGroupId, nowUnixSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiSnapshotLastKnownForCircleConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_snapshot_last_known_for_circle",
        argNames: ["that", "nostrGroupId", "nowUnixSecs"],
      );

  @override
  Future<bool> crateApiCircleManagerFfiStillAMember({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required String pubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_String(pubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 75,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiStillAMemberConstMeta,
        argValues: [that, mlsGroupId, pubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiStillAMemberConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_still_a_member",
        argNames: ["that", "mlsGroupId", "pubkeyHex"],
      );

  @override
  Future<ConvergenceSweepFfi> crateApiCircleManagerFfiSweepUnresolvableInputs({
    required CircleManagerFfi that,
    required BigInt nowSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_u_64(nowSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 76,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_convergence_sweep_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSweepUnresolvableInputsConstMeta,
        argValues: [that, nowSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSweepUnresolvableInputsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_sweep_unresolvable_inputs",
        argNames: ["that", "nowSecs"],
      );

  @override
  Future<ProfileSyncResultFfi> crateApiCircleManagerFfiSyncMyProfile({
    required CircleManagerFfi that,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 77,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_profile_sync_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiSyncMyProfileConstMeta,
        argValues: [that, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiSyncMyProfileConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_sync_my_profile",
        argNames: ["that", "identitySecretBytes"],
      );

  @override
  Future<CommitToPublishFfi> crateApiCircleManagerFfiUpdateCircleRelays({
    required CircleManagerFfi that,
    required List<int> mlsGroupId,
    required List<String> newRelays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(mlsGroupId, serializer);
          sse_encode_list_String(newRelays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 78,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_commit_to_publish_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiUpdateCircleRelaysConstMeta,
        argValues: [that, mlsGroupId, newRelays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiUpdateCircleRelaysConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_update_circle_relays",
        argNames: ["that", "mlsGroupId", "newRelays"],
      );

  @override
  Future<void> crateApiCircleManagerFfiUpsertLastKnownLocation({
    required CircleManagerFfi that,
    required LastKnownLocationFfi location,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_box_autoadd_last_known_location_ffi(location, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 79,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiUpsertLastKnownLocationConstMeta,
        argValues: [that, location],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiUpsertLastKnownLocationConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_upsert_last_known_location",
        argNames: ["that", "location"],
      );

  @override
  Future<void> crateApiCircleManagerFfiWipeAllLastKnownLocations({
    required CircleManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 80,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiWipeAllLastKnownLocationsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiCircleManagerFfiWipeAllLastKnownLocationsConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_wipe_all_last_known_locations",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiCircleManagerFfiWipeAndResetDefaultsFor({
    required CircleManagerFfi that,
    required RelayTypeFfi relayType,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            that,
            serializer,
          );
          sse_encode_relay_type_ffi(relayType, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 81,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiCircleManagerFfiWipeAndResetDefaultsForConstMeta,
        argValues: [that, relayType],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleManagerFfiWipeAndResetDefaultsForConstMeta =>
      const TaskConstMeta(
        debugName: "CircleManagerFfi_wipe_and_reset_defaults_for",
        argNames: ["that", "relayType"],
      );

  @override
  Future<HavenCore> crateApiHavenCoreDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 82,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreDefaultConstMeta =>
      const TaskConstMeta(debugName: "HavenCore_default", argNames: []);

  @override
  LocationSettings crateApiHavenCoreGetLocationSettings({
    required HavenCore that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
            that,
            serializer,
          );
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 83)!;
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreGetLocationSettingsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreGetLocationSettingsConstMeta =>
      const TaskConstMeta(
        debugName: "HavenCore_get_location_settings",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiHavenCoreInitialize({required HavenCore that}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_RefMut_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 84,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiHavenCoreInitializeConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreInitializeConstMeta =>
      const TaskConstMeta(
        debugName: "HavenCore_initialize",
        argNames: ["that"],
      );

  @override
  bool crateApiHavenCoreIsInitialized({required HavenCore that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
            that,
            serializer,
          );
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 85)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreIsInitializedConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreIsInitializedConstMeta =>
      const TaskConstMeta(
        debugName: "HavenCore_is_initialized",
        argNames: ["that"],
      );

  @override
  Future<HavenCore> crateApiHavenCoreNew() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 86,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreNewConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreNewConstMeta =>
      const TaskConstMeta(debugName: "HavenCore_new", argNames: []);

  @override
  void crateApiHavenCoreSetLocationSettings({
    required HavenCore that,
    required LocationSettings settings,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_RefMut_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
            that,
            serializer,
          );
          sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
            settings,
            serializer,
          );
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 87)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreSetLocationSettingsConstMeta,
        argValues: [that, settings],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreSetLocationSettingsConstMeta =>
      const TaskConstMeta(
        debugName: "HavenCore_set_location_settings",
        argNames: ["that", "settings"],
      );

  @override
  LocationMessage crateApiHavenCoreUpdateLocation({
    required HavenCore that,
    required double latitude,
    required double longitude,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
            that,
            serializer,
          );
          sse_encode_f_64(latitude, serializer);
          sse_encode_f_64(longitude, serializer);
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 88)!;
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiHavenCoreUpdateLocationConstMeta,
        argValues: [that, latitude, longitude],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiHavenCoreUpdateLocationConstMeta =>
      const TaskConstMeta(
        debugName: "HavenCore_update_location",
        argNames: ["that", "latitude", "longitude"],
      );

  @override
  Future<int> crateApiLiveSyncFfiInFlightPublishes({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 89,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_32,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiInFlightPublishesConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiInFlightPublishesConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_in_flight_publishes",
        argNames: ["that"],
      );

  @override
  bool crateApiLiveSyncFfiIsPaused({required LiveSyncFfi that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 90)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLiveSyncFfiIsPausedConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiIsPausedConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_is_paused",
        argNames: ["that"],
      );

  @override
  bool crateApiLiveSyncFfiIsRunning({required LiveSyncFfi that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          return pdeCallFfi(generalizedFrbRustBinding, serializer, funcId: 91)!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLiveSyncFfiIsRunningConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiIsRunningConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_is_running",
        argNames: ["that"],
      );

  @override
  Stream<FfiRelayEvent> crateApiLiveSyncFfiLiveEvents({
    required LiveSyncFfi that,
  }) {
    final sink = RustStreamSink<FfiRelayEvent>();
    unawaited(
      handler.executeNormal(
        NormalTask(
          callFfi: (port_) {
            final serializer = SseSerializer(generalizedFrbRustBinding);
            sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
              that,
              serializer,
            );
            sse_encode_StreamSink_ffi_relay_event_Sse(sink, serializer);
            pdeCallFfi(
              generalizedFrbRustBinding,
              serializer,
              funcId: 92,
              port: port_,
            );
          },
          codec: SseCodec(
            decodeSuccessData: sse_decode_unit,
            decodeErrorData: sse_decode_String,
          ),
          constMeta: kCrateApiLiveSyncFfiLiveEventsConstMeta,
          argValues: [that, sink],
          apiImpl: this,
        ),
      ),
    );
    return sink.stream;
  }

  TaskConstMeta get kCrateApiLiveSyncFfiLiveEventsConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_live_events",
        argNames: ["that", "sink"],
      );

  @override
  Future<LiveSyncFfi> crateApiLiveSyncFfiNewInstance({
    required CircleManagerFfi circle,
    required String ownPubkeyHex,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            circle,
            serializer,
          );
          sse_encode_String(ownPubkeyHex, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 93,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiNewInstanceConstMeta,
        argValues: [circle, ownPubkeyHex],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiNewInstanceConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_new_instance",
        argNames: ["circle", "ownPubkeyHex"],
      );

  @override
  Future<void> crateApiLiveSyncFfiOpenBackgroundBurst({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 94,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiOpenBackgroundBurstConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiOpenBackgroundBurstConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_open_background_burst",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiLiveSyncFfiPauseSubscriptions({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 95,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiPauseSubscriptionsConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiPauseSubscriptionsConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_pause_subscriptions",
        argNames: ["that"],
      );

  @override
  Future<int> crateApiLiveSyncFfiPoolSubscriptionCount({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 96,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_32,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiPoolSubscriptionCountConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiPoolSubscriptionCountConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_pool_subscription_count",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiLiveSyncFfiResumeAfterBackground({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 97,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiResumeAfterBackgroundConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiResumeAfterBackgroundConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_resume_after_background",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiLiveSyncFfiSettleBeforePause({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 98,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiSettleBeforePauseConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiSettleBeforePauseConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_settle_before_pause",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiLiveSyncFfiStartSession({
    required LiveSyncFfi that,
    required List<FfiGroupSpec> groups,
    required List<String> inboxRelays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          sse_encode_list_ffi_group_spec(groups, serializer);
          sse_encode_list_String(inboxRelays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 99,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiStartSessionConstMeta,
        argValues: [that, groups, inboxRelays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiStartSessionConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_start_session",
        argNames: ["that", "groups", "inboxRelays"],
      );

  @override
  Future<void> crateApiLiveSyncFfiStopSession({required LiveSyncFfi that}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 100,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiStopSessionConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiStopSessionConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_stop_session",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiLiveSyncFfiSubscribeCircle({
    required LiveSyncFfi that,
    required FfiGroupSpec spec,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          sse_encode_box_autoadd_ffi_group_spec(spec, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 101,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiSubscribeCircleConstMeta,
        argValues: [that, spec],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiSubscribeCircleConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_subscribe_circle",
        argNames: ["that", "spec"],
      );

  @override
  Future<void> crateApiLiveSyncFfiUnsubscribeCircle({
    required LiveSyncFfi that,
    required List<int> nostrGroupId,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 102,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiUnsubscribeCircleConstMeta,
        argValues: [that, nostrGroupId],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiUnsubscribeCircleConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_unsubscribe_circle",
        argNames: ["that", "nostrGroupId"],
      );

  @override
  Future<BacklogOutcomeFfi> crateApiLiveSyncFfiWaitBacklogSettled({
    required LiveSyncFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 103,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_backlog_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLiveSyncFfiWaitBacklogSettledConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLiveSyncFfiWaitBacklogSettledConstMeta =>
      const TaskConstMeta(
        debugName: "LiveSyncFfi_wait_backlog_settled",
        argNames: ["that"],
      );

  @override
  Future<LocationEventService> crateApiLocationEventServiceDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 104,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationEventServiceDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationEventServiceDefaultConstMeta =>
      const TaskConstMeta(
        debugName: "LocationEventService_default",
        argNames: [],
      );

  @override
  BigInt crateApiLocationEventServiceJitteredPublishIntervalSecs({
    required LocationEventService that,
    required BigInt nominalSecs,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
            that,
            serializer,
          );
          sse_encode_u_64(nominalSecs, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 105,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: null,
        ),
        constMeta:
            kCrateApiLocationEventServiceJitteredPublishIntervalSecsConstMeta,
        argValues: [that, nominalSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiLocationEventServiceJitteredPublishIntervalSecsConstMeta =>
      const TaskConstMeta(
        debugName: "LocationEventService_jittered_publish_interval_secs",
        argNames: ["that", "nominalSecs"],
      );

  @override
  LocationEventService crateApiLocationEventServiceNew() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 106,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationEventServiceNewConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationEventServiceNewConstMeta =>
      const TaskConstMeta(debugName: "LocationEventService_new", argNames: []);

  @override
  bool crateApiLocationEventServiceVerifySignature({
    required LocationEventService that,
    required SignedLocationEventFfi event,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
            that,
            serializer,
          );
          sse_encode_box_autoadd_signed_location_event_ffi(event, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 107,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiLocationEventServiceVerifySignatureConstMeta,
        argValues: [that, event],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationEventServiceVerifySignatureConstMeta =>
      const TaskConstMeta(
        debugName: "LocationEventService_verify_signature",
        argNames: ["that", "event"],
      );

  @override
  PlatformInt64 crateApiLocationMessageExpiresAt({
    required LocationMessage that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 108,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_i_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageExpiresAtConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageExpiresAtConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_expires_at",
        argNames: ["that"],
      );

  @override
  String crateApiLocationMessageGeohash({required LocationMessage that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 109,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageGeohashConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageGeohashConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_geohash",
        argNames: ["that"],
      );

  @override
  bool crateApiLocationMessageIsExpired({required LocationMessage that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 110,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageIsExpiredConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageIsExpiredConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_is_expired",
        argNames: ["that"],
      );

  @override
  double crateApiLocationMessageLatitude({required LocationMessage that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 111,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_f_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageLatitudeConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageLatitudeConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_latitude",
        argNames: ["that"],
      );

  @override
  double crateApiLocationMessageLongitude({required LocationMessage that}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 112,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_f_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageLongitudeConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageLongitudeConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_longitude",
        argNames: ["that"],
      );

  @override
  PlatformInt64 crateApiLocationMessageTimestamp({
    required LocationMessage that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 113,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_i_64,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationMessageTimestampConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationMessageTimestampConstMeta =>
      const TaskConstMeta(
        debugName: "LocationMessage_timestamp",
        argNames: ["that"],
      );

  @override
  Future<LocationSettings> crateApiLocationSettingsNew({
    required int updateIntervalMinutes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_u_32(updateIntervalMinutes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 114,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationSettingsNewConstMeta,
        argValues: [updateIntervalMinutes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationSettingsNewConstMeta =>
      const TaskConstMeta(
        debugName: "LocationSettings_new",
        argNames: ["updateIntervalMinutes"],
      );

  @override
  int crateApiLocationSettingsUpdateIntervalMinutes({
    required LocationSettings that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 115,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_32,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLocationSettingsUpdateIntervalMinutesConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLocationSettingsUpdateIntervalMinutesConstMeta =>
      const TaskConstMeta(
        debugName: "LocationSettings_update_interval_minutes",
        argNames: ["that"],
      );

  @override
  Future<void> crateApiNostrIdentityManagerClearCache({
    required NostrIdentityManager that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 116,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiNostrIdentityManagerClearCacheConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerClearCacheConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_clear_cache",
        argNames: ["that"],
      );

  @override
  Future<PublicIdentity> crateApiNostrIdentityManagerCreateIdentity({
    required NostrIdentityManager that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 117,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_public_identity,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerCreateIdentityConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerCreateIdentityConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_create_identity",
        argNames: ["that"],
      );

  @override
  Future<NostrIdentityManager> crateApiNostrIdentityManagerDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 118,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiNostrIdentityManagerDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerDefaultConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_default",
        argNames: [],
      );

  @override
  Future<void> crateApiNostrIdentityManagerDeleteIdentity({
    required NostrIdentityManager that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 119,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerDeleteIdentityConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerDeleteIdentityConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_delete_identity",
        argNames: ["that"],
      );

  @override
  Future<String> crateApiNostrIdentityManagerExportNsec({
    required NostrIdentityManager that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 120,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerExportNsecConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerExportNsecConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_export_nsec",
        argNames: ["that"],
      );

  @override
  PublicIdentity? crateApiNostrIdentityManagerGetIdentity({
    required NostrIdentityManager that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 121,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_public_identity,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerGetIdentityConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerGetIdentityConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_get_identity",
        argNames: ["that"],
      );

  @override
  Future<Uint8List> crateApiNostrIdentityManagerGetSecretBytes({
    required NostrIdentityManager that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 122,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_prim_u_8_strict,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerGetSecretBytesConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerGetSecretBytesConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_get_secret_bytes",
        argNames: ["that"],
      );

  @override
  bool crateApiNostrIdentityManagerHasIdentity({
    required NostrIdentityManager that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 123,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiNostrIdentityManagerHasIdentityConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerHasIdentityConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_has_identity",
        argNames: ["that"],
      );

  @override
  Future<PublicIdentity> crateApiNostrIdentityManagerImportFromNsec({
    required NostrIdentityManager that,
    required String nsec,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          sse_encode_String(nsec, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 124,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_public_identity,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerImportFromNsecConstMeta,
        argValues: [that, nsec],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerImportFromNsecConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_import_from_nsec",
        argNames: ["that", "nsec"],
      );

  @override
  Future<PublicIdentity> crateApiNostrIdentityManagerLoadFromBytes({
    required NostrIdentityManager that,
    required List<int> secretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(secretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 125,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_public_identity,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerLoadFromBytesConstMeta,
        argValues: [that, secretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerLoadFromBytesConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_load_from_bytes",
        argNames: ["that", "secretBytes"],
      );

  @override
  Future<NostrIdentityManager> crateApiNostrIdentityManagerNew() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 126,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiNostrIdentityManagerNewConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerNewConstMeta =>
      const TaskConstMeta(debugName: "NostrIdentityManager_new", argNames: []);

  @override
  String crateApiNostrIdentityManagerPubkeyHex({
    required NostrIdentityManager that,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 127,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerPubkeyHexConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerPubkeyHexConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_pubkey_hex",
        argNames: ["that"],
      );

  @override
  Future<String> crateApiNostrIdentityManagerSign({
    required NostrIdentityManager that,
    required List<int> messageHash,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(messageHash, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 128,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiNostrIdentityManagerSignConstMeta,
        argValues: [that, messageHash],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiNostrIdentityManagerSignConstMeta =>
      const TaskConstMeta(
        debugName: "NostrIdentityManager_sign",
        argNames: ["that", "messageHash"],
      );

  @override
  Future<RelayEventCheckFfi> crateApiRelayManagerFfiCheckEventOnRelay({
    required RelayManagerFfi that,
    required String relayUrl,
    required String authorPubkey,
    required int eventKind,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(relayUrl, serializer);
          sse_encode_String(authorPubkey, serializer);
          sse_encode_u_16(eventKind, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 129,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_relay_event_check_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiCheckEventOnRelayConstMeta,
        argValues: [that, relayUrl, authorPubkey, eventKind],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiCheckEventOnRelayConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_check_event_on_relay",
        argNames: ["that", "relayUrl", "authorPubkey", "eventKind"],
      );

  @override
  Future<void> crateApiRelayManagerFfiDisconnectRelay({
    required RelayManagerFfi that,
    required String url,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(url, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 130,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiDisconnectRelayConstMeta,
        argValues: [that, url],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiDisconnectRelayConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_disconnect_relay",
        argNames: ["that", "url"],
      );

  @override
  Future<List<String>> crateApiRelayManagerFfiFetchGiftWraps({
    required RelayManagerFfi that,
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(recipientPubkey, serializer);
          sse_encode_list_String(relays, serializer);
          sse_encode_opt_box_autoadd_i_64(since, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 131,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchGiftWrapsConstMeta,
        argValues: [that, recipientPubkey, relays, since],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchGiftWrapsConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_gift_wraps",
        argNames: ["that", "recipientPubkey", "relays", "since"],
      );

  @override
  Future<List<RelayGiftWrapFetchFfi>>
  crateApiRelayManagerFfiFetchGiftWrapsPerRelay({
    required RelayManagerFfi that,
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(recipientPubkey, serializer);
          sse_encode_list_String(relays, serializer);
          sse_encode_opt_box_autoadd_i_64(since, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 132,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_relay_gift_wrap_fetch_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchGiftWrapsPerRelayConstMeta,
        argValues: [that, recipientPubkey, relays, since],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchGiftWrapsPerRelayConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_gift_wraps_per_relay",
        argNames: ["that", "recipientPubkey", "relays", "since"],
      );

  @override
  Future<List<String>> crateApiRelayManagerFfiFetchGroupMessages({
    required RelayManagerFfi that,
    required List<int> nostrGroupId,
    required List<String> relays,
    PlatformInt64? since,
    int? limit,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(nostrGroupId, serializer);
          sse_encode_list_String(relays, serializer);
          sse_encode_opt_box_autoadd_i_64(since, serializer);
          sse_encode_opt_box_autoadd_u_32(limit, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 133,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchGroupMessagesConstMeta,
        argValues: [that, nostrGroupId, relays, since, limit],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchGroupMessagesConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_group_messages",
        argNames: ["that", "nostrGroupId", "relays", "since", "limit"],
      );

  @override
  Future<String?> crateApiRelayManagerFfiFetchKeypackage({
    required RelayManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 134,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchKeypackageConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchKeypackageConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_keypackage",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<List<String>> crateApiRelayManagerFfiFetchKeypackageRelays({
    required RelayManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 135,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchKeypackageRelaysConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchKeypackageRelaysConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_keypackage_relays",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<MemberKeyPackageFfi?> crateApiRelayManagerFfiFetchMemberKeypackage({
    required RelayManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 136,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_member_key_package_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchMemberKeypackageConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchMemberKeypackageConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_member_keypackage",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<List<String>> crateApiRelayManagerFfiFetchNip65Relays({
    required RelayManagerFfi that,
    required String pubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(pubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 137,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiFetchNip65RelaysConstMeta,
        argValues: [that, pubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiFetchNip65RelaysConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_fetch_nip65_relays",
        argNames: ["that", "pubkey"],
      );

  @override
  Future<List<RelayConnectionStatusFfi>> crateApiRelayManagerFfiGetRelayStatus({
    required RelayManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 138,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_relay_connection_status_ffi,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiRelayManagerFfiGetRelayStatusConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiGetRelayStatusConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_get_relay_status",
        argNames: ["that"],
      );

  @override
  Future<KpMaintenanceOutcomeFfi> crateApiRelayManagerFfiMaintainKeyPackage({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            circle,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 139,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_kp_maintenance_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiMaintainKeyPackageConstMeta,
        argValues: [that, circle, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiMaintainKeyPackageConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_maintain_key_package",
        argNames: ["that", "circle", "identitySecretBytes"],
      );

  @override
  Future<RelayListMaintenanceOutcomeFfi>
  crateApiRelayManagerFfiMaintainRelayList({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            circle,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 140,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_relay_list_maintenance_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiMaintainRelayListConstMeta,
        argValues: [that, circle, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiMaintainRelayListConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_maintain_relay_list",
        argNames: ["that", "circle", "identitySecretBytes"],
      );

  @override
  Future<RelayManagerFfi> crateApiRelayManagerFfiNewInstance() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 141,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData:
              sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiNewInstanceConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiNewInstanceConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_new_instance",
        argNames: [],
      );

  @override
  Future<PublishResultFfi> crateApiRelayManagerFfiPublishEvent({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(eventJson, serializer);
          sse_encode_list_String(relays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 142,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_publish_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiPublishEventConstMeta,
        argValues: [that, eventJson, relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiPublishEventConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_publish_event",
        argNames: ["that", "eventJson", "relays"],
      );

  @override
  Future<void> crateApiRelayManagerFfiPublishEventFireAndForget({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(eventJson, serializer);
          sse_encode_list_String(relays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 143,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiPublishEventFireAndForgetConstMeta,
        argValues: [that, eventJson, relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta
  get kCrateApiRelayManagerFfiPublishEventFireAndForgetConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_publish_event_fire_and_forget",
        argNames: ["that", "eventJson", "relays"],
      );

  @override
  Future<PublishResultFfi> crateApiRelayManagerFfiPublishLocationEvent({
    required RelayManagerFfi that,
    required String eventJson,
    required List<String> relays,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_String(eventJson, serializer);
          sse_encode_list_String(relays, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 144,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_publish_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiPublishLocationEventConstMeta,
        argValues: [that, eventJson, relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiPublishLocationEventConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_publish_location_event",
        argNames: ["that", "eventJson", "relays"],
      );

  @override
  Future<LegacyRetractionOutcomeFfi>
  crateApiRelayManagerFfiRetractLegacyKeyMaterial({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            circle,
            serializer,
          );
          sse_encode_list_prim_u_8_loose(identitySecretBytes, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 145,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_legacy_retraction_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiRetractLegacyKeyMaterialConstMeta,
        argValues: [that, circle, identitySecretBytes],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiRetractLegacyKeyMaterialConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_retract_legacy_key_material",
        argNames: ["that", "circle", "identitySecretBytes"],
      );

  @override
  Future<CatchupResultFfi> crateApiRelayManagerFfiRunCatchupAllCircles({
    required RelayManagerFfi that,
    required CircleManagerFfi circle,
    required String ownPubkeyHex,
    required BigInt maxDurationSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
            circle,
            serializer,
          );
          sse_encode_String(ownPubkeyHex, serializer);
          sse_encode_u_64(maxDurationSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 146,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_catchup_result_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiRelayManagerFfiRunCatchupAllCirclesConstMeta,
        argValues: [that, circle, ownPubkeyHex, maxDurationSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiRunCatchupAllCirclesConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_run_catchup_all_circles",
        argNames: ["that", "circle", "ownPubkeyHex", "maxDurationSecs"],
      );

  @override
  Future<void> crateApiRelayManagerFfiShutdown({
    required RelayManagerFfi that,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
            that,
            serializer,
          );
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 147,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiRelayManagerFfiShutdownConstMeta,
        argValues: [that],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRelayManagerFfiShutdownConstMeta =>
      const TaskConstMeta(
        debugName: "RelayManagerFfi_shutdown",
        argNames: ["that"],
      );

  @override
  void crateApiAllowPrivateBlossomForTest() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 148,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiAllowPrivateBlossomForTestConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiAllowPrivateBlossomForTestConstMeta =>
      const TaskConstMeta(
        debugName: "allow_private_blossom_for_test",
        argNames: [],
      );

  @override
  void crateApiAllowWsLoopbackForTest() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 149,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiAllowWsLoopbackForTestConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiAllowWsLoopbackForTestConstMeta =>
      const TaskConstMeta(
        debugName: "allow_ws_loopback_for_test",
        argNames: [],
      );

  @override
  Future<CircleHealthFfi> crateApiCircleHealthFfiDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 150,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_circle_health_ffi,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiCircleHealthFfiDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiCircleHealthFfiDefaultConstMeta =>
      const TaskConstMeta(debugName: "circle_health_ffi_default", argNames: []);

  @override
  Future<ConvergenceSweepFfi> crateApiConvergenceSweepFfiDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 151,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_convergence_sweep_ffi,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiConvergenceSweepFfiDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiConvergenceSweepFfiDefaultConstMeta =>
      const TaskConstMeta(
        debugName: "convergence_sweep_ffi_default",
        argNames: [],
      );

  @override
  List<String> crateApiDefaultRelays() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 152,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiDefaultRelaysConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiDefaultRelaysConstMeta =>
      const TaskConstMeta(debugName: "default_relays", argNames: []);

  @override
  Future<void> crateApiDestroyLegacyMlsState({required String dataDir}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(dataDir, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 153,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiDestroyLegacyMlsStateConstMeta,
        argValues: [dataDir],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiDestroyLegacyMlsStateConstMeta =>
      const TaskConstMeta(
        debugName: "destroy_legacy_mls_state",
        argNames: ["dataDir"],
      );

  @override
  List<String> crateApiDiscoveryRelays() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 154,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiDiscoveryRelaysConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiDiscoveryRelaysConstMeta =>
      const TaskConstMeta(debugName: "discovery_relays", argNames: []);

  @override
  String crateApiFoldForSearch({required String query}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(query, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 155,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiFoldForSearchConstMeta,
        argValues: [query],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiFoldForSearchConstMeta =>
      const TaskConstMeta(debugName: "fold_for_search", argNames: ["query"]);

  @override
  Future<ForceReleaseOutcomeFfi> crateApiForceReleaseLiveSession() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 156,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_force_release_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiForceReleaseLiveSessionConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiForceReleaseLiveSessionConstMeta =>
      const TaskConstMeta(
        debugName: "force_release_live_session",
        argNames: [],
      );

  @override
  Future<void> crateApiInitApp() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 157,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiInitAppConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiInitAppConstMeta =>
      const TaskConstMeta(debugName: "init_app", argNames: []);

  @override
  Future<void> crateApiInitKeyringStore() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 158,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiInitKeyringStoreConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiInitKeyringStoreConstMeta =>
      const TaskConstMeta(debugName: "init_keyring_store", argNames: []);

  @override
  Future<bool> crateApiIsSessionLive({required String dataDir}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(dataDir, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 159,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_bool,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiIsSessionLiveConstMeta,
        argValues: [dataDir],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiIsSessionLiveConstMeta =>
      const TaskConstMeta(debugName: "is_session_live", argNames: ["dataDir"]);

  @override
  Future<LegacyRetractionOutcomeFfi>
  crateApiLegacyRetractionOutcomeFfiDefault() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 160,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_legacy_retraction_outcome_ffi,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLegacyRetractionOutcomeFfiDefaultConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLegacyRetractionOutcomeFfiDefaultConstMeta =>
      const TaskConstMeta(
        debugName: "legacy_retraction_outcome_ffi_default",
        argNames: [],
      );

  @override
  String crateApiLogAlias({
    required LogAliasClassFfi class_,
    required String value,
  }) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_log_alias_class_ffi(class_, serializer);
          sse_encode_String(value, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 161,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_String,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiLogAliasConstMeta,
        argValues: [class_, value],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiLogAliasConstMeta => const TaskConstMeta(
    debugName: "log_alias",
    argNames: ["class_", "value"],
  );

  @override
  Future<SubscriptionHealthOutcomeFfi> crateApiMaintainSubscriptionHealth() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 162,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_subscription_health_outcome_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiMaintainSubscriptionHealthConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiMaintainSubscriptionHealthConstMeta =>
      const TaskConstMeta(
        debugName: "maintain_subscription_health",
        argNames: [],
      );

  @override
  Future<DecryptedLocationFfi> crateApiParseEngineLocation({
    required String contentJson,
    required String senderPubkey,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(contentJson, serializer);
          sse_encode_String(senderPubkey, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 163,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_decrypted_location_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiParseEngineLocationConstMeta,
        argValues: [contentJson, senderPubkey],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiParseEngineLocationConstMeta =>
      const TaskConstMeta(
        debugName: "parse_engine_location",
        argNames: ["contentJson", "senderPubkey"],
      );

  @override
  void crateApiRotateLogAliasSalt() {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 164,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiRotateLogAliasSaltConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiRotateLogAliasSaltConstMeta =>
      const TaskConstMeta(debugName: "rotate_log_alias_salt", argNames: []);

  @override
  void crateApiSetBlossomServerForTest({required String url}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(url, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 165,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiSetBlossomServerForTestConstMeta,
        argValues: [url],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSetBlossomServerForTestConstMeta =>
      const TaskConstMeta(
        debugName: "set_blossom_server_for_test",
        argNames: ["url"],
      );

  @override
  void crateApiSetDefaultRelaysForTest({required List<String> relays}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_String(relays, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 166,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiSetDefaultRelaysForTestConstMeta,
        argValues: [relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSetDefaultRelaysForTestConstMeta =>
      const TaskConstMeta(
        debugName: "set_default_relays_for_test",
        argNames: ["relays"],
      );

  @override
  void crateApiSetDiscoveryRelaysForTest({required List<String> relays}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_String(relays, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 167,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiSetDiscoveryRelaysForTestConstMeta,
        argValues: [relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSetDiscoveryRelaysForTestConstMeta =>
      const TaskConstMeta(
        debugName: "set_discovery_relays_for_test",
        argNames: ["relays"],
      );

  @override
  void crateApiSetProfileRelaysForTest({required List<String> relays}) {
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_String(relays, serializer);
          return pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 168,
          )!;
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiSetProfileRelaysForTestConstMeta,
        argValues: [relays],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiSetProfileRelaysForTestConstMeta =>
      const TaskConstMeta(
        debugName: "set_profile_relays_for_test",
        argNames: ["relays"],
      );

  @override
  Future<BigInt> crateApiTileCacheEvict({
    required PlatformInt64 maxBytes,
    required PlatformInt64 idleAgeSecs,
    required PlatformInt64 maxRetentionSecs,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_i_64(maxBytes, serializer);
          sse_encode_i_64(idleAgeSecs, serializer);
          sse_encode_i_64(maxRetentionSecs, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 169,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_u_64,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCacheEvictConstMeta,
        argValues: [maxBytes, idleAgeSecs, maxRetentionSecs],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCacheEvictConstMeta => const TaskConstMeta(
    debugName: "tile_cache_evict",
    argNames: ["maxBytes", "idleAgeSecs", "maxRetentionSecs"],
  );

  @override
  Future<TileCacheEntryFfi?> crateApiTileCacheGet({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(style, serializer);
          sse_encode_i_64(z, serializer);
          sse_encode_i_64(x, serializer);
          sse_encode_i_64(y, serializer);
          sse_encode_bool(retina, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 170,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_opt_box_autoadd_tile_cache_entry_ffi,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCacheGetConstMeta,
        argValues: [style, z, x, y, retina],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCacheGetConstMeta => const TaskConstMeta(
    debugName: "tile_cache_get",
    argNames: ["style", "z", "x", "y", "retina"],
  );

  @override
  Future<void> crateApiTileCacheInit({required String dataDir}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(dataDir, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 171,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCacheInitConstMeta,
        argValues: [dataDir],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCacheInitConstMeta =>
      const TaskConstMeta(debugName: "tile_cache_init", argNames: ["dataDir"]);

  @override
  Future<void> crateApiTileCachePut({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
    required List<int> bytes,
    required PlatformInt64 staleAtMs,
    PlatformInt64? lastModifiedMs,
    String? etag,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(style, serializer);
          sse_encode_i_64(z, serializer);
          sse_encode_i_64(x, serializer);
          sse_encode_i_64(y, serializer);
          sse_encode_bool(retina, serializer);
          sse_encode_list_prim_u_8_loose(bytes, serializer);
          sse_encode_i_64(staleAtMs, serializer);
          sse_encode_opt_box_autoadd_i_64(lastModifiedMs, serializer);
          sse_encode_opt_String(etag, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 172,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCachePutConstMeta,
        argValues: [
          style,
          z,
          x,
          y,
          retina,
          bytes,
          staleAtMs,
          lastModifiedMs,
          etag,
        ],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCachePutConstMeta => const TaskConstMeta(
    debugName: "tile_cache_put",
    argNames: [
      "style",
      "z",
      "x",
      "y",
      "retina",
      "bytes",
      "staleAtMs",
      "lastModifiedMs",
      "etag",
    ],
  );

  @override
  Future<void> crateApiTileCachePutMetadata({
    required String style,
    required PlatformInt64 z,
    required PlatformInt64 x,
    required PlatformInt64 y,
    required bool retina,
    required PlatformInt64 staleAtMs,
    PlatformInt64? lastModifiedMs,
    String? etag,
  }) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(style, serializer);
          sse_encode_i_64(z, serializer);
          sse_encode_i_64(x, serializer);
          sse_encode_i_64(y, serializer);
          sse_encode_bool(retina, serializer);
          sse_encode_i_64(staleAtMs, serializer);
          sse_encode_opt_box_autoadd_i_64(lastModifiedMs, serializer);
          sse_encode_opt_String(etag, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 173,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCachePutMetadataConstMeta,
        argValues: [style, z, x, y, retina, staleAtMs, lastModifiedMs, etag],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCachePutMetadataConstMeta =>
      const TaskConstMeta(
        debugName: "tile_cache_put_metadata",
        argNames: [
          "style",
          "z",
          "x",
          "y",
          "retina",
          "staleAtMs",
          "lastModifiedMs",
          "etag",
        ],
      );

  @override
  Future<void> crateApiTileCacheWipe() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 174,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiTileCacheWipeConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiTileCacheWipeConstMeta =>
      const TaskConstMeta(debugName: "tile_cache_wipe", argNames: []);

  @override
  Future<void> crateApiUseInMemoryKeyringForTest() {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 175,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiUseInMemoryKeyringForTestConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiUseInMemoryKeyringForTestConstMeta =>
      const TaskConstMeta(
        debugName: "use_in_memory_keyring_for_test",
        argNames: [],
      );

  @override
  Future<void> crateApiWipeAllMlsState({required String dataDir}) {
    return handler.executeNormal(
      NormalTask(
        callFfi: (port_) {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_String(dataDir, serializer);
          pdeCallFfi(
            generalizedFrbRustBinding,
            serializer,
            funcId: 176,
            port: port_,
          );
        },
        codec: SseCodec(
          decodeSuccessData: sse_decode_unit,
          decodeErrorData: sse_decode_String,
        ),
        constMeta: kCrateApiWipeAllMlsStateConstMeta,
        argValues: [dataDir],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiWipeAllMlsStateConstMeta => const TaskConstMeta(
    debugName: "wipe_all_mls_state",
    argNames: ["dataDir"],
  );

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_CircleManagerFfi => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_CircleManagerFfi => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_HavenCore => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_HavenCore => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LiveSyncFfi => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LiveSyncFfi => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationEventService => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationEventService => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationMessage => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationMessage => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_LocationSettings => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_LocationSettings => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_NostrIdentityManager => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_NostrIdentityManager => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager;

  RustArcIncrementStrongCountFnType
  get rust_arc_increment_strong_count_RelayManagerFfi => wire
      .rust_arc_increment_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi;

  RustArcDecrementStrongCountFnType
  get rust_arc_decrement_strong_count_RelayManagerFfi => wire
      .rust_arc_decrement_strong_count_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi;

  @protected
  AnyhowException dco_decode_AnyhowException(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return AnyhowException(raw as String);
  }

  @protected
  CircleManagerFfi
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  HavenCore
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return HavenCoreImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LiveSyncFfi
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationEventService
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationMessage
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationMessageImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationSettings
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  NostrIdentityManager
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  RelayManagerFfi
  dco_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  HavenCore
  dco_decode_Auto_RefMut_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return HavenCoreImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  CircleManagerFfi
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  HavenCore
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return HavenCoreImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LiveSyncFfi
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationEventService
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationMessage
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationMessageImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationSettings
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  NostrIdentityManager
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  RelayManagerFfi
  dco_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  CircleManagerFfi
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  HavenCore
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return HavenCoreImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LiveSyncFfi
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationEventService
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationMessage
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationMessageImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  LocationSettings
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  NostrIdentityManager
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  RelayManagerFfi
  dco_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalDcoDecode(raw as List<dynamic>);
  }

  @protected
  RustStreamSink<FfiRelayEvent> dco_decode_StreamSink_ffi_relay_event_Sse(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    throw UnimplementedError();
  }

  @protected
  String dco_decode_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as String;
  }

  @protected
  AddMembersResultFfi dco_decode_add_members_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return AddMembersResultFfi(
      commitEventJson: dco_decode_String(arr[0]),
      welcomeEvents: dco_decode_list_gift_wrapped_welcome_ffi(arr[1]),
      pending: dco_decode_pending_state_ref_ffi(arr[2]),
    );
  }

  @protected
  BacklogOutcomeFfi dco_decode_backlog_outcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return BacklogOutcomeFfi.values[raw as int];
  }

  @protected
  bool dco_decode_bool(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as bool;
  }

  @protected
  CircleWithMembersFfi dco_decode_box_autoadd_circle_with_members_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_circle_with_members_ffi(raw);
  }

  @protected
  CommitToPublishFfi dco_decode_box_autoadd_commit_to_publish_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_commit_to_publish_ffi(raw);
  }

  @protected
  ContactFfi dco_decode_box_autoadd_contact_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_contact_ffi(raw);
  }

  @protected
  DecryptedLocationFfi dco_decode_box_autoadd_decrypted_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_decrypted_location_ffi(raw);
  }

  @protected
  DeferredSendFfi dco_decode_box_autoadd_deferred_send_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_deferred_send_ffi(raw);
  }

  @protected
  EncryptedLocationFfi dco_decode_box_autoadd_encrypted_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_encrypted_location_ffi(raw);
  }

  @protected
  FfiGroupSpec dco_decode_box_autoadd_ffi_group_spec(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_ffi_group_spec(raw);
  }

  @protected
  FfiSyncStatusReason dco_decode_box_autoadd_ffi_sync_status_reason(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_ffi_sync_status_reason(raw);
  }

  @protected
  PlatformInt64 dco_decode_box_autoadd_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_i_64(raw);
  }

  @protected
  InvitationFfi dco_decode_box_autoadd_invitation_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_invitation_ffi(raw);
  }

  @protected
  LastKnownLocationFfi dco_decode_box_autoadd_last_known_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_last_known_location_ffi(raw);
  }

  @protected
  MemberKeyPackageFfi dco_decode_box_autoadd_member_key_package_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_member_key_package_ffi(raw);
  }

  @protected
  PendingStateRefFfi dco_decode_box_autoadd_pending_state_ref_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_pending_state_ref_ffi(raw);
  }

  @protected
  ProfileMetadataFfi dco_decode_box_autoadd_profile_metadata_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_profile_metadata_ffi(raw);
  }

  @protected
  PublicIdentity dco_decode_box_autoadd_public_identity(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_public_identity(raw);
  }

  @protected
  SignedLocationEventFfi dco_decode_box_autoadd_signed_location_event_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_signed_location_event_ffi(raw);
  }

  @protected
  SkipReasonFfi dco_decode_box_autoadd_skip_reason_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_skip_reason_ffi(raw);
  }

  @protected
  TileCacheEntryFfi dco_decode_box_autoadd_tile_cache_entry_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dco_decode_tile_cache_entry_ffi(raw);
  }

  @protected
  int dco_decode_box_autoadd_u_16(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  int dco_decode_box_autoadd_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  BuiltRelayListEventFfi dco_decode_built_relay_list_event_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return BuiltRelayListEventFfi(
      eventJson: dco_decode_opt_String(arr[0]),
      eventIdHex: dco_decode_opt_String(arr[1]),
      targets: dco_decode_list_String(arr[2]),
      kind: dco_decode_opt_box_autoadd_u_16(arr[3]),
      createdAtSecs: dco_decode_opt_box_autoadd_i_64(arr[4]),
      suppressed: dco_decode_bool(arr[5]),
    );
  }

  @protected
  BuiltUnpublishFfi dco_decode_built_unpublish_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return BuiltUnpublishFfi(
      replacementEventJson: dco_decode_opt_String(arr[0]),
      deletionEventJson: dco_decode_opt_String(arr[1]),
      targets: dco_decode_list_String(arr[2]),
      suppressed: dco_decode_bool(arr[3]),
    );
  }

  @protected
  CatchupResultFfi dco_decode_catchup_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return CatchupResultFfi(
      circlesSwept: dco_decode_u_32(arr[0]),
      eventsApplied: dco_decode_u_32(arr[1]),
      eventsDeferred: dco_decode_u_32(arr[2]),
      cursorsAdvanced: dco_decode_u_32(arr[3]),
      deadlineHit: dco_decode_bool(arr[4]),
      relayErrors: dco_decode_u_32(arr[5]),
    );
  }

  @protected
  CircleCreationResultFfi dco_decode_circle_creation_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return CircleCreationResultFfi(
      circle: dco_decode_circle_ffi(arr[0]),
      welcomeEvents: dco_decode_list_gift_wrapped_welcome_ffi(arr[1]),
      pending: dco_decode_pending_state_ref_ffi(arr[2]),
    );
  }

  @protected
  CircleFfi dco_decode_circle_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return CircleFfi(
      mlsGroupId: dco_decode_list_prim_u_8_strict(arr[0]),
      nostrGroupId: dco_decode_list_prim_u_8_strict(arr[1]),
      displayName: dco_decode_String(arr[2]),
      circleType: dco_decode_String(arr[3]),
      relays: dco_decode_list_String(arr[4]),
      createdAt: dco_decode_i_64(arr[5]),
      updatedAt: dco_decode_i_64(arr[6]),
    );
  }

  @protected
  CircleHealthFfi dco_decode_circle_health_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return CircleHealthFfi(
      lastPublishAckedAtMs: dco_decode_opt_box_autoadd_i_64(arr[0]),
      lastPeerEventAtMs: dco_decode_opt_box_autoadd_i_64(arr[1]),
    );
  }

  @protected
  CircleMemberFfi dco_decode_circle_member_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return CircleMemberFfi(
      pubkey: dco_decode_String(arr[0]),
      npub: dco_decode_String(arr[1]),
      displayName: dco_decode_opt_String(arr[2]),
      isAdmin: dco_decode_bool(arr[3]),
    );
  }

  @protected
  CircleWithMembersFfi dco_decode_circle_with_members_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return CircleWithMembersFfi(
      circle: dco_decode_circle_ffi(arr[0]),
      membershipStatus: dco_decode_String(arr[1]),
      inviterPubkey: dco_decode_opt_String(arr[2]),
      members: dco_decode_list_circle_member_ffi(arr[3]),
    );
  }

  @protected
  CommitToPublishFfi dco_decode_commit_to_publish_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return CommitToPublishFfi(
      commitEventJson: dco_decode_String(arr[0]),
      pending: dco_decode_pending_state_ref_ffi(arr[1]),
    );
  }

  @protected
  ContactFfi dco_decode_contact_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return ContactFfi(
      pubkey: dco_decode_String(arr[0]),
      displayName: dco_decode_opt_String(arr[1]),
      notes: dco_decode_opt_String(arr[2]),
      createdAt: dco_decode_i_64(arr[3]),
      updatedAt: dco_decode_i_64(arr[4]),
    );
  }

  @protected
  ConvergenceSweepFfi dco_decode_convergence_sweep_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return ConvergenceSweepFfi(
      disposedMessages: dco_decode_u_32(arr[0]),
      discardedIntents: dco_decode_u_32(arr[1]),
      gatingRows: dco_decode_u_32(arr[2]),
      settled: dco_decode_bool(arr[3]),
    );
  }

  @protected
  DecryptLocationOutcomeFfi dco_decode_decrypt_location_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return DecryptLocationOutcomeFfi(
      results: dco_decode_list_location_message_result_ffi(arr[0]),
      autoCommits: dco_decode_list_commit_to_publish_ffi(arr[1]),
    );
  }

  @protected
  DecryptedLocationFfi dco_decode_decrypted_location_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 6)
      throw Exception('unexpected arr length: expect 6 but see ${arr.length}');
    return DecryptedLocationFfi(
      senderPubkey: dco_decode_String(arr[0]),
      latitude: dco_decode_f_64(arr[1]),
      longitude: dco_decode_f_64(arr[2]),
      geohash: dco_decode_String(arr[3]),
      timestamp: dco_decode_i_64(arr[4]),
      expiresAt: dco_decode_i_64(arr[5]),
    );
  }

  @protected
  DeferredSendFfi dco_decode_deferred_send_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return DeferredSendFfi(
      unresolvedInputs: dco_decode_u_32(arr[0]),
      discardedIntents: dco_decode_u_32(arr[1]),
      repaired: dco_decode_bool(arr[2]),
      commits: dco_decode_list_commit_to_publish_ffi(arr[3]),
      proposals: dco_decode_list_String(arr[4]),
    );
  }

  @protected
  DirectoryEntryFfi dco_decode_directory_entry_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return DirectoryEntryFfi(
      pubkeyHex: dco_decode_String(arr[0]),
      npub: dco_decode_String(arr[1]),
      tier: dco_decode_directory_tier_ffi(arr[2]),
    );
  }

  @protected
  DirectoryTierFfi dco_decode_directory_tier_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return DirectoryTierFfi.values[raw as int];
  }

  @protected
  EncryptLocationOutcomeFfi dco_decode_encrypt_location_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return EncryptLocationOutcomeFfi(
      sent: dco_decode_opt_box_autoadd_encrypted_location_ffi(arr[0]),
      deferredSend: dco_decode_opt_box_autoadd_deferred_send_ffi(arr[1]),
    );
  }

  @protected
  EncryptedLocationFfi dco_decode_encrypted_location_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return EncryptedLocationFfi(
      eventJson: dco_decode_String(arr[0]),
      nostrGroupId: dco_decode_list_prim_u_8_strict(arr[1]),
      relays: dco_decode_list_String(arr[2]),
    );
  }

  @protected
  double dco_decode_f_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as double;
  }

  @protected
  FfiGroupSpec dco_decode_ffi_group_spec(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return FfiGroupSpec(
      nostrGroupId: dco_decode_list_prim_u_8_strict(arr[0]),
      relays: dco_decode_list_String(arr[1]),
    );
  }

  @protected
  FfiRelayEvent dco_decode_ffi_relay_event(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 9)
      throw Exception('unexpected arr length: expect 9 but see ${arr.length}');
    return FfiRelayEvent(
      kind: dco_decode_ffi_relay_event_kind(arr[0]),
      nostrGroupId: dco_decode_opt_list_prim_u_8_strict(arr[1]),
      senderPubkey: dco_decode_opt_String(arr[2]),
      content: dco_decode_opt_String(arr[3]),
      eventCreatedAtSecs: dco_decode_opt_box_autoadd_i_64(arr[4]),
      evolutionEventJson: dco_decode_opt_String(arr[5]),
      giftWrapJson: dco_decode_opt_String(arr[6]),
      statusReason: dco_decode_opt_box_autoadd_ffi_sync_status_reason(arr[7]),
      unrecoverableNostrGroupId: dco_decode_opt_list_prim_u_8_strict(arr[8]),
    );
  }

  @protected
  FfiRelayEventKind dco_decode_ffi_relay_event_kind(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return FfiRelayEventKind.values[raw as int];
  }

  @protected
  FfiSyncStatusReason dco_decode_ffi_sync_status_reason(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return FfiSyncStatusReason.values[raw as int];
  }

  @protected
  ForceReleaseOutcomeFfi dco_decode_force_release_outcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return ForceReleaseOutcomeFfi.values[raw as int];
  }

  @protected
  GiftWrappedWelcomeFfi dco_decode_gift_wrapped_welcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return GiftWrappedWelcomeFfi(
      recipientPubkey: dco_decode_String(arr[0]),
      recipientRelays: dco_decode_list_String(arr[1]),
      eventJson: dco_decode_String(arr[2]),
    );
  }

  @protected
  int dco_decode_i_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  PlatformInt64 dco_decode_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeI64(raw);
  }

  @protected
  InvitationFfi dco_decode_invitation_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return InvitationFfi(
      mlsGroupId: dco_decode_list_prim_u_8_strict(arr[0]),
      circleName: dco_decode_String(arr[1]),
      inviterPubkey: dco_decode_String(arr[2]),
      inviterNpub: dco_decode_String(arr[3]),
      invitedAt: dco_decode_i_64(arr[4]),
    );
  }

  @protected
  KpMaintenanceActionFfi dco_decode_kp_maintenance_action_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return KpMaintenanceActionFfi.values[raw as int];
  }

  @protected
  KpMaintenanceOutcomeFfi dco_decode_kp_maintenance_outcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 8)
      throw Exception('unexpected arr length: expect 8 but see ${arr.length}');
    return KpMaintenanceOutcomeFfi(
      action: dco_decode_kp_maintenance_action_ffi(arr[0]),
      canonicalOnRelays: dco_decode_u_32(arr[1]),
      respondersProbed: dco_decode_u_32(arr[2]),
      relaysTargeted: dco_decode_u_32(arr[3]),
      relaysHealed: dco_decode_u_32(arr[4]),
      relayErrors: dco_decode_u_32(arr[5]),
      expiredInitKeyPurged: dco_decode_bool(arr[6]),
      retiredMalformedSlot: dco_decode_bool(arr[7]),
    );
  }

  @protected
  LastKnownLocationFfi dco_decode_last_known_location_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 10)
      throw Exception('unexpected arr length: expect 10 but see ${arr.length}');
    return LastKnownLocationFfi(
      nostrGroupId: dco_decode_list_prim_u_8_strict(arr[0]),
      senderPubkey: dco_decode_String(arr[1]),
      latitude: dco_decode_f_64(arr[2]),
      longitude: dco_decode_f_64(arr[3]),
      geohash: dco_decode_String(arr[4]),
      displayName: dco_decode_opt_String(arr[5]),
      timestamp: dco_decode_i_64(arr[6]),
      expiresAt: dco_decode_i_64(arr[7]),
      purgeAfter: dco_decode_i_64(arr[8]),
      updatedAt: dco_decode_i_64(arr[9]),
    );
  }

  @protected
  LeavePlanFfi dco_decode_leave_plan_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return LeavePlanFfi(
      kind: dco_decode_leave_plan_kind_ffi(arr[0]),
      successorHex: dco_decode_opt_String(arr[1]),
    );
  }

  @protected
  LeavePlanKindFfi dco_decode_leave_plan_kind_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LeavePlanKindFfi.values[raw as int];
  }

  @protected
  LegacyRetractionOutcomeFfi dco_decode_legacy_retraction_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return LegacyRetractionOutcomeFfi(
      alreadyDone: dco_decode_bool(arr[0]),
      legacy443Scrubbed: dco_decode_u_32(arr[1]),
      relayListRetracted: dco_decode_bool(arr[2]),
      relayErrors: dco_decode_u_32(arr[3]),
    );
  }

  @protected
  List<String> dco_decode_list_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_String).toList();
  }

  @protected
  List<CircleMemberFfi> dco_decode_list_circle_member_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_circle_member_ffi).toList();
  }

  @protected
  List<CircleWithMembersFfi> dco_decode_list_circle_with_members_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_circle_with_members_ffi)
        .toList();
  }

  @protected
  List<CommitToPublishFfi> dco_decode_list_commit_to_publish_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_commit_to_publish_ffi)
        .toList();
  }

  @protected
  List<ContactFfi> dco_decode_list_contact_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_contact_ffi).toList();
  }

  @protected
  List<DirectoryEntryFfi> dco_decode_list_directory_entry_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_directory_entry_ffi).toList();
  }

  @protected
  List<FfiGroupSpec> dco_decode_list_ffi_group_spec(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_ffi_group_spec).toList();
  }

  @protected
  List<GiftWrappedWelcomeFfi> dco_decode_list_gift_wrapped_welcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_gift_wrapped_welcome_ffi)
        .toList();
  }

  @protected
  List<InvitationFfi> dco_decode_list_invitation_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_invitation_ffi).toList();
  }

  @protected
  List<LastKnownLocationFfi> dco_decode_list_last_known_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_last_known_location_ffi)
        .toList();
  }

  @protected
  List<List<String>> dco_decode_list_list_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_list_String).toList();
  }

  @protected
  List<LocationMessageResultFfi> dco_decode_list_location_message_result_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_location_message_result_ffi)
        .toList();
  }

  @protected
  List<MemberKeyPackageFfi> dco_decode_list_member_key_package_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_member_key_package_ffi)
        .toList();
  }

  @protected
  List<int> dco_decode_list_prim_u_8_loose(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as List<int>;
  }

  @protected
  Uint8List dco_decode_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as Uint8List;
  }

  @protected
  List<ProfileMetadataFfi> dco_decode_list_profile_metadata_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_profile_metadata_ffi).toList();
  }

  @protected
  List<RelayConnectionStatusFfi> dco_decode_list_relay_connection_status_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_relay_connection_status_ffi)
        .toList();
  }

  @protected
  List<RelayGiftWrapFetchFfi> dco_decode_list_relay_gift_wrap_fetch_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>)
        .map(dco_decode_relay_gift_wrap_fetch_ffi)
        .toList();
  }

  @protected
  List<RelayRejectionFfi> dco_decode_list_relay_rejection_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_relay_rejection_ffi).toList();
  }

  @protected
  LocationMessageResultFfi dco_decode_location_message_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return LocationMessageResultFfi(
      kind: dco_decode_location_message_result_kind_ffi(arr[0]),
      location: dco_decode_opt_box_autoadd_decrypted_location_ffi(arr[1]),
      mlsGroupId: dco_decode_list_prim_u_8_strict(arr[2]),
      epoch: dco_decode_u_64(arr[3]),
    );
  }

  @protected
  LocationMessageResultKindFfi dco_decode_location_message_result_kind_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LocationMessageResultKindFfi.values[raw as int];
  }

  @protected
  LogAliasClassFfi dco_decode_log_alias_class_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return LogAliasClassFfi.values[raw as int];
  }

  @protected
  MemberKeyPackageFfi dco_decode_member_key_package_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return MemberKeyPackageFfi(
      keyPackageJson: dco_decode_String(arr[0]),
      inboxRelays: dco_decode_list_String(arr[1]),
      nip65Relays: dco_decode_list_String(arr[2]),
    );
  }

  @protected
  String? dco_decode_opt_String(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_String(raw);
  }

  @protected
  CircleWithMembersFfi? dco_decode_opt_box_autoadd_circle_with_members_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_circle_with_members_ffi(raw);
  }

  @protected
  CommitToPublishFfi? dco_decode_opt_box_autoadd_commit_to_publish_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_commit_to_publish_ffi(raw);
  }

  @protected
  ContactFfi? dco_decode_opt_box_autoadd_contact_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_contact_ffi(raw);
  }

  @protected
  DecryptedLocationFfi? dco_decode_opt_box_autoadd_decrypted_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_decrypted_location_ffi(raw);
  }

  @protected
  DeferredSendFfi? dco_decode_opt_box_autoadd_deferred_send_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_deferred_send_ffi(raw);
  }

  @protected
  EncryptedLocationFfi? dco_decode_opt_box_autoadd_encrypted_location_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_encrypted_location_ffi(raw);
  }

  @protected
  FfiSyncStatusReason? dco_decode_opt_box_autoadd_ffi_sync_status_reason(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_ffi_sync_status_reason(raw);
  }

  @protected
  PlatformInt64? dco_decode_opt_box_autoadd_i_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_i_64(raw);
  }

  @protected
  InvitationFfi? dco_decode_opt_box_autoadd_invitation_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_invitation_ffi(raw);
  }

  @protected
  MemberKeyPackageFfi? dco_decode_opt_box_autoadd_member_key_package_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_member_key_package_ffi(raw);
  }

  @protected
  ProfileMetadataFfi? dco_decode_opt_box_autoadd_profile_metadata_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_profile_metadata_ffi(raw);
  }

  @protected
  PublicIdentity? dco_decode_opt_box_autoadd_public_identity(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_public_identity(raw);
  }

  @protected
  SkipReasonFfi? dco_decode_opt_box_autoadd_skip_reason_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_skip_reason_ffi(raw);
  }

  @protected
  TileCacheEntryFfi? dco_decode_opt_box_autoadd_tile_cache_entry_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null
        ? null
        : dco_decode_box_autoadd_tile_cache_entry_ffi(raw);
  }

  @protected
  int? dco_decode_opt_box_autoadd_u_16(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_u_16(raw);
  }

  @protected
  int? dco_decode_opt_box_autoadd_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_box_autoadd_u_32(raw);
  }

  @protected
  Uint8List? dco_decode_opt_list_prim_u_8_strict(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw == null ? null : dco_decode_list_prim_u_8_strict(raw);
  }

  @protected
  PendingStateRefFfi dco_decode_pending_state_ref_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 1)
      throw Exception('unexpected arr length: expect 1 but see ${arr.length}');
    return PendingStateRefFfi(token: dco_decode_u_64(arr[0]));
  }

  @protected
  ProfileMetadataFfi dco_decode_profile_metadata_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 9)
      throw Exception('unexpected arr length: expect 9 but see ${arr.length}');
    return ProfileMetadataFfi(
      pubkeyHex: dco_decode_String(arr[0]),
      npub: dco_decode_String(arr[1]),
      displayName: dco_decode_opt_String(arr[2]),
      name: dco_decode_opt_String(arr[3]),
      about: dco_decode_opt_String(arr[4]),
      hasPicture: dco_decode_bool(arr[5]),
      isKnown: dco_decode_bool(arr[6]),
      fetchedAt: dco_decode_i_64(arr[7]),
      pictureSha256Hex: dco_decode_opt_String(arr[8]),
    );
  }

  @protected
  ProfilePendingStateFfi dco_decode_profile_pending_state_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return ProfilePendingStateFfi(
      pending: dco_decode_bool(arr[0]),
      partial: dco_decode_bool(arr[1]),
      retryDue: dco_decode_bool(arr[2]),
    );
  }

  @protected
  ProfilePictureRefFfi dco_decode_profile_picture_ref_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return ProfilePictureRefFfi(
      pubkeyHex: dco_decode_String(arr[0]),
      sha256Hex: dco_decode_String(arr[1]),
    );
  }

  @protected
  ProfilePoolStatusFfi dco_decode_profile_pool_status_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return ProfilePoolStatusFfi(
      configured: dco_decode_u_32(arr[0]),
      excluded: dco_decode_u_32(arr[1]),
      usable: dco_decode_u_32(arr[2]),
      isUnderflow: dco_decode_bool(arr[3]),
    );
  }

  @protected
  ProfileSyncOutcomeFfi dco_decode_profile_sync_outcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return ProfileSyncOutcomeFfi.values[raw as int];
  }

  @protected
  ProfileSyncResultFfi dco_decode_profile_sync_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return ProfileSyncResultFfi(
      outcome: dco_decode_profile_sync_outcome_ffi(arr[0]),
      relaysAcked: dco_decode_u_32(arr[1]),
      relaysAttempted: dco_decode_u_32(arr[2]),
      stillPending: dco_decode_bool(arr[3]),
    );
  }

  @protected
  PublicIdentity dco_decode_public_identity(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return PublicIdentity(
      pubkeyHex: dco_decode_String(arr[0]),
      npub: dco_decode_String(arr[1]),
      createdAt: dco_decode_i_64(arr[2]),
    );
  }

  @protected
  PublishResultFfi dco_decode_publish_result_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 5)
      throw Exception('unexpected arr length: expect 5 but see ${arr.length}');
    return PublishResultFfi(
      eventId: dco_decode_String(arr[0]),
      acceptedBy: dco_decode_list_String(arr[1]),
      rejectedBy: dco_decode_list_relay_rejection_ffi(arr[2]),
      failed: dco_decode_list_String(arr[3]),
      isSuccess: dco_decode_bool(arr[4]),
    );
  }

  @protected
  RelayConnectionStatusFfi dco_decode_relay_connection_status_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return RelayConnectionStatusFfi(
      url: dco_decode_String(arr[0]),
      status: dco_decode_String(arr[1]),
      lastSeen: dco_decode_opt_box_autoadd_i_64(arr[2]),
    );
  }

  @protected
  RelayEventCheckFfi dco_decode_relay_event_check_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return RelayEventCheckFfi(
      relayUrl: dco_decode_String(arr[0]),
      found: dco_decode_bool(arr[1]),
      eventCount: dco_decode_u_32(arr[2]),
      newestTimestamp: dco_decode_opt_box_autoadd_i_64(arr[3]),
    );
  }

  @protected
  RelayGiftWrapFetchFfi dco_decode_relay_gift_wrap_fetch_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return RelayGiftWrapFetchFfi(
      relayUrl: dco_decode_String(arr[0]),
      responded: dco_decode_bool(arr[1]),
      events: dco_decode_list_String(arr[2]),
    );
  }

  @protected
  RelayListActionFfi dco_decode_relay_list_action_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return RelayListActionFfi.values[raw as int];
  }

  @protected
  RelayListCategoryOutcomeFfi dco_decode_relay_list_category_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return RelayListCategoryOutcomeFfi(
      action: dco_decode_relay_list_action_ffi(arr[0]),
      respondersProbed: dco_decode_u_32(arr[1]),
      relaysHealed: dco_decode_u_32(arr[2]),
      relayErrors: dco_decode_u_32(arr[3]),
    );
  }

  @protected
  RelayListMaintenanceOutcomeFfi dco_decode_relay_list_maintenance_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return RelayListMaintenanceOutcomeFfi(
      inbox: dco_decode_relay_list_category_outcome_ffi(arr[0]),
      keyPackage: dco_decode_relay_list_category_outcome_ffi(arr[1]),
    );
  }

  @protected
  RelayRejectionFfi dco_decode_relay_rejection_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 2)
      throw Exception('unexpected arr length: expect 2 but see ${arr.length}');
    return RelayRejectionFfi(
      url: dco_decode_String(arr[0]),
      reason: dco_decode_String(arr[1]),
    );
  }

  @protected
  RelayTypeFfi dco_decode_relay_type_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return RelayTypeFfi.values[raw as int];
  }

  @protected
  RepairRotationOutcomeFfi dco_decode_repair_rotation_outcome_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return RepairRotationOutcomeFfi(
      rotated: dco_decode_opt_box_autoadd_commit_to_publish_ffi(arr[0]),
      skipped: dco_decode_opt_box_autoadd_skip_reason_ffi(arr[1]),
      deferredSend: dco_decode_opt_box_autoadd_deferred_send_ffi(arr[2]),
    );
  }

  @protected
  SignedLocationEventFfi dco_decode_signed_location_event_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return SignedLocationEventFfi(
      id: dco_decode_String(arr[0]),
      pubkey: dco_decode_String(arr[1]),
      createdAt: dco_decode_i_64(arr[2]),
      kind: dco_decode_u_16(arr[3]),
      tags: dco_decode_list_list_String(arr[4]),
      content: dco_decode_String(arr[5]),
      sig: dco_decode_String(arr[6]),
    );
  }

  @protected
  SkipReasonFfi dco_decode_skip_reason_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return SkipReasonFfi.values[raw as int];
  }

  @protected
  SubscriptionHealthActionFfi dco_decode_subscription_health_action_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return SubscriptionHealthActionFfi.values[raw as int];
  }

  @protected
  SubscriptionHealthOutcomeFfi dco_decode_subscription_health_outcome_ffi(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 7)
      throw Exception('unexpected arr length: expect 7 but see ${arr.length}');
    return SubscriptionHealthOutcomeFfi(
      action: dco_decode_subscription_health_action_ffi(arr[0]),
      relaysTotal: dco_decode_u_32(arr[1]),
      relaysStillConnecting: dco_decode_u_32(arr[2]),
      relaysDisconnected: dco_decode_u_32(arr[3]),
      subscriptionsExpected: dco_decode_u_32(arr[4]),
      subscriptionsLive: dco_decode_u_32(arr[5]),
      subscriptionsSilent: dco_decode_u_32(arr[6]),
    );
  }

  @protected
  TileCacheEntryFfi dco_decode_tile_cache_entry_ffi(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return TileCacheEntryFfi(
      bytes: dco_decode_list_prim_u_8_strict(arr[0]),
      staleAtMs: dco_decode_i_64(arr[1]),
      lastModifiedMs: dco_decode_opt_box_autoadd_i_64(arr[2]),
      etag: dco_decode_opt_String(arr[3]),
    );
  }

  @protected
  int dco_decode_u_16(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  int dco_decode_u_32(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  BigInt dco_decode_u_64(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeU64(raw);
  }

  @protected
  int dco_decode_u_8(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return raw as int;
  }

  @protected
  void dco_decode_unit(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return;
  }

  @protected
  BigInt dco_decode_usize(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return dcoDecodeU64(raw);
  }

  @protected
  AnyhowException sse_decode_AnyhowException(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_String(deserializer);
    return AnyhowException(inner);
  }

  @protected
  CircleManagerFfi
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  HavenCore
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return HavenCoreImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LiveSyncFfi
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationEventService
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationMessage
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationMessageImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationSettings
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  NostrIdentityManager
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  RelayManagerFfi
  sse_decode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  HavenCore
  sse_decode_Auto_RefMut_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return HavenCoreImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  CircleManagerFfi
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  HavenCore
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return HavenCoreImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LiveSyncFfi
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationEventService
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationMessage
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationMessageImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationSettings
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  NostrIdentityManager
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  RelayManagerFfi
  sse_decode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  CircleManagerFfi
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return CircleManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  HavenCore
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return HavenCoreImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LiveSyncFfi
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LiveSyncFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationEventService
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationEventServiceImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationMessage
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationMessageImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  LocationSettings
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return LocationSettingsImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  NostrIdentityManager
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return NostrIdentityManagerImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  RelayManagerFfi
  sse_decode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return RelayManagerFfiImpl.frbInternalSseDecode(
      sse_decode_usize(deserializer),
      sse_decode_i_32(deserializer),
    );
  }

  @protected
  RustStreamSink<FfiRelayEvent> sse_decode_StreamSink_ffi_relay_event_Sse(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    throw UnimplementedError('Unreachable ()');
  }

  @protected
  String sse_decode_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_list_prim_u_8_strict(deserializer);
    return utf8.decoder.convert(inner);
  }

  @protected
  AddMembersResultFfi sse_decode_add_members_result_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_commitEventJson = sse_decode_String(deserializer);
    var var_welcomeEvents = sse_decode_list_gift_wrapped_welcome_ffi(
      deserializer,
    );
    var var_pending = sse_decode_pending_state_ref_ffi(deserializer);
    return AddMembersResultFfi(
      commitEventJson: var_commitEventJson,
      welcomeEvents: var_welcomeEvents,
      pending: var_pending,
    );
  }

  @protected
  BacklogOutcomeFfi sse_decode_backlog_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return BacklogOutcomeFfi.values[inner];
  }

  @protected
  bool sse_decode_bool(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8() != 0;
  }

  @protected
  CircleWithMembersFfi sse_decode_box_autoadd_circle_with_members_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_circle_with_members_ffi(deserializer));
  }

  @protected
  CommitToPublishFfi sse_decode_box_autoadd_commit_to_publish_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_commit_to_publish_ffi(deserializer));
  }

  @protected
  ContactFfi sse_decode_box_autoadd_contact_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_contact_ffi(deserializer));
  }

  @protected
  DecryptedLocationFfi sse_decode_box_autoadd_decrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_decrypted_location_ffi(deserializer));
  }

  @protected
  DeferredSendFfi sse_decode_box_autoadd_deferred_send_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_deferred_send_ffi(deserializer));
  }

  @protected
  EncryptedLocationFfi sse_decode_box_autoadd_encrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_encrypted_location_ffi(deserializer));
  }

  @protected
  FfiGroupSpec sse_decode_box_autoadd_ffi_group_spec(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_ffi_group_spec(deserializer));
  }

  @protected
  FfiSyncStatusReason sse_decode_box_autoadd_ffi_sync_status_reason(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_ffi_sync_status_reason(deserializer));
  }

  @protected
  PlatformInt64 sse_decode_box_autoadd_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_i_64(deserializer));
  }

  @protected
  InvitationFfi sse_decode_box_autoadd_invitation_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_invitation_ffi(deserializer));
  }

  @protected
  LastKnownLocationFfi sse_decode_box_autoadd_last_known_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_last_known_location_ffi(deserializer));
  }

  @protected
  MemberKeyPackageFfi sse_decode_box_autoadd_member_key_package_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_member_key_package_ffi(deserializer));
  }

  @protected
  PendingStateRefFfi sse_decode_box_autoadd_pending_state_ref_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_pending_state_ref_ffi(deserializer));
  }

  @protected
  ProfileMetadataFfi sse_decode_box_autoadd_profile_metadata_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_profile_metadata_ffi(deserializer));
  }

  @protected
  PublicIdentity sse_decode_box_autoadd_public_identity(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_public_identity(deserializer));
  }

  @protected
  SignedLocationEventFfi sse_decode_box_autoadd_signed_location_event_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_signed_location_event_ffi(deserializer));
  }

  @protected
  SkipReasonFfi sse_decode_box_autoadd_skip_reason_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_skip_reason_ffi(deserializer));
  }

  @protected
  TileCacheEntryFfi sse_decode_box_autoadd_tile_cache_entry_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_tile_cache_entry_ffi(deserializer));
  }

  @protected
  int sse_decode_box_autoadd_u_16(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_u_16(deserializer));
  }

  @protected
  int sse_decode_box_autoadd_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return (sse_decode_u_32(deserializer));
  }

  @protected
  BuiltRelayListEventFfi sse_decode_built_relay_list_event_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_eventJson = sse_decode_opt_String(deserializer);
    var var_eventIdHex = sse_decode_opt_String(deserializer);
    var var_targets = sse_decode_list_String(deserializer);
    var var_kind = sse_decode_opt_box_autoadd_u_16(deserializer);
    var var_createdAtSecs = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_suppressed = sse_decode_bool(deserializer);
    return BuiltRelayListEventFfi(
      eventJson: var_eventJson,
      eventIdHex: var_eventIdHex,
      targets: var_targets,
      kind: var_kind,
      createdAtSecs: var_createdAtSecs,
      suppressed: var_suppressed,
    );
  }

  @protected
  BuiltUnpublishFfi sse_decode_built_unpublish_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_replacementEventJson = sse_decode_opt_String(deserializer);
    var var_deletionEventJson = sse_decode_opt_String(deserializer);
    var var_targets = sse_decode_list_String(deserializer);
    var var_suppressed = sse_decode_bool(deserializer);
    return BuiltUnpublishFfi(
      replacementEventJson: var_replacementEventJson,
      deletionEventJson: var_deletionEventJson,
      targets: var_targets,
      suppressed: var_suppressed,
    );
  }

  @protected
  CatchupResultFfi sse_decode_catchup_result_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_circlesSwept = sse_decode_u_32(deserializer);
    var var_eventsApplied = sse_decode_u_32(deserializer);
    var var_eventsDeferred = sse_decode_u_32(deserializer);
    var var_cursorsAdvanced = sse_decode_u_32(deserializer);
    var var_deadlineHit = sse_decode_bool(deserializer);
    var var_relayErrors = sse_decode_u_32(deserializer);
    return CatchupResultFfi(
      circlesSwept: var_circlesSwept,
      eventsApplied: var_eventsApplied,
      eventsDeferred: var_eventsDeferred,
      cursorsAdvanced: var_cursorsAdvanced,
      deadlineHit: var_deadlineHit,
      relayErrors: var_relayErrors,
    );
  }

  @protected
  CircleCreationResultFfi sse_decode_circle_creation_result_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_circle = sse_decode_circle_ffi(deserializer);
    var var_welcomeEvents = sse_decode_list_gift_wrapped_welcome_ffi(
      deserializer,
    );
    var var_pending = sse_decode_pending_state_ref_ffi(deserializer);
    return CircleCreationResultFfi(
      circle: var_circle,
      welcomeEvents: var_welcomeEvents,
      pending: var_pending,
    );
  }

  @protected
  CircleFfi sse_decode_circle_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_mlsGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_nostrGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_displayName = sse_decode_String(deserializer);
    var var_circleType = sse_decode_String(deserializer);
    var var_relays = sse_decode_list_String(deserializer);
    var var_createdAt = sse_decode_i_64(deserializer);
    var var_updatedAt = sse_decode_i_64(deserializer);
    return CircleFfi(
      mlsGroupId: var_mlsGroupId,
      nostrGroupId: var_nostrGroupId,
      displayName: var_displayName,
      circleType: var_circleType,
      relays: var_relays,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
    );
  }

  @protected
  CircleHealthFfi sse_decode_circle_health_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_lastPublishAckedAtMs = sse_decode_opt_box_autoadd_i_64(
      deserializer,
    );
    var var_lastPeerEventAtMs = sse_decode_opt_box_autoadd_i_64(deserializer);
    return CircleHealthFfi(
      lastPublishAckedAtMs: var_lastPublishAckedAtMs,
      lastPeerEventAtMs: var_lastPeerEventAtMs,
    );
  }

  @protected
  CircleMemberFfi sse_decode_circle_member_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkey = sse_decode_String(deserializer);
    var var_npub = sse_decode_String(deserializer);
    var var_displayName = sse_decode_opt_String(deserializer);
    var var_isAdmin = sse_decode_bool(deserializer);
    return CircleMemberFfi(
      pubkey: var_pubkey,
      npub: var_npub,
      displayName: var_displayName,
      isAdmin: var_isAdmin,
    );
  }

  @protected
  CircleWithMembersFfi sse_decode_circle_with_members_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_circle = sse_decode_circle_ffi(deserializer);
    var var_membershipStatus = sse_decode_String(deserializer);
    var var_inviterPubkey = sse_decode_opt_String(deserializer);
    var var_members = sse_decode_list_circle_member_ffi(deserializer);
    return CircleWithMembersFfi(
      circle: var_circle,
      membershipStatus: var_membershipStatus,
      inviterPubkey: var_inviterPubkey,
      members: var_members,
    );
  }

  @protected
  CommitToPublishFfi sse_decode_commit_to_publish_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_commitEventJson = sse_decode_String(deserializer);
    var var_pending = sse_decode_pending_state_ref_ffi(deserializer);
    return CommitToPublishFfi(
      commitEventJson: var_commitEventJson,
      pending: var_pending,
    );
  }

  @protected
  ContactFfi sse_decode_contact_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkey = sse_decode_String(deserializer);
    var var_displayName = sse_decode_opt_String(deserializer);
    var var_notes = sse_decode_opt_String(deserializer);
    var var_createdAt = sse_decode_i_64(deserializer);
    var var_updatedAt = sse_decode_i_64(deserializer);
    return ContactFfi(
      pubkey: var_pubkey,
      displayName: var_displayName,
      notes: var_notes,
      createdAt: var_createdAt,
      updatedAt: var_updatedAt,
    );
  }

  @protected
  ConvergenceSweepFfi sse_decode_convergence_sweep_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_disposedMessages = sse_decode_u_32(deserializer);
    var var_discardedIntents = sse_decode_u_32(deserializer);
    var var_gatingRows = sse_decode_u_32(deserializer);
    var var_settled = sse_decode_bool(deserializer);
    return ConvergenceSweepFfi(
      disposedMessages: var_disposedMessages,
      discardedIntents: var_discardedIntents,
      gatingRows: var_gatingRows,
      settled: var_settled,
    );
  }

  @protected
  DecryptLocationOutcomeFfi sse_decode_decrypt_location_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_results = sse_decode_list_location_message_result_ffi(deserializer);
    var var_autoCommits = sse_decode_list_commit_to_publish_ffi(deserializer);
    return DecryptLocationOutcomeFfi(
      results: var_results,
      autoCommits: var_autoCommits,
    );
  }

  @protected
  DecryptedLocationFfi sse_decode_decrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_senderPubkey = sse_decode_String(deserializer);
    var var_latitude = sse_decode_f_64(deserializer);
    var var_longitude = sse_decode_f_64(deserializer);
    var var_geohash = sse_decode_String(deserializer);
    var var_timestamp = sse_decode_i_64(deserializer);
    var var_expiresAt = sse_decode_i_64(deserializer);
    return DecryptedLocationFfi(
      senderPubkey: var_senderPubkey,
      latitude: var_latitude,
      longitude: var_longitude,
      geohash: var_geohash,
      timestamp: var_timestamp,
      expiresAt: var_expiresAt,
    );
  }

  @protected
  DeferredSendFfi sse_decode_deferred_send_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_unresolvedInputs = sse_decode_u_32(deserializer);
    var var_discardedIntents = sse_decode_u_32(deserializer);
    var var_repaired = sse_decode_bool(deserializer);
    var var_commits = sse_decode_list_commit_to_publish_ffi(deserializer);
    var var_proposals = sse_decode_list_String(deserializer);
    return DeferredSendFfi(
      unresolvedInputs: var_unresolvedInputs,
      discardedIntents: var_discardedIntents,
      repaired: var_repaired,
      commits: var_commits,
      proposals: var_proposals,
    );
  }

  @protected
  DirectoryEntryFfi sse_decode_directory_entry_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkeyHex = sse_decode_String(deserializer);
    var var_npub = sse_decode_String(deserializer);
    var var_tier = sse_decode_directory_tier_ffi(deserializer);
    return DirectoryEntryFfi(
      pubkeyHex: var_pubkeyHex,
      npub: var_npub,
      tier: var_tier,
    );
  }

  @protected
  DirectoryTierFfi sse_decode_directory_tier_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return DirectoryTierFfi.values[inner];
  }

  @protected
  EncryptLocationOutcomeFfi sse_decode_encrypt_location_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_sent = sse_decode_opt_box_autoadd_encrypted_location_ffi(
      deserializer,
    );
    var var_deferredSend = sse_decode_opt_box_autoadd_deferred_send_ffi(
      deserializer,
    );
    return EncryptLocationOutcomeFfi(
      sent: var_sent,
      deferredSend: var_deferredSend,
    );
  }

  @protected
  EncryptedLocationFfi sse_decode_encrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_eventJson = sse_decode_String(deserializer);
    var var_nostrGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_relays = sse_decode_list_String(deserializer);
    return EncryptedLocationFfi(
      eventJson: var_eventJson,
      nostrGroupId: var_nostrGroupId,
      relays: var_relays,
    );
  }

  @protected
  double sse_decode_f_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getFloat64();
  }

  @protected
  FfiGroupSpec sse_decode_ffi_group_spec(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_nostrGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_relays = sse_decode_list_String(deserializer);
    return FfiGroupSpec(nostrGroupId: var_nostrGroupId, relays: var_relays);
  }

  @protected
  FfiRelayEvent sse_decode_ffi_relay_event(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_kind = sse_decode_ffi_relay_event_kind(deserializer);
    var var_nostrGroupId = sse_decode_opt_list_prim_u_8_strict(deserializer);
    var var_senderPubkey = sse_decode_opt_String(deserializer);
    var var_content = sse_decode_opt_String(deserializer);
    var var_eventCreatedAtSecs = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_evolutionEventJson = sse_decode_opt_String(deserializer);
    var var_giftWrapJson = sse_decode_opt_String(deserializer);
    var var_statusReason = sse_decode_opt_box_autoadd_ffi_sync_status_reason(
      deserializer,
    );
    var var_unrecoverableNostrGroupId = sse_decode_opt_list_prim_u_8_strict(
      deserializer,
    );
    return FfiRelayEvent(
      kind: var_kind,
      nostrGroupId: var_nostrGroupId,
      senderPubkey: var_senderPubkey,
      content: var_content,
      eventCreatedAtSecs: var_eventCreatedAtSecs,
      evolutionEventJson: var_evolutionEventJson,
      giftWrapJson: var_giftWrapJson,
      statusReason: var_statusReason,
      unrecoverableNostrGroupId: var_unrecoverableNostrGroupId,
    );
  }

  @protected
  FfiRelayEventKind sse_decode_ffi_relay_event_kind(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return FfiRelayEventKind.values[inner];
  }

  @protected
  FfiSyncStatusReason sse_decode_ffi_sync_status_reason(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return FfiSyncStatusReason.values[inner];
  }

  @protected
  ForceReleaseOutcomeFfi sse_decode_force_release_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return ForceReleaseOutcomeFfi.values[inner];
  }

  @protected
  GiftWrappedWelcomeFfi sse_decode_gift_wrapped_welcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_recipientPubkey = sse_decode_String(deserializer);
    var var_recipientRelays = sse_decode_list_String(deserializer);
    var var_eventJson = sse_decode_String(deserializer);
    return GiftWrappedWelcomeFfi(
      recipientPubkey: var_recipientPubkey,
      recipientRelays: var_recipientRelays,
      eventJson: var_eventJson,
    );
  }

  @protected
  int sse_decode_i_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getInt32();
  }

  @protected
  PlatformInt64 sse_decode_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getPlatformInt64();
  }

  @protected
  InvitationFfi sse_decode_invitation_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_mlsGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_circleName = sse_decode_String(deserializer);
    var var_inviterPubkey = sse_decode_String(deserializer);
    var var_inviterNpub = sse_decode_String(deserializer);
    var var_invitedAt = sse_decode_i_64(deserializer);
    return InvitationFfi(
      mlsGroupId: var_mlsGroupId,
      circleName: var_circleName,
      inviterPubkey: var_inviterPubkey,
      inviterNpub: var_inviterNpub,
      invitedAt: var_invitedAt,
    );
  }

  @protected
  KpMaintenanceActionFfi sse_decode_kp_maintenance_action_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return KpMaintenanceActionFfi.values[inner];
  }

  @protected
  KpMaintenanceOutcomeFfi sse_decode_kp_maintenance_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_action = sse_decode_kp_maintenance_action_ffi(deserializer);
    var var_canonicalOnRelays = sse_decode_u_32(deserializer);
    var var_respondersProbed = sse_decode_u_32(deserializer);
    var var_relaysTargeted = sse_decode_u_32(deserializer);
    var var_relaysHealed = sse_decode_u_32(deserializer);
    var var_relayErrors = sse_decode_u_32(deserializer);
    var var_expiredInitKeyPurged = sse_decode_bool(deserializer);
    var var_retiredMalformedSlot = sse_decode_bool(deserializer);
    return KpMaintenanceOutcomeFfi(
      action: var_action,
      canonicalOnRelays: var_canonicalOnRelays,
      respondersProbed: var_respondersProbed,
      relaysTargeted: var_relaysTargeted,
      relaysHealed: var_relaysHealed,
      relayErrors: var_relayErrors,
      expiredInitKeyPurged: var_expiredInitKeyPurged,
      retiredMalformedSlot: var_retiredMalformedSlot,
    );
  }

  @protected
  LastKnownLocationFfi sse_decode_last_known_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_nostrGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_senderPubkey = sse_decode_String(deserializer);
    var var_latitude = sse_decode_f_64(deserializer);
    var var_longitude = sse_decode_f_64(deserializer);
    var var_geohash = sse_decode_String(deserializer);
    var var_displayName = sse_decode_opt_String(deserializer);
    var var_timestamp = sse_decode_i_64(deserializer);
    var var_expiresAt = sse_decode_i_64(deserializer);
    var var_purgeAfter = sse_decode_i_64(deserializer);
    var var_updatedAt = sse_decode_i_64(deserializer);
    return LastKnownLocationFfi(
      nostrGroupId: var_nostrGroupId,
      senderPubkey: var_senderPubkey,
      latitude: var_latitude,
      longitude: var_longitude,
      geohash: var_geohash,
      displayName: var_displayName,
      timestamp: var_timestamp,
      expiresAt: var_expiresAt,
      purgeAfter: var_purgeAfter,
      updatedAt: var_updatedAt,
    );
  }

  @protected
  LeavePlanFfi sse_decode_leave_plan_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_kind = sse_decode_leave_plan_kind_ffi(deserializer);
    var var_successorHex = sse_decode_opt_String(deserializer);
    return LeavePlanFfi(kind: var_kind, successorHex: var_successorHex);
  }

  @protected
  LeavePlanKindFfi sse_decode_leave_plan_kind_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return LeavePlanKindFfi.values[inner];
  }

  @protected
  LegacyRetractionOutcomeFfi sse_decode_legacy_retraction_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_alreadyDone = sse_decode_bool(deserializer);
    var var_legacy443Scrubbed = sse_decode_u_32(deserializer);
    var var_relayListRetracted = sse_decode_bool(deserializer);
    var var_relayErrors = sse_decode_u_32(deserializer);
    return LegacyRetractionOutcomeFfi(
      alreadyDone: var_alreadyDone,
      legacy443Scrubbed: var_legacy443Scrubbed,
      relayListRetracted: var_relayListRetracted,
      relayErrors: var_relayErrors,
    );
  }

  @protected
  List<String> sse_decode_list_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <String>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_String(deserializer));
    }
    return ans_;
  }

  @protected
  List<CircleMemberFfi> sse_decode_list_circle_member_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CircleMemberFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_circle_member_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<CircleWithMembersFfi> sse_decode_list_circle_with_members_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CircleWithMembersFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_circle_with_members_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<CommitToPublishFfi> sse_decode_list_commit_to_publish_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <CommitToPublishFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_commit_to_publish_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<ContactFfi> sse_decode_list_contact_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ContactFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_contact_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<DirectoryEntryFfi> sse_decode_list_directory_entry_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <DirectoryEntryFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_directory_entry_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<FfiGroupSpec> sse_decode_list_ffi_group_spec(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <FfiGroupSpec>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_ffi_group_spec(deserializer));
    }
    return ans_;
  }

  @protected
  List<GiftWrappedWelcomeFfi> sse_decode_list_gift_wrapped_welcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <GiftWrappedWelcomeFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_gift_wrapped_welcome_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<InvitationFfi> sse_decode_list_invitation_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <InvitationFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_invitation_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<LastKnownLocationFfi> sse_decode_list_last_known_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <LastKnownLocationFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_last_known_location_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<List<String>> sse_decode_list_list_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <List<String>>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_list_String(deserializer));
    }
    return ans_;
  }

  @protected
  List<LocationMessageResultFfi> sse_decode_list_location_message_result_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <LocationMessageResultFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_location_message_result_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<MemberKeyPackageFfi> sse_decode_list_member_key_package_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <MemberKeyPackageFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_member_key_package_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<int> sse_decode_list_prim_u_8_loose(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint8List(len_);
  }

  @protected
  Uint8List sse_decode_list_prim_u_8_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var len_ = sse_decode_i_32(deserializer);
    return deserializer.buffer.getUint8List(len_);
  }

  @protected
  List<ProfileMetadataFfi> sse_decode_list_profile_metadata_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ProfileMetadataFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_profile_metadata_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<RelayConnectionStatusFfi> sse_decode_list_relay_connection_status_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <RelayConnectionStatusFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_relay_connection_status_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<RelayGiftWrapFetchFfi> sse_decode_list_relay_gift_wrap_fetch_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <RelayGiftWrapFetchFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_relay_gift_wrap_fetch_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  List<RelayRejectionFfi> sse_decode_list_relay_rejection_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <RelayRejectionFfi>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_relay_rejection_ffi(deserializer));
    }
    return ans_;
  }

  @protected
  LocationMessageResultFfi sse_decode_location_message_result_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_kind = sse_decode_location_message_result_kind_ffi(deserializer);
    var var_location = sse_decode_opt_box_autoadd_decrypted_location_ffi(
      deserializer,
    );
    var var_mlsGroupId = sse_decode_list_prim_u_8_strict(deserializer);
    var var_epoch = sse_decode_u_64(deserializer);
    return LocationMessageResultFfi(
      kind: var_kind,
      location: var_location,
      mlsGroupId: var_mlsGroupId,
      epoch: var_epoch,
    );
  }

  @protected
  LocationMessageResultKindFfi sse_decode_location_message_result_kind_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return LocationMessageResultKindFfi.values[inner];
  }

  @protected
  LogAliasClassFfi sse_decode_log_alias_class_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return LogAliasClassFfi.values[inner];
  }

  @protected
  MemberKeyPackageFfi sse_decode_member_key_package_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_keyPackageJson = sse_decode_String(deserializer);
    var var_inboxRelays = sse_decode_list_String(deserializer);
    var var_nip65Relays = sse_decode_list_String(deserializer);
    return MemberKeyPackageFfi(
      keyPackageJson: var_keyPackageJson,
      inboxRelays: var_inboxRelays,
      nip65Relays: var_nip65Relays,
    );
  }

  @protected
  String? sse_decode_opt_String(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_String(deserializer));
    } else {
      return null;
    }
  }

  @protected
  CircleWithMembersFfi? sse_decode_opt_box_autoadd_circle_with_members_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_circle_with_members_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  CommitToPublishFfi? sse_decode_opt_box_autoadd_commit_to_publish_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_commit_to_publish_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  ContactFfi? sse_decode_opt_box_autoadd_contact_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_contact_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  DecryptedLocationFfi? sse_decode_opt_box_autoadd_decrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_decrypted_location_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  DeferredSendFfi? sse_decode_opt_box_autoadd_deferred_send_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_deferred_send_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  EncryptedLocationFfi? sse_decode_opt_box_autoadd_encrypted_location_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_encrypted_location_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  FfiSyncStatusReason? sse_decode_opt_box_autoadd_ffi_sync_status_reason(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_ffi_sync_status_reason(deserializer));
    } else {
      return null;
    }
  }

  @protected
  PlatformInt64? sse_decode_opt_box_autoadd_i_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_i_64(deserializer));
    } else {
      return null;
    }
  }

  @protected
  InvitationFfi? sse_decode_opt_box_autoadd_invitation_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_invitation_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  MemberKeyPackageFfi? sse_decode_opt_box_autoadd_member_key_package_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_member_key_package_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  ProfileMetadataFfi? sse_decode_opt_box_autoadd_profile_metadata_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_profile_metadata_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  PublicIdentity? sse_decode_opt_box_autoadd_public_identity(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_public_identity(deserializer));
    } else {
      return null;
    }
  }

  @protected
  SkipReasonFfi? sse_decode_opt_box_autoadd_skip_reason_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_skip_reason_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  TileCacheEntryFfi? sse_decode_opt_box_autoadd_tile_cache_entry_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_tile_cache_entry_ffi(deserializer));
    } else {
      return null;
    }
  }

  @protected
  int? sse_decode_opt_box_autoadd_u_16(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_u_16(deserializer));
    } else {
      return null;
    }
  }

  @protected
  int? sse_decode_opt_box_autoadd_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_box_autoadd_u_32(deserializer));
    } else {
      return null;
    }
  }

  @protected
  Uint8List? sse_decode_opt_list_prim_u_8_strict(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    if (sse_decode_bool(deserializer)) {
      return (sse_decode_list_prim_u_8_strict(deserializer));
    } else {
      return null;
    }
  }

  @protected
  PendingStateRefFfi sse_decode_pending_state_ref_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_token = sse_decode_u_64(deserializer);
    return PendingStateRefFfi(token: var_token);
  }

  @protected
  ProfileMetadataFfi sse_decode_profile_metadata_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkeyHex = sse_decode_String(deserializer);
    var var_npub = sse_decode_String(deserializer);
    var var_displayName = sse_decode_opt_String(deserializer);
    var var_name = sse_decode_opt_String(deserializer);
    var var_about = sse_decode_opt_String(deserializer);
    var var_hasPicture = sse_decode_bool(deserializer);
    var var_isKnown = sse_decode_bool(deserializer);
    var var_fetchedAt = sse_decode_i_64(deserializer);
    var var_pictureSha256Hex = sse_decode_opt_String(deserializer);
    return ProfileMetadataFfi(
      pubkeyHex: var_pubkeyHex,
      npub: var_npub,
      displayName: var_displayName,
      name: var_name,
      about: var_about,
      hasPicture: var_hasPicture,
      isKnown: var_isKnown,
      fetchedAt: var_fetchedAt,
      pictureSha256Hex: var_pictureSha256Hex,
    );
  }

  @protected
  ProfilePendingStateFfi sse_decode_profile_pending_state_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pending = sse_decode_bool(deserializer);
    var var_partial = sse_decode_bool(deserializer);
    var var_retryDue = sse_decode_bool(deserializer);
    return ProfilePendingStateFfi(
      pending: var_pending,
      partial: var_partial,
      retryDue: var_retryDue,
    );
  }

  @protected
  ProfilePictureRefFfi sse_decode_profile_picture_ref_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkeyHex = sse_decode_String(deserializer);
    var var_sha256Hex = sse_decode_String(deserializer);
    return ProfilePictureRefFfi(
      pubkeyHex: var_pubkeyHex,
      sha256Hex: var_sha256Hex,
    );
  }

  @protected
  ProfilePoolStatusFfi sse_decode_profile_pool_status_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_configured = sse_decode_u_32(deserializer);
    var var_excluded = sse_decode_u_32(deserializer);
    var var_usable = sse_decode_u_32(deserializer);
    var var_isUnderflow = sse_decode_bool(deserializer);
    return ProfilePoolStatusFfi(
      configured: var_configured,
      excluded: var_excluded,
      usable: var_usable,
      isUnderflow: var_isUnderflow,
    );
  }

  @protected
  ProfileSyncOutcomeFfi sse_decode_profile_sync_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return ProfileSyncOutcomeFfi.values[inner];
  }

  @protected
  ProfileSyncResultFfi sse_decode_profile_sync_result_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_outcome = sse_decode_profile_sync_outcome_ffi(deserializer);
    var var_relaysAcked = sse_decode_u_32(deserializer);
    var var_relaysAttempted = sse_decode_u_32(deserializer);
    var var_stillPending = sse_decode_bool(deserializer);
    return ProfileSyncResultFfi(
      outcome: var_outcome,
      relaysAcked: var_relaysAcked,
      relaysAttempted: var_relaysAttempted,
      stillPending: var_stillPending,
    );
  }

  @protected
  PublicIdentity sse_decode_public_identity(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_pubkeyHex = sse_decode_String(deserializer);
    var var_npub = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_i_64(deserializer);
    return PublicIdentity(
      pubkeyHex: var_pubkeyHex,
      npub: var_npub,
      createdAt: var_createdAt,
    );
  }

  @protected
  PublishResultFfi sse_decode_publish_result_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_eventId = sse_decode_String(deserializer);
    var var_acceptedBy = sse_decode_list_String(deserializer);
    var var_rejectedBy = sse_decode_list_relay_rejection_ffi(deserializer);
    var var_failed = sse_decode_list_String(deserializer);
    var var_isSuccess = sse_decode_bool(deserializer);
    return PublishResultFfi(
      eventId: var_eventId,
      acceptedBy: var_acceptedBy,
      rejectedBy: var_rejectedBy,
      failed: var_failed,
      isSuccess: var_isSuccess,
    );
  }

  @protected
  RelayConnectionStatusFfi sse_decode_relay_connection_status_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_url = sse_decode_String(deserializer);
    var var_status = sse_decode_String(deserializer);
    var var_lastSeen = sse_decode_opt_box_autoadd_i_64(deserializer);
    return RelayConnectionStatusFfi(
      url: var_url,
      status: var_status,
      lastSeen: var_lastSeen,
    );
  }

  @protected
  RelayEventCheckFfi sse_decode_relay_event_check_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_relayUrl = sse_decode_String(deserializer);
    var var_found = sse_decode_bool(deserializer);
    var var_eventCount = sse_decode_u_32(deserializer);
    var var_newestTimestamp = sse_decode_opt_box_autoadd_i_64(deserializer);
    return RelayEventCheckFfi(
      relayUrl: var_relayUrl,
      found: var_found,
      eventCount: var_eventCount,
      newestTimestamp: var_newestTimestamp,
    );
  }

  @protected
  RelayGiftWrapFetchFfi sse_decode_relay_gift_wrap_fetch_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_relayUrl = sse_decode_String(deserializer);
    var var_responded = sse_decode_bool(deserializer);
    var var_events = sse_decode_list_String(deserializer);
    return RelayGiftWrapFetchFfi(
      relayUrl: var_relayUrl,
      responded: var_responded,
      events: var_events,
    );
  }

  @protected
  RelayListActionFfi sse_decode_relay_list_action_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return RelayListActionFfi.values[inner];
  }

  @protected
  RelayListCategoryOutcomeFfi sse_decode_relay_list_category_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_action = sse_decode_relay_list_action_ffi(deserializer);
    var var_respondersProbed = sse_decode_u_32(deserializer);
    var var_relaysHealed = sse_decode_u_32(deserializer);
    var var_relayErrors = sse_decode_u_32(deserializer);
    return RelayListCategoryOutcomeFfi(
      action: var_action,
      respondersProbed: var_respondersProbed,
      relaysHealed: var_relaysHealed,
      relayErrors: var_relayErrors,
    );
  }

  @protected
  RelayListMaintenanceOutcomeFfi sse_decode_relay_list_maintenance_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_inbox = sse_decode_relay_list_category_outcome_ffi(deserializer);
    var var_keyPackage = sse_decode_relay_list_category_outcome_ffi(
      deserializer,
    );
    return RelayListMaintenanceOutcomeFfi(
      inbox: var_inbox,
      keyPackage: var_keyPackage,
    );
  }

  @protected
  RelayRejectionFfi sse_decode_relay_rejection_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_url = sse_decode_String(deserializer);
    var var_reason = sse_decode_String(deserializer);
    return RelayRejectionFfi(url: var_url, reason: var_reason);
  }

  @protected
  RelayTypeFfi sse_decode_relay_type_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return RelayTypeFfi.values[inner];
  }

  @protected
  RepairRotationOutcomeFfi sse_decode_repair_rotation_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_rotated = sse_decode_opt_box_autoadd_commit_to_publish_ffi(
      deserializer,
    );
    var var_skipped = sse_decode_opt_box_autoadd_skip_reason_ffi(deserializer);
    var var_deferredSend = sse_decode_opt_box_autoadd_deferred_send_ffi(
      deserializer,
    );
    return RepairRotationOutcomeFfi(
      rotated: var_rotated,
      skipped: var_skipped,
      deferredSend: var_deferredSend,
    );
  }

  @protected
  SignedLocationEventFfi sse_decode_signed_location_event_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_id = sse_decode_String(deserializer);
    var var_pubkey = sse_decode_String(deserializer);
    var var_createdAt = sse_decode_i_64(deserializer);
    var var_kind = sse_decode_u_16(deserializer);
    var var_tags = sse_decode_list_list_String(deserializer);
    var var_content = sse_decode_String(deserializer);
    var var_sig = sse_decode_String(deserializer);
    return SignedLocationEventFfi(
      id: var_id,
      pubkey: var_pubkey,
      createdAt: var_createdAt,
      kind: var_kind,
      tags: var_tags,
      content: var_content,
      sig: var_sig,
    );
  }

  @protected
  SkipReasonFfi sse_decode_skip_reason_ffi(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return SkipReasonFfi.values[inner];
  }

  @protected
  SubscriptionHealthActionFfi sse_decode_subscription_health_action_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var inner = sse_decode_i_32(deserializer);
    return SubscriptionHealthActionFfi.values[inner];
  }

  @protected
  SubscriptionHealthOutcomeFfi sse_decode_subscription_health_outcome_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_action = sse_decode_subscription_health_action_ffi(deserializer);
    var var_relaysTotal = sse_decode_u_32(deserializer);
    var var_relaysStillConnecting = sse_decode_u_32(deserializer);
    var var_relaysDisconnected = sse_decode_u_32(deserializer);
    var var_subscriptionsExpected = sse_decode_u_32(deserializer);
    var var_subscriptionsLive = sse_decode_u_32(deserializer);
    var var_subscriptionsSilent = sse_decode_u_32(deserializer);
    return SubscriptionHealthOutcomeFfi(
      action: var_action,
      relaysTotal: var_relaysTotal,
      relaysStillConnecting: var_relaysStillConnecting,
      relaysDisconnected: var_relaysDisconnected,
      subscriptionsExpected: var_subscriptionsExpected,
      subscriptionsLive: var_subscriptionsLive,
      subscriptionsSilent: var_subscriptionsSilent,
    );
  }

  @protected
  TileCacheEntryFfi sse_decode_tile_cache_entry_ffi(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_bytes = sse_decode_list_prim_u_8_strict(deserializer);
    var var_staleAtMs = sse_decode_i_64(deserializer);
    var var_lastModifiedMs = sse_decode_opt_box_autoadd_i_64(deserializer);
    var var_etag = sse_decode_opt_String(deserializer);
    return TileCacheEntryFfi(
      bytes: var_bytes,
      staleAtMs: var_staleAtMs,
      lastModifiedMs: var_lastModifiedMs,
      etag: var_etag,
    );
  }

  @protected
  int sse_decode_u_16(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint16();
  }

  @protected
  int sse_decode_u_32(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint32();
  }

  @protected
  BigInt sse_decode_u_64(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getBigUint64();
  }

  @protected
  int sse_decode_u_8(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getUint8();
  }

  @protected
  void sse_decode_unit(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  BigInt sse_decode_usize(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    return deserializer.buffer.getBigUint64();
  }

  @protected
  void sse_encode_AnyhowException(
    AnyhowException self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.message, serializer);
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    CircleManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as CircleManagerFfiImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    HavenCore self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as HavenCoreImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    LiveSyncFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LiveSyncFfiImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    LocationEventService self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationEventServiceImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    LocationMessage self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationMessageImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    LocationSettings self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationSettingsImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    NostrIdentityManager self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as NostrIdentityManagerImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Owned_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    RelayManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as RelayManagerFfiImpl).frbInternalSseEncode(move: true),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_RefMut_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    HavenCore self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as HavenCoreImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    CircleManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as CircleManagerFfiImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    HavenCore self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as HavenCoreImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    LiveSyncFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LiveSyncFfiImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    LocationEventService self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationEventServiceImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    LocationMessage self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationMessageImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    LocationSettings self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationSettingsImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    NostrIdentityManager self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as NostrIdentityManagerImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_Auto_Ref_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    RelayManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as RelayManagerFfiImpl).frbInternalSseEncode(move: false),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerCircleManagerFfi(
    CircleManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as CircleManagerFfiImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerHavenCore(
    HavenCore self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as HavenCoreImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLiveSyncFfi(
    LiveSyncFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LiveSyncFfiImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationEventService(
    LocationEventService self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationEventServiceImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationMessage(
    LocationMessage self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationMessageImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerLocationSettings(
    LocationSettings self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as LocationSettingsImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerNostrIdentityManager(
    NostrIdentityManager self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as NostrIdentityManagerImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void
  sse_encode_RustOpaque_flutter_rust_bridgefor_generatedRustAutoOpaqueInnerRelayManagerFfi(
    RelayManagerFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_usize(
      (self as RelayManagerFfiImpl).frbInternalSseEncode(move: null),
      serializer,
    );
  }

  @protected
  void sse_encode_StreamSink_ffi_relay_event_Sse(
    RustStreamSink<FfiRelayEvent> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(
      self.setupAndSerialize(
        codec: SseCodec(
          decodeSuccessData: sse_decode_ffi_relay_event,
          decodeErrorData: sse_decode_AnyhowException,
        ),
      ),
      serializer,
    );
  }

  @protected
  void sse_encode_String(String self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(utf8.encoder.convert(self), serializer);
  }

  @protected
  void sse_encode_add_members_result_ffi(
    AddMembersResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.commitEventJson, serializer);
    sse_encode_list_gift_wrapped_welcome_ffi(self.welcomeEvents, serializer);
    sse_encode_pending_state_ref_ffi(self.pending, serializer);
  }

  @protected
  void sse_encode_backlog_outcome_ffi(
    BacklogOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_bool(bool self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self ? 1 : 0);
  }

  @protected
  void sse_encode_box_autoadd_circle_with_members_ffi(
    CircleWithMembersFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_circle_with_members_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_commit_to_publish_ffi(
    CommitToPublishFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_commit_to_publish_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_contact_ffi(
    ContactFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_contact_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_decrypted_location_ffi(
    DecryptedLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_decrypted_location_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_deferred_send_ffi(
    DeferredSendFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_deferred_send_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_encrypted_location_ffi(
    EncryptedLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_encrypted_location_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_ffi_group_spec(
    FfiGroupSpec self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_ffi_group_spec(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_ffi_sync_status_reason(
    FfiSyncStatusReason self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_ffi_sync_status_reason(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_i_64(
    PlatformInt64 self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_64(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_invitation_ffi(
    InvitationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_invitation_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_last_known_location_ffi(
    LastKnownLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_last_known_location_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_member_key_package_ffi(
    MemberKeyPackageFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_member_key_package_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_pending_state_ref_ffi(
    PendingStateRefFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_pending_state_ref_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_profile_metadata_ffi(
    ProfileMetadataFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_profile_metadata_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_public_identity(
    PublicIdentity self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_public_identity(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_signed_location_event_ffi(
    SignedLocationEventFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_signed_location_event_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_skip_reason_ffi(
    SkipReasonFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_skip_reason_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_tile_cache_entry_ffi(
    TileCacheEntryFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_tile_cache_entry_ffi(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_u_16(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_16(self, serializer);
  }

  @protected
  void sse_encode_box_autoadd_u_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self, serializer);
  }

  @protected
  void sse_encode_built_relay_list_event_ffi(
    BuiltRelayListEventFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.eventJson, serializer);
    sse_encode_opt_String(self.eventIdHex, serializer);
    sse_encode_list_String(self.targets, serializer);
    sse_encode_opt_box_autoadd_u_16(self.kind, serializer);
    sse_encode_opt_box_autoadd_i_64(self.createdAtSecs, serializer);
    sse_encode_bool(self.suppressed, serializer);
  }

  @protected
  void sse_encode_built_unpublish_ffi(
    BuiltUnpublishFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_String(self.replacementEventJson, serializer);
    sse_encode_opt_String(self.deletionEventJson, serializer);
    sse_encode_list_String(self.targets, serializer);
    sse_encode_bool(self.suppressed, serializer);
  }

  @protected
  void sse_encode_catchup_result_ffi(
    CatchupResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.circlesSwept, serializer);
    sse_encode_u_32(self.eventsApplied, serializer);
    sse_encode_u_32(self.eventsDeferred, serializer);
    sse_encode_u_32(self.cursorsAdvanced, serializer);
    sse_encode_bool(self.deadlineHit, serializer);
    sse_encode_u_32(self.relayErrors, serializer);
  }

  @protected
  void sse_encode_circle_creation_result_ffi(
    CircleCreationResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_circle_ffi(self.circle, serializer);
    sse_encode_list_gift_wrapped_welcome_ffi(self.welcomeEvents, serializer);
    sse_encode_pending_state_ref_ffi(self.pending, serializer);
  }

  @protected
  void sse_encode_circle_ffi(CircleFfi self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.mlsGroupId, serializer);
    sse_encode_list_prim_u_8_strict(self.nostrGroupId, serializer);
    sse_encode_String(self.displayName, serializer);
    sse_encode_String(self.circleType, serializer);
    sse_encode_list_String(self.relays, serializer);
    sse_encode_i_64(self.createdAt, serializer);
    sse_encode_i_64(self.updatedAt, serializer);
  }

  @protected
  void sse_encode_circle_health_ffi(
    CircleHealthFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_box_autoadd_i_64(self.lastPublishAckedAtMs, serializer);
    sse_encode_opt_box_autoadd_i_64(self.lastPeerEventAtMs, serializer);
  }

  @protected
  void sse_encode_circle_member_ffi(
    CircleMemberFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkey, serializer);
    sse_encode_String(self.npub, serializer);
    sse_encode_opt_String(self.displayName, serializer);
    sse_encode_bool(self.isAdmin, serializer);
  }

  @protected
  void sse_encode_circle_with_members_ffi(
    CircleWithMembersFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_circle_ffi(self.circle, serializer);
    sse_encode_String(self.membershipStatus, serializer);
    sse_encode_opt_String(self.inviterPubkey, serializer);
    sse_encode_list_circle_member_ffi(self.members, serializer);
  }

  @protected
  void sse_encode_commit_to_publish_ffi(
    CommitToPublishFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.commitEventJson, serializer);
    sse_encode_pending_state_ref_ffi(self.pending, serializer);
  }

  @protected
  void sse_encode_contact_ffi(ContactFfi self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkey, serializer);
    sse_encode_opt_String(self.displayName, serializer);
    sse_encode_opt_String(self.notes, serializer);
    sse_encode_i_64(self.createdAt, serializer);
    sse_encode_i_64(self.updatedAt, serializer);
  }

  @protected
  void sse_encode_convergence_sweep_ffi(
    ConvergenceSweepFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.disposedMessages, serializer);
    sse_encode_u_32(self.discardedIntents, serializer);
    sse_encode_u_32(self.gatingRows, serializer);
    sse_encode_bool(self.settled, serializer);
  }

  @protected
  void sse_encode_decrypt_location_outcome_ffi(
    DecryptLocationOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_location_message_result_ffi(self.results, serializer);
    sse_encode_list_commit_to_publish_ffi(self.autoCommits, serializer);
  }

  @protected
  void sse_encode_decrypted_location_ffi(
    DecryptedLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.senderPubkey, serializer);
    sse_encode_f_64(self.latitude, serializer);
    sse_encode_f_64(self.longitude, serializer);
    sse_encode_String(self.geohash, serializer);
    sse_encode_i_64(self.timestamp, serializer);
    sse_encode_i_64(self.expiresAt, serializer);
  }

  @protected
  void sse_encode_deferred_send_ffi(
    DeferredSendFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.unresolvedInputs, serializer);
    sse_encode_u_32(self.discardedIntents, serializer);
    sse_encode_bool(self.repaired, serializer);
    sse_encode_list_commit_to_publish_ffi(self.commits, serializer);
    sse_encode_list_String(self.proposals, serializer);
  }

  @protected
  void sse_encode_directory_entry_ffi(
    DirectoryEntryFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkeyHex, serializer);
    sse_encode_String(self.npub, serializer);
    sse_encode_directory_tier_ffi(self.tier, serializer);
  }

  @protected
  void sse_encode_directory_tier_ffi(
    DirectoryTierFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_encrypt_location_outcome_ffi(
    EncryptLocationOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_box_autoadd_encrypted_location_ffi(self.sent, serializer);
    sse_encode_opt_box_autoadd_deferred_send_ffi(self.deferredSend, serializer);
  }

  @protected
  void sse_encode_encrypted_location_ffi(
    EncryptedLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.eventJson, serializer);
    sse_encode_list_prim_u_8_strict(self.nostrGroupId, serializer);
    sse_encode_list_String(self.relays, serializer);
  }

  @protected
  void sse_encode_f_64(double self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putFloat64(self);
  }

  @protected
  void sse_encode_ffi_group_spec(FfiGroupSpec self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.nostrGroupId, serializer);
    sse_encode_list_String(self.relays, serializer);
  }

  @protected
  void sse_encode_ffi_relay_event(
    FfiRelayEvent self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_ffi_relay_event_kind(self.kind, serializer);
    sse_encode_opt_list_prim_u_8_strict(self.nostrGroupId, serializer);
    sse_encode_opt_String(self.senderPubkey, serializer);
    sse_encode_opt_String(self.content, serializer);
    sse_encode_opt_box_autoadd_i_64(self.eventCreatedAtSecs, serializer);
    sse_encode_opt_String(self.evolutionEventJson, serializer);
    sse_encode_opt_String(self.giftWrapJson, serializer);
    sse_encode_opt_box_autoadd_ffi_sync_status_reason(
      self.statusReason,
      serializer,
    );
    sse_encode_opt_list_prim_u_8_strict(
      self.unrecoverableNostrGroupId,
      serializer,
    );
  }

  @protected
  void sse_encode_ffi_relay_event_kind(
    FfiRelayEventKind self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_ffi_sync_status_reason(
    FfiSyncStatusReason self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_force_release_outcome_ffi(
    ForceReleaseOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_gift_wrapped_welcome_ffi(
    GiftWrappedWelcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.recipientPubkey, serializer);
    sse_encode_list_String(self.recipientRelays, serializer);
    sse_encode_String(self.eventJson, serializer);
  }

  @protected
  void sse_encode_i_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putInt32(self);
  }

  @protected
  void sse_encode_i_64(PlatformInt64 self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putPlatformInt64(self);
  }

  @protected
  void sse_encode_invitation_ffi(InvitationFfi self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.mlsGroupId, serializer);
    sse_encode_String(self.circleName, serializer);
    sse_encode_String(self.inviterPubkey, serializer);
    sse_encode_String(self.inviterNpub, serializer);
    sse_encode_i_64(self.invitedAt, serializer);
  }

  @protected
  void sse_encode_kp_maintenance_action_ffi(
    KpMaintenanceActionFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_kp_maintenance_outcome_ffi(
    KpMaintenanceOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_kp_maintenance_action_ffi(self.action, serializer);
    sse_encode_u_32(self.canonicalOnRelays, serializer);
    sse_encode_u_32(self.respondersProbed, serializer);
    sse_encode_u_32(self.relaysTargeted, serializer);
    sse_encode_u_32(self.relaysHealed, serializer);
    sse_encode_u_32(self.relayErrors, serializer);
    sse_encode_bool(self.expiredInitKeyPurged, serializer);
    sse_encode_bool(self.retiredMalformedSlot, serializer);
  }

  @protected
  void sse_encode_last_known_location_ffi(
    LastKnownLocationFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.nostrGroupId, serializer);
    sse_encode_String(self.senderPubkey, serializer);
    sse_encode_f_64(self.latitude, serializer);
    sse_encode_f_64(self.longitude, serializer);
    sse_encode_String(self.geohash, serializer);
    sse_encode_opt_String(self.displayName, serializer);
    sse_encode_i_64(self.timestamp, serializer);
    sse_encode_i_64(self.expiresAt, serializer);
    sse_encode_i_64(self.purgeAfter, serializer);
    sse_encode_i_64(self.updatedAt, serializer);
  }

  @protected
  void sse_encode_leave_plan_ffi(LeavePlanFfi self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_leave_plan_kind_ffi(self.kind, serializer);
    sse_encode_opt_String(self.successorHex, serializer);
  }

  @protected
  void sse_encode_leave_plan_kind_ffi(
    LeavePlanKindFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_legacy_retraction_outcome_ffi(
    LegacyRetractionOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_bool(self.alreadyDone, serializer);
    sse_encode_u_32(self.legacy443Scrubbed, serializer);
    sse_encode_bool(self.relayListRetracted, serializer);
    sse_encode_u_32(self.relayErrors, serializer);
  }

  @protected
  void sse_encode_list_String(List<String> self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_String(item, serializer);
    }
  }

  @protected
  void sse_encode_list_circle_member_ffi(
    List<CircleMemberFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_circle_member_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_circle_with_members_ffi(
    List<CircleWithMembersFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_circle_with_members_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_commit_to_publish_ffi(
    List<CommitToPublishFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_commit_to_publish_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_contact_ffi(
    List<ContactFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_contact_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_directory_entry_ffi(
    List<DirectoryEntryFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_directory_entry_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_ffi_group_spec(
    List<FfiGroupSpec> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_ffi_group_spec(item, serializer);
    }
  }

  @protected
  void sse_encode_list_gift_wrapped_welcome_ffi(
    List<GiftWrappedWelcomeFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_gift_wrapped_welcome_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_invitation_ffi(
    List<InvitationFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_invitation_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_last_known_location_ffi(
    List<LastKnownLocationFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_last_known_location_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_list_String(
    List<List<String>> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_list_String(item, serializer);
    }
  }

  @protected
  void sse_encode_list_location_message_result_ffi(
    List<LocationMessageResultFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_location_message_result_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_member_key_package_ffi(
    List<MemberKeyPackageFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_member_key_package_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_prim_u_8_loose(
    List<int> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint8List(
      self is Uint8List ? self : Uint8List.fromList(self),
    );
  }

  @protected
  void sse_encode_list_prim_u_8_strict(
    Uint8List self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    serializer.buffer.putUint8List(self);
  }

  @protected
  void sse_encode_list_profile_metadata_ffi(
    List<ProfileMetadataFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_profile_metadata_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_relay_connection_status_ffi(
    List<RelayConnectionStatusFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_relay_connection_status_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_relay_gift_wrap_fetch_ffi(
    List<RelayGiftWrapFetchFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_relay_gift_wrap_fetch_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_list_relay_rejection_ffi(
    List<RelayRejectionFfi> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_relay_rejection_ffi(item, serializer);
    }
  }

  @protected
  void sse_encode_location_message_result_ffi(
    LocationMessageResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_location_message_result_kind_ffi(self.kind, serializer);
    sse_encode_opt_box_autoadd_decrypted_location_ffi(
      self.location,
      serializer,
    );
    sse_encode_list_prim_u_8_strict(self.mlsGroupId, serializer);
    sse_encode_u_64(self.epoch, serializer);
  }

  @protected
  void sse_encode_location_message_result_kind_ffi(
    LocationMessageResultKindFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_log_alias_class_ffi(
    LogAliasClassFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_member_key_package_ffi(
    MemberKeyPackageFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.keyPackageJson, serializer);
    sse_encode_list_String(self.inboxRelays, serializer);
    sse_encode_list_String(self.nip65Relays, serializer);
  }

  @protected
  void sse_encode_opt_String(String? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_String(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_circle_with_members_ffi(
    CircleWithMembersFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_circle_with_members_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_commit_to_publish_ffi(
    CommitToPublishFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_commit_to_publish_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_contact_ffi(
    ContactFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_contact_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_decrypted_location_ffi(
    DecryptedLocationFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_decrypted_location_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_deferred_send_ffi(
    DeferredSendFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_deferred_send_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_encrypted_location_ffi(
    EncryptedLocationFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_encrypted_location_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_ffi_sync_status_reason(
    FfiSyncStatusReason? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_ffi_sync_status_reason(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_i_64(
    PlatformInt64? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_i_64(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_invitation_ffi(
    InvitationFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_invitation_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_member_key_package_ffi(
    MemberKeyPackageFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_member_key_package_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_profile_metadata_ffi(
    ProfileMetadataFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_profile_metadata_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_public_identity(
    PublicIdentity? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_public_identity(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_skip_reason_ffi(
    SkipReasonFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_skip_reason_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_tile_cache_entry_ffi(
    TileCacheEntryFfi? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_tile_cache_entry_ffi(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_u_16(int? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_u_16(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_box_autoadd_u_32(int? self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_box_autoadd_u_32(self, serializer);
    }
  }

  @protected
  void sse_encode_opt_list_prim_u_8_strict(
    Uint8List? self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    sse_encode_bool(self != null, serializer);
    if (self != null) {
      sse_encode_list_prim_u_8_strict(self, serializer);
    }
  }

  @protected
  void sse_encode_pending_state_ref_ffi(
    PendingStateRefFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_64(self.token, serializer);
  }

  @protected
  void sse_encode_profile_metadata_ffi(
    ProfileMetadataFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkeyHex, serializer);
    sse_encode_String(self.npub, serializer);
    sse_encode_opt_String(self.displayName, serializer);
    sse_encode_opt_String(self.name, serializer);
    sse_encode_opt_String(self.about, serializer);
    sse_encode_bool(self.hasPicture, serializer);
    sse_encode_bool(self.isKnown, serializer);
    sse_encode_i_64(self.fetchedAt, serializer);
    sse_encode_opt_String(self.pictureSha256Hex, serializer);
  }

  @protected
  void sse_encode_profile_pending_state_ffi(
    ProfilePendingStateFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_bool(self.pending, serializer);
    sse_encode_bool(self.partial, serializer);
    sse_encode_bool(self.retryDue, serializer);
  }

  @protected
  void sse_encode_profile_picture_ref_ffi(
    ProfilePictureRefFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkeyHex, serializer);
    sse_encode_String(self.sha256Hex, serializer);
  }

  @protected
  void sse_encode_profile_pool_status_ffi(
    ProfilePoolStatusFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_32(self.configured, serializer);
    sse_encode_u_32(self.excluded, serializer);
    sse_encode_u_32(self.usable, serializer);
    sse_encode_bool(self.isUnderflow, serializer);
  }

  @protected
  void sse_encode_profile_sync_outcome_ffi(
    ProfileSyncOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_profile_sync_result_ffi(
    ProfileSyncResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_profile_sync_outcome_ffi(self.outcome, serializer);
    sse_encode_u_32(self.relaysAcked, serializer);
    sse_encode_u_32(self.relaysAttempted, serializer);
    sse_encode_bool(self.stillPending, serializer);
  }

  @protected
  void sse_encode_public_identity(
    PublicIdentity self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.pubkeyHex, serializer);
    sse_encode_String(self.npub, serializer);
    sse_encode_i_64(self.createdAt, serializer);
  }

  @protected
  void sse_encode_publish_result_ffi(
    PublishResultFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.eventId, serializer);
    sse_encode_list_String(self.acceptedBy, serializer);
    sse_encode_list_relay_rejection_ffi(self.rejectedBy, serializer);
    sse_encode_list_String(self.failed, serializer);
    sse_encode_bool(self.isSuccess, serializer);
  }

  @protected
  void sse_encode_relay_connection_status_ffi(
    RelayConnectionStatusFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.url, serializer);
    sse_encode_String(self.status, serializer);
    sse_encode_opt_box_autoadd_i_64(self.lastSeen, serializer);
  }

  @protected
  void sse_encode_relay_event_check_ffi(
    RelayEventCheckFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.relayUrl, serializer);
    sse_encode_bool(self.found, serializer);
    sse_encode_u_32(self.eventCount, serializer);
    sse_encode_opt_box_autoadd_i_64(self.newestTimestamp, serializer);
  }

  @protected
  void sse_encode_relay_gift_wrap_fetch_ffi(
    RelayGiftWrapFetchFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.relayUrl, serializer);
    sse_encode_bool(self.responded, serializer);
    sse_encode_list_String(self.events, serializer);
  }

  @protected
  void sse_encode_relay_list_action_ffi(
    RelayListActionFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_relay_list_category_outcome_ffi(
    RelayListCategoryOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_relay_list_action_ffi(self.action, serializer);
    sse_encode_u_32(self.respondersProbed, serializer);
    sse_encode_u_32(self.relaysHealed, serializer);
    sse_encode_u_32(self.relayErrors, serializer);
  }

  @protected
  void sse_encode_relay_list_maintenance_outcome_ffi(
    RelayListMaintenanceOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_relay_list_category_outcome_ffi(self.inbox, serializer);
    sse_encode_relay_list_category_outcome_ffi(self.keyPackage, serializer);
  }

  @protected
  void sse_encode_relay_rejection_ffi(
    RelayRejectionFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.url, serializer);
    sse_encode_String(self.reason, serializer);
  }

  @protected
  void sse_encode_relay_type_ffi(RelayTypeFfi self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_repair_rotation_outcome_ffi(
    RepairRotationOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_opt_box_autoadd_commit_to_publish_ffi(self.rotated, serializer);
    sse_encode_opt_box_autoadd_skip_reason_ffi(self.skipped, serializer);
    sse_encode_opt_box_autoadd_deferred_send_ffi(self.deferredSend, serializer);
  }

  @protected
  void sse_encode_signed_location_event_ffi(
    SignedLocationEventFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(self.id, serializer);
    sse_encode_String(self.pubkey, serializer);
    sse_encode_i_64(self.createdAt, serializer);
    sse_encode_u_16(self.kind, serializer);
    sse_encode_list_list_String(self.tags, serializer);
    sse_encode_String(self.content, serializer);
    sse_encode_String(self.sig, serializer);
  }

  @protected
  void sse_encode_skip_reason_ffi(
    SkipReasonFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_subscription_health_action_ffi(
    SubscriptionHealthActionFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.index, serializer);
  }

  @protected
  void sse_encode_subscription_health_outcome_ffi(
    SubscriptionHealthOutcomeFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_subscription_health_action_ffi(self.action, serializer);
    sse_encode_u_32(self.relaysTotal, serializer);
    sse_encode_u_32(self.relaysStillConnecting, serializer);
    sse_encode_u_32(self.relaysDisconnected, serializer);
    sse_encode_u_32(self.subscriptionsExpected, serializer);
    sse_encode_u_32(self.subscriptionsLive, serializer);
    sse_encode_u_32(self.subscriptionsSilent, serializer);
  }

  @protected
  void sse_encode_tile_cache_entry_ffi(
    TileCacheEntryFfi self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_list_prim_u_8_strict(self.bytes, serializer);
    sse_encode_i_64(self.staleAtMs, serializer);
    sse_encode_opt_box_autoadd_i_64(self.lastModifiedMs, serializer);
    sse_encode_opt_String(self.etag, serializer);
  }

  @protected
  void sse_encode_u_16(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint16(self);
  }

  @protected
  void sse_encode_u_32(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint32(self);
  }

  @protected
  void sse_encode_u_64(BigInt self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putBigUint64(self);
  }

  @protected
  void sse_encode_u_8(int self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putUint8(self);
  }

  @protected
  void sse_encode_unit(void self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
  }

  @protected
  void sse_encode_usize(BigInt self, SseSerializer serializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    serializer.buffer.putBigUint64(self);
  }
}

@sealed
class CircleManagerFfiImpl extends RustOpaque implements CircleManagerFfi {
  // Not to be used by end users
  CircleManagerFfiImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  CircleManagerFfiImpl.frbInternalSseDecode(
    BigInt ptr,
    int externalSizeOnNative,
  ) : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_CircleManagerFfi,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_CircleManagerFfi,
    rustArcDecrementStrongCountPtr: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_CircleManagerFfiPtr,
  );

  /// Wipes local state for the `Abandon` plan — sole-member cleanup with
  /// no MLS commit and no relay publish. Refreshes the member directory for
  /// the same reason as [`complete_leave`](Self::complete_leave).
  Future<void> abandonCircleLocalOnly({required List<int> mlsGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiAbandonCircleLocalOnly(
        that: this,
        mlsGroupId: mlsGroupId,
      );

  /// Accepts an invitation to join a circle, keyed by the gift-wrap event id.
  ///
  /// Feeds the still-encrypted 1059 held for `gift_wrap_id` (the stand-in
  /// `mlsGroupId` bytes from [`process_gift_wrapped_invitation`]) to the
  /// engine, which peels + joins and yields the real circle.
  Future<CircleWithMembersFfi> acceptInvitation({
    required List<int> giftWrapId,
  }) => RustLib.instance.api.crateApiCircleManagerFfiAcceptInvitation(
    that: this,
    giftWrapId: giftWrapId,
  );

  /// Adds members to an existing circle and gift-wraps their Welcomes.
  ///
  /// The add-time counterpart to [`create_circle`]: stages an MLS Add commit
  /// (kind 445, advances existing members on finalize) and gift-wraps the
  /// resulting per-member Welcome rumors (kind 444) for delivery, resolving
  /// each recipient's relays through the same fail-closed cascade.
  ///
  /// The caller owns the publish/finalize cycle: publish
  /// `evolution_event_json` to the circle's relays, finalize the pending
  /// commit on success (or clear it on failure), then publish each
  /// gift-wrapped Welcome only after a successful finalize.
  ///
  /// # Arguments
  ///
  /// * `identity_secret_bytes` - The admin's 32-byte Nostr secret key.
  /// * `mls_group_id` - The circle's MLS group ID.
  /// * `members` - Key packages and inbox/NIP-65 relays for the new members.
  /// * `creator_fallback_relays` - The admin's own inbox relays (kind 10050),
  ///   used as the third tier in the Welcome delivery cascade (member 10050 →
  ///   member 10002 → admin inbox → FAIL CLOSED). Pass an empty list if the
  ///   admin has no inbox relays; delivery then fails closed (no
  ///   public-default fallback) when tiers 1–2 are also empty.
  ///
  /// # Security
  ///
  /// The Welcome events are gift-wrapped per NIP-59, hiding the sender's
  /// identity behind a fresh ephemeral key. The secret bytes are zeroized.
  ///
  /// # Errors
  ///
  /// Returns an error if the secret bytes are invalid, a key package fails to
  /// parse, the caller is not an admin, or a member has no reachable Welcome
  /// relay (fail-closed).
  Future<AddMembersResultFfi> addMembersToCircle({
    required List<int> identitySecretBytes,
    required List<int> mlsGroupId,
    required List<MemberKeyPackageFfi> members,
    required List<String> creatorFallbackRelays,
  }) => RustLib.instance.api.crateApiCircleManagerFfiAddMembersToCircle(
    that: this,
    identitySecretBytes: identitySecretBytes,
    mlsGroupId: mlsGroupId,
    members: members,
    creatorFallbackRelays: creatorFallbackRelays,
  );

  /// Adds a relay to one category (idempotent).
  ///
  /// The URL is normalized via `nostr::RelayUrl::parse`; duplicates are
  /// silent no-ops. `ws://` and credential-bearing URLs are rejected.
  Future<void> addUserRelay({
    required String url,
    required RelayTypeFfi relayType,
  }) => RustLib.instance.api.crateApiCircleManagerFfiAddUserRelay(
    that: this,
    url: url,
    relayType: relayType,
  );

  /// Atomically gates on the toggle, signs a kind 10050 / 10051 event,
  /// and resolves the publish targets.
  ///
  /// This is the **only** path through which Dart should publish a relay
  /// list: it ensures the toggle check, signing, and target resolution
  /// happen as one operation. When the toggle is off, returns
  /// `suppressed=true` and no event — Dart MUST NOT publish anything in
  /// that case.
  ///
  /// On success, after publishing the returned event JSON to the
  /// returned `targets`, Dart should call
  /// [`Self::record_published_relay_list`] so the unpublish path can
  /// later issue a NIP-09 deletion referencing the event id.
  Future<BuiltRelayListEventFfi> buildRelayListPublish({
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  }) => RustLib.instance.api.crateApiCircleManagerFfiBuildRelayListPublish(
    that: this,
    identitySecretBytes: identitySecretBytes,
    relayType: relayType,
  );

  /// Builds a best-effort NIP-09 deletion to scrub a removed relay's stale
  /// copy of the user's relay list.
  ///
  /// Two-plane removal hygiene: when the user removes relay(s) from a list
  /// type, the new (smaller) list is republished to the *kept* relays, but
  /// the *dropped* relays still hold the previous event — which may name a
  /// private relay the user is now trying to keep private. This builds a
  /// kind-5 deletion (referencing the last published event id) to publish
  /// to `dropped_relays` so cooperative relays drop that stale copy.
  ///
  /// No empty-replacement is produced: a replaceable empty event with a
  /// higher `created_at` than the corrected list could make indexers treat
  /// the user as having no relays (undiscoverable). The corrected list is
  /// reasserted globally by the kept-relay republish; this deletion only
  /// cleans up direct queriers of the dropped relays.
  ///
  /// Best-effort: relays that do not honor NIP-09 may retain the old event.
  /// MUST be called BEFORE republishing the new list, so the looked-up
  /// last-published event is the stale one being scrubbed. Returns
  /// `suppressed=true` with no event when nothing was ever published for
  /// this kind (the dropped relays never received our list).
  Future<BuiltUnpublishFfi> buildRelayRemovalScrub({
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
    required List<String> droppedRelays,
  }) => RustLib.instance.api.crateApiCircleManagerFfiBuildRelayRemovalScrub(
    that: this,
    identitySecretBytes: identitySecretBytes,
    relayType: relayType,
    droppedRelays: droppedRelays,
  );

  /// Builds the events needed to unpublish a relay list category.
  ///
  /// Produces (1) an empty-replacement event with `created_at` chosen to
  /// supersede the previous publication via Nostr's replaceable-event
  /// semantics, and (2) a best-effort NIP-09 (kind 5) deletion if a
  /// prior publication is on record.
  ///
  /// Dart should publish both events to the returned `targets` and then
  /// also flip the toggle off via `set_publish_relay_list`. This method
  /// itself does not change the toggle.
  Future<BuiltUnpublishFfi> buildUnpublishRelayList({
    required List<int> identitySecretBytes,
    required RelayTypeFfi relayType,
  }) => RustLib.instance.api.crateApiCircleManagerFfiBuildUnpublishRelayList(
    that: this,
    identitySecretBytes: identitySecretBytes,
    relayType: relayType,
  );

  /// Reads a circle's delivery-health timestamps.
  Future<CircleHealthFfi> circleHealth({required List<int> nostrGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiCircleHealth(
        that: this,
        nostrGroupId: nostrGroupId,
      );

  /// Removes the local circle row after a successful leave sequence, or
  /// for the `OrphanLocalOnly` plan.
  ///
  /// Also refreshes the member directory, so the departed circle's co-members
  /// stop reading as current members and start their retention window —
  /// deleting the circle row does not cascade to the directory.
  Future<void> completeLeave({required List<int> mlsGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiCompleteLeave(
        that: this,
        mlsGroupId: mlsGroupId,
      );

  /// Confirms a staged commit was published (≥1-relay OK-ack) so the engine
  /// applies it and advances the epoch.
  ///
  /// "Acked" MUST mean a relay returned OK — never merely "sent" — to avoid
  /// optimistic-merge forks (Rule 13, security F13). Pass the `pending` token
  /// carried in a [`CircleCreationResultFfi`] / [`AddMembersResultFfi`] /
  /// [`CommitToPublishFfi`].
  Future<void> confirmPublished({required PendingStateRefFfi pending}) =>
      RustLib.instance.api.crateApiCircleManagerFfiConfirmPublished(
        that: this,
        pending: pending,
      );

  /// Creates a new circle with gift-wrapped Welcome events.
  ///
  /// Returns the created circle and gift-wrapped Welcome events ready
  /// to publish to the invited members' inbox relays.
  ///
  /// # Arguments
  ///
  /// * `identity_secret_bytes` - The creator's identity secret bytes (32 bytes,
  ///   from `NostrIdentityManager.get_secret_bytes()`)
  /// * `members` - Key packages and inbox relays for each member
  /// * `name` - Circle name
  /// * `description` - Optional circle description
  /// * `circle_type` - Circle type: "location_sharing" or "direct_share"
  /// * `relays` - Relay URLs for the circle's messages
  /// * `creator_fallback_relays` - The creator's own inbox relays (kind
  ///   10050), used as the third tier in the Welcome delivery cascade
  ///   (member 10050 → member 10002 → creator inbox → FAIL CLOSED). Pass an
  ///   empty list if the creator has no inbox relays; delivery then fails
  ///   closed (no public-default fallback) when tiers 1–2 are also empty.
  ///
  /// # Security
  ///
  /// The Welcome events are gift-wrapped per NIP-59, hiding the sender's
  /// identity behind an ephemeral key. Each Welcome uses a fresh ephemeral
  /// keypair and randomized timestamp.
  Future<CircleCreationResultFfi> createCircle({
    required List<int> identitySecretBytes,
    required List<MemberKeyPackageFfi> members,
    required String name,
    String? description,
    required String circleType,
    required List<String> relays,
    required List<String> creatorFallbackRelays,
  }) => RustLib.instance.api.crateApiCircleManagerFfiCreateCircle(
    that: this,
    identitySecretBytes: identitySecretBytes,
    members: members,
    name: name,
    description: description,
    circleType: circleType,
    relays: relays,
    creatorFallbackRelays: creatorFallbackRelays,
  );

  /// Reads the persisted relay sync cursor (raw ms) for `stream`.
  ///
  /// Returns `None` when the stream has never been seeded. Seeding — like
  /// every other operation that can RAISE a cursor — happens in haven-core,
  /// on the receive plane that is about to issue the REQ; this boundary is
  /// read-only on that axis. See [`haven_core::relay::cursor`] for stream
  /// keys and semantics.
  ///
  /// # Errors
  ///
  /// Returns an error string if the storage read fails.
  ///
  /// Storage errors here can only reference the constant `stream` key, the
  /// `sync_cursors` table, or an `i64`, never secret material — but they are
  /// still re-redacted via `redact_hex_sequences` so every error crossing
  /// this FFI boundary honors the same no-raw-hex invariant uniformly.
  Future<PlatformInt64?> cursorGet({required String stream}) => RustLib
      .instance
      .api
      .crateApiCircleManagerFfiCursorGet(that: this, stream: stream);

  /// Resets `stream`'s cursor to the unseeded state (wipe-on-logout).
  ///
  /// # Errors
  ///
  /// Returns a redacted error string if the storage write fails.
  Future<void> cursorReset({required String stream}) => RustLib.instance.api
      .crateApiCircleManagerFfiCursorReset(that: this, stream: stream);

  /// Declines an invitation, keyed by the gift-wrap event id.
  ///
  /// Drops the held 1059 locally (never ingested → nothing on the wire,
  /// Rule 10) and marks the gift wrap resolved so a re-poll never re-surfaces
  /// it. (Storage-only; sync in the core.)
  Future<void> declineInvitation({required List<int> giftWrapId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiDeclineInvitation(
        that: this,
        giftWrapId: giftWrapId,
      );

  /// Decrypts / ingests a received `kind:445` event, returning the folded
  /// engine results (Dark Matter five-variant taxonomy).
  ///
  /// A single ingest can yield SEVERAL [`LocationMessageResultFfi`] — the
  /// engine's `advance_convergence` may release buffered inbound after the
  /// outer event — so this returns a `Vec`. The engine owns stale / duplicate
  /// / out-of-order handling internally (a future-epoch event is durably
  /// buffered and re-surfaced once the gap fills), so there is no
  /// `Unprocessable` / `PreviouslyFailed` outcome anymore.
  ///
  /// # Peer `SelfRemove` eviction (auto-commit) — Rule 13
  ///
  /// This variant does NOT surface receive-side auto-commits (a peer
  /// `SelfRemove` eviction the engine staged): to stay Rule-13-safe the core
  /// rolls back any that surfaced rather than confirm an unpublished commit.
  /// The eviction still propagates via the background catch-up sweep (which
  /// publishes it). Callers that own relay publishing in poll mode SHOULD
  /// migrate to [`Self::decrypt_location_collecting_commits`], which surfaces the
  /// auto-commit so the caller can publish-then-confirm it in the foreground.
  ///
  /// # Cursor contract (DM-4b)
  ///
  /// The engine owns retry/buffering, so the caller advances its relay sync
  /// cursor on the OUTER event's `created_at` (which the Dart caller already
  /// holds — it passed `event_json` in). A buffered event is re-delivered by
  /// the engine when the epoch gap fills; the caller never re-fetches it.
  ///
  /// # Concurrency
  ///
  /// The session is `&mut`-serialized behind a `tokio` mutex, so concurrent
  /// calls are serialized by the engine; this awaits directly.
  ///
  /// # Arguments
  ///
  /// * `event_json` - JSON-serialized `kind:445` event.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string if the event JSON is invalid or the
  /// engine ingest fails hard.
  Future<List<LocationMessageResultFfi>> decryptLocation({
    required String eventJson,
  }) => RustLib.instance.api.crateApiCircleManagerFfiDecryptLocation(
    that: this,
    eventJson: eventJson,
  );

  /// Decrypts / ingests a received `kind:445`, returning the folded results AND
  /// any receive-side auto-commit the engine staged (publish-before-apply).
  ///
  /// Identical ingest to [`Self::decrypt_location`], but surfaces a peer
  /// `SelfRemove` eviction the engine auto-committed instead of dropping it.
  /// Rule 13 / security F13: for EACH
  /// [`DecryptLocationOutcomeFfi::auto_commits`] entry, publish
  /// `commit_event_json` to the circle's relays, then
  /// [`confirm_published`](Self::confirm_published) on a ≥1-relay ACK (or
  /// [`publish_failed`](Self::publish_failed) on failure) — exactly like the
  /// [`CommitToPublishFfi`] from remove / relay-update. Confirming before an
  /// ACK, or dropping an entry, re-forks the group the leaver departed. The
  /// foreground poll receive path SHOULD call this in place of
  /// [`Self::decrypt_location`].
  ///
  /// The cursor contract of [`Self::decrypt_location`] applies unchanged.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string if the event JSON is invalid or the
  /// engine ingest fails hard.
  Future<DecryptLocationOutcomeFfi> decryptLocationCollectingCommits({
    required String eventJson,
  }) => RustLib.instance.api
      .crateApiCircleManagerFfiDecryptLocationCollectingCommits(
        that: this,
        eventJson: eventJson,
      );

  /// Deletes a contact.
  Future<void> deleteContact({required String pubkey}) => RustLib.instance.api
      .crateApiCircleManagerFfiDeleteContact(that: this, pubkey: pubkey);

  /// Deletes the local user's OWN public profile (best-effort, plan D10).
  ///
  /// A **no-op unless a profile was published** (`has_published_profile`).
  /// Otherwise it republishes a blank kind-0, best-effort NIP-09-deletes the
  /// last published kind-0, and clears the local profile cache. The Blossom
  /// blob DELETE is deferred (no delete helper in the profile module;
  /// documented best-effort).
  ///
  /// Either way the own-profile outbox is emptied: anything still queued was
  /// going to publish the profile the user is deleting. Nothing else about
  /// the no-op branch changes — a pubkey that never published keeps its local
  /// rows.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on a secret that is not this account's
  /// identity, or on relay or database failure.
  Future<void> deleteMyPublicProfile({
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiCircleManagerFfiDeleteMyPublicProfile(
    that: this,
    identitySecretBytes: identitySecretBytes,
  );

  /// Reconciles a member's cached profile-picture bytes with their current
  /// kind-0 `picture` URL (authoritative; never driven by Dart).
  ///
  /// Reads the CURRENT `picture` URL from the cached kind-0 and the URL stored
  /// with any cached bytes, then (bug HIGH-2):
  /// * **URL present and changed / no bytes yet** → download + overwrite,
  ///   recording the new URL;
  /// * **URL absent/cleared but stale bytes cached** → delete the stale row so
  ///   a removed avatar stops rendering;
  /// * **URL unchanged** → no-op.
  ///
  /// This makes the Dart gate `if (!hasPicture) downloadMemberPicture(..)`
  /// correct for every transition: a changed URL → `has_picture` false →
  /// re-download; a removed URL → `has_picture` false → this call clears the
  /// row.
  ///
  /// The download applies the anti-SSRF connect-time IP filter and re-encodes
  /// (plan §4 blossom.rs); bytes are cached in SQLCipher; the URL never crosses
  /// the FFI (plan D2). Because the URL is read from the cached kind-0, an
  /// attacker-supplied Dart string can never drive the download target.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on download or database failure.
  Future<void> downloadMemberPicture({required String pubkeyHex}) =>
      RustLib.instance.api.crateApiCircleManagerFfiDownloadMemberPicture(
        that: this,
        pubkeyHex: pubkeyHex,
      );

  /// Reconciles a BATCH of members' profile pictures on a shuffled, paced
  /// schedule, and returns the pubkeys that were reconciled.
  ///
  /// # Why this exists
  ///
  /// [`Self::download_member_picture`] is correct per member but wrong per
  /// roster: looping it from Dart fires the whole set back-to-back at one
  /// Blossom host — frequently run by the same operator as a default circle
  /// relay — so the burst is directly readable as "these N people are in this
  /// user's circles", in roster order, at a moment that correlates with the
  /// app coming to the foreground. The single-author kind-0 fetch would have
  /// been pointless if the avatar fetch handed the same set over anyway.
  ///
  /// So this call:
  /// * **shuffles** the order every cycle from the OS CSPRNG (`rand::OsRng`;
  ///   `thread_rng` is banned), so arrival order carries no roster ordering
  ///   and a newly added member does not appear at a stable position;
  /// * runs the downloads **one at a time**, never concurrently, so there is
  ///   no burst to fingerprint;
  /// * sleeps a **CSPRNG-sampled** [`profile_picture_delay`] between requests,
  ///   so the remaining inter-arrival pattern carries no exploitable
  ///   regularity;
  /// * bounds the whole batch by [`PROFILE_PICTURE_BATCH_DEADLINE`], and each
  ///   individual download by whatever is left of it.
  ///
  /// Per-member failures (timeout, dead host, oversized blob) are absorbed:
  /// one member's picture must not deny every other member theirs. Members
  /// not reached inside the deadline are simply omitted from the result and
  /// retried on the next refresh — nothing is recorded against them.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string only on a failure that affects the whole
  /// batch; individual download failures are absorbed.
  Future<List<String>> downloadMemberPictures({
    required List<String> pubkeysHex,
  }) => RustLib.instance.api.crateApiCircleManagerFfiDownloadMemberPictures(
    that: this,
    pubkeysHex: pubkeysHex,
  );

  /// Encrypts a location for a circle.
  ///
  /// Creates an MLS-encrypted kind 445 event containing the location data.
  /// The returned event is ready to publish to the circle's relays.
  ///
  /// # Concurrency
  ///
  /// Concurrent calls for the same group are **safe**. Every engine write
  /// funnels through one `tokio::sync::Mutex<AccountDeviceSession>`
  /// (`haven-core/src/nostr/mls/manager.rs:99-101`, taken in `send` at
  /// `:494-501`), which `encrypt_location` reaches via `send_location` →
  /// `create_message` → `send`. Application messages advance no epoch and
  /// carry no `PendingStateRef`, and past-epoch app messages are tolerated to
  /// `app_message_past_epoch_limit`, so the encrypt→publish gap is covered
  /// too.
  ///
  /// This paragraph previously said the opposite — that `create_message`
  /// races on the epoch counter and callers "must not" invoke it
  /// concurrently, naming a `locationPublisherProvider` that no longer owns
  /// publishing. That described PRE-Dark-Matter MDK, before the session
  /// mutex existed. It is corrected rather than deleted because acting on it
  /// is expensive in a specific way: it invites building a cross-isolate lock
  /// for a problem upstream already solved, while the REAL hazard sits
  /// elsewhere.
  ///
  /// **The real hazard: compound sequences, not this call.** The mutex is
  /// released between FFI calls, so a Rule-13 publish-before-apply cycle —
  /// stage → publish → `confirm_published`/`publish_failed` — spans three
  /// round-trips with the lock dropped in between (`manager.rs:651-658`,
  /// `:667-674`). Two concurrent commit-producing flows CAN interleave there.
  /// The engine refuses a second send while the group is non-`Stable`, so the
  /// result is a wedged group rather than a fork — a liveness failure, not a
  /// confidentiality one — but callers must still serialize the whole
  /// TRANSACTION, which is what `LocationPublishSchedulerNotifier`'s
  /// `_publishChain` does. Location publishing produces no commits and needs
  /// no such fence.
  ///
  /// # Arguments
  ///
  /// * `mls_group_id` - The circle's MLS group ID
  /// * `sender_pubkey_hex` - The sender's Nostr public key (hex)
  /// * `latitude` - GPS latitude (exact)
  /// * `longitude` - GPS longitude (exact)
  /// * `update_interval_secs` - **Retired, and no longer reaches the wire.**
  ///   It once seeded a per-send jittered NIP-40 TTL; since the Dark Matter
  ///   cutover the expiration is stamped by the ENGINE from the group-level
  ///   `marmot.group.message-retention.v1` component (0x8005) as
  ///   `inner_created_at + LOCATION_MESSAGE_RETENTION_SECS` (228 s), so
  ///   `CircleManager::encrypt_location` binds this argument as
  ///   `_update_interval_secs` and discards it. The `[60, 3600]` range check
  ///   below is therefore the parameter's only remaining effect. Kept because
  ///   removing it is an FFI signature change; do NOT reintroduce a per-send
  ///   TTL path here without re-reading `haven-core/SECURITY.md`, "Outer
  ///   kind:445 metadata".
  ///
  /// # The deferred outcome
  ///
  /// Returns [`EncryptLocationOutcomeFfi`], not a bare event: the engine can
  /// QUEUE the update instead of encrypting it, and that is a distinct,
  /// actionable state rather than a failure. It is detected by matching the
  /// `CircleError::SendDeferred` VARIANT — never by inspecting error prose,
  /// which interpolates remote-authored text and would hand a remote party a
  /// classification channel (see `nostr::mls::storage::is_session_live`).
  Future<EncryptLocationOutcomeFfi> encryptLocation({
    required List<int> mlsGroupId,
    required String senderPubkeyHex,
    required double latitude,
    required double longitude,
    required BigInt updateIntervalSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiEncryptLocation(
    that: this,
    mlsGroupId: mlsGroupId,
    senderPubkeyHex: senderPubkeyHex,
    latitude: latitude,
    longitude: longitude,
    updateIntervalSecs: updateIntervalSecs,
  );

  /// Resolves public profiles for the given member pubkeys, fetching stale or
  /// missing ones, and returns the merged set.
  ///
  /// Callers pass the UNION of member pubkeys across all circles (plan §1.7) —
  /// never a clean per-circle partition, which would hand the relay exact
  /// co-membership clusters.
  ///
  /// `max_age_secs` is the caller's staleness tolerance: a pubkey whose cached
  /// row was fetched more recently than this is served from cache and never
  /// refetched. `0` forces a fetch for every pubkey; a negative value is
  /// clamped to `0`. Each refresh trigger picks its own tier
  /// (`PROFILE_INTERACTIVE_MAX_AGE_SECS` / `PROFILE_PERIODIC_MAX_AGE_SECS` /
  /// forced), so one constant no longer has to serve every call site.
  ///
  /// # One relay per author
  ///
  /// Due authors are NOT broadcast to a read set. Each is pinned by a salted
  /// rendezvous hash to exactly ONE relay of the profile pool and asked for
  /// alone, so no relay ever observes a co-membership set. The pool is the
  /// curated profile plane minus every relay in the contamination ledger, and
  /// it fails closed: on pool underflow the network fetch is SKIPPED (cached
  /// rows are still returned) rather than falling back onto a relay that
  /// already carries this account's location traffic. Dart surfaces that state
  /// via [`Self::profile_pool_status`].
  ///
  /// # Stamping
  ///
  /// Fetched kind-0s are upserted newest-wins and stamped as HITS; authors
  /// their assigned relay answered nothing for are stamped as MISSES (which
  /// advances the bounded retry ladder without resetting the staleness clock).
  /// Authors the cycle never reached — dropped at the batch deadline, or on a
  /// relay that turned out to be unusable — are stamped **not at all**.
  /// `has_picture` reflects whether CURRENT picture bytes are cached.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on relay or database failure.
  Future<List<ProfileMetadataFfi>> fetchMemberProfiles({
    required List<String> pubkeysHex,
    required PlatformInt64 maxAgeSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiFetchMemberProfiles(
    that: this,
    pubkeysHex: pubkeysHex,
    maxAgeSecs: maxAgeSecs,
  );

  /// Fetches the local user's OWN kind-0 by pubkey, caches it, and returns it.
  ///
  /// Reads need no signer (security review F7): the pubkey identifies the
  /// profile; no secret crosses the FFI. A missing kind-0 yields an `Unknown`
  /// result rather than an error (offline-tolerant, plan D7).
  ///
  /// Reads from the WHOLE profile pool rather than one salted-assignment
  /// relay, concurrently — see [`fetch_own_profile`].
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on relay or database failure.
  Future<ProfileMetadataFfi> fetchMyProfile({required String pubkeyHex}) =>
      RustLib.instance.api.crateApiCircleManagerFfiFetchMyProfile(
        that: this,
        pubkeyHex: pubkeyHex,
      );

  /// Finalizes an admin relay update: confirms the pending commit, then
  /// re-syncs the admin's own `circle.relays` from the engine's routing
  /// component so the admin converges on the new set immediately.
  ///
  /// Use this instead of a bare [`confirm_published`](Self::confirm_published)
  /// for the [`update_circle_relays`](Self::update_circle_relays) flow
  /// (members converge via the receive path). Pass the `pending` token from
  /// the [`CommitToPublishFfi`] and the circle's `mls_group_id`.
  Future<void> finalizeRelayUpdate({
    required PendingStateRefFfi pending,
    required List<int> mlsGroupId,
  }) => RustLib.instance.api.crateApiCircleManagerFfiFinalizeRelayUpdate(
    that: this,
    pending: pending,
    mlsGroupId: mlsGroupId,
  );

  /// Gets all contacts.
  Future<List<ContactFfi>> getAllContacts() =>
      RustLib.instance.api.crateApiCircleManagerFfiGetAllContacts(that: this);

  /// Returns the locally cached profile for a pubkey, or `None`.
  ///
  /// Pure cache read (no network) — the hot path for member markers/tiles.
  /// Async rather than `#[frb(sync)]`: the read is three SQLCipher queries,
  /// and a sync FFI call runs them on the UI isolate.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<ProfileMetadataFfi?> getCachedProfile({required String pubkeyHex}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetCachedProfile(
        that: this,
        pubkeyHex: pubkeyHex,
      );

  /// [`Self::get_cached_profile`] for a whole set, resolved under ONE
  /// connection lock.
  ///
  /// Returns one row per input pubkey, in input order. A pubkey with no
  /// cached kind-0 comes back `is_known: false` rather than being dropped, so
  /// "looked up, nothing found" and "never fetched" read the same — which is
  /// what they mean.
  ///
  /// Pure cache read (no network), and the read a name-only surface wants:
  /// issuing the single-pubkey call per member costs one blocking thread and
  /// three `SQLCipher` queries EACH, and every one of those threads then
  /// queues on the same connection mutex — so the concurrency buys nothing
  /// and the threads are pure cost.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<List<ProfileMetadataFfi>> getCachedProfiles({
    required List<String> pubkeysHex,
  }) => RustLib.instance.api.crateApiCircleManagerFfiGetCachedProfiles(
    that: this,
    pubkeysHex: pubkeysHex,
  );

  /// Gets a circle by its MLS group ID.
  ///
  /// Async: resolving the roster reads the Dark Matter session (which is
  /// `&mut`-serialized behind a `tokio` mutex), so this awaits directly on
  /// the current worker rather than dispatching to the blocking pool.
  Future<CircleWithMembersFfi?> getCircle({required List<int> mlsGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetCircle(
        that: this,
        mlsGroupId: mlsGroupId,
      );

  /// Gets all circles.
  Future<List<CircleWithMembersFfi>> getCircles() =>
      RustLib.instance.api.crateApiCircleManagerFfiGetCircles(that: this);

  /// Gets a contact by pubkey.
  Future<ContactFfi?> getContact({required String pubkey}) => RustLib
      .instance
      .api
      .crateApiCircleManagerFfiGetContact(that: this, pubkey: pubkey);

  /// Gets members of a circle with resolved contact info.
  ///
  /// Async: reads the roster from the Dark Matter session (awaits directly).
  Future<List<CircleMemberFfi>> getMembers({required List<int> mlsGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetMembers(
        that: this,
        mlsGroupId: mlsGroupId,
      );

  /// Gets all pending invitations from the in-memory held-welcome store.
  ///
  /// Each [`InvitationFfi`] carries pre-join STAND-IN fields (the gift-wrap
  /// event id as `mlsGroupId`, `"New Circle"` as the name) because the real
  /// MLS group state lives inside the still-encrypted 1059 held until Accept
  /// (F3). `inviterPubkey`/`inviterNpub` are the only roster fact a pre-join
  /// preview can prove — the NIP-59 seal author — never the full roster. The
  /// gift-wrap id is the key the caller passes to
  /// [`accept_invitation`](Self::accept_invitation) /
  /// [`decline_invitation`](Self::decline_invitation).
  Future<List<InvitationFfi>> getPendingInvitations() => RustLib.instance.api
      .crateApiCircleManagerFfiGetPendingInvitations(that: this);

  /// Returns a member's cached full-resolution profile-picture bytes, or `None`.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<Uint8List?> getProfilePicture({required String pubkeyHex}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetProfilePicture(
        that: this,
        pubkeyHex: pubkeyHex,
      );

  /// Returns a member's cached profile-picture thumbnail bytes, or `None`.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<Uint8List?> getProfileThumbnail({required String pubkeyHex}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetProfileThumbnail(
        that: this,
        pubkeyHex: pubkeyHex,
      );

  /// Returns whether this user wants to publish their relay list for the
  /// given category. Defaults to `true` when never set.
  ///
  /// # Errors
  ///
  /// Fails closed for [`RelayTypeFfi::Profile`]: that category has no
  /// publish toggle because it has no publishable form. A toggle would imply
  /// the plane *could* be advertised and invite a future "just default it to
  /// on" change.
  Future<bool> getPublishRelayList({required RelayTypeFfi relayType}) =>
      RustLib.instance.api.crateApiCircleManagerFfiGetPublishRelayList(
        that: this,
        relayType: relayType,
      );

  /// Gets visible circles (excludes declined invitations).
  Future<List<CircleWithMembersFfi>> getVisibleCircles() => RustLib.instance.api
      .crateApiCircleManagerFfiGetVisibleCircles(that: this);

  /// Returns this manager's current MLS epoch for a group.
  ///
  /// Each peer (the production UI plus the synthetic E2E FFI peers) owns its
  /// own MDK instance, so the epoch must be read per-manager — hence a method
  /// on [`CircleManagerFfi`] rather than a free function.
  ///
  /// Two consumers:
  /// - E2E asserts real key rotation: after an Add/remove/self-update commit
  ///   is finalized (or a peer processes one), the epoch MUST advance by
  ///   exactly 1.
  /// - The circle-details sheet renders it discreetly, so members can compare
  ///   epochs across devices when messages stop decrypting.
  ///
  /// The epoch counter is not secret — it is a commit counter carrying no key
  /// material and no group identifier — so, unlike the other test seams, this
  /// one is compiled into release builds.
  ///
  /// # Errors
  ///
  /// Returns an error if the group does not exist or the MDK query fails.
  Future<BigInt> groupEpoch({required List<int> mlsGroupId}) => RustLib
      .instance
      .api
      .crateApiCircleManagerFfiGroupEpoch(that: this, mlsGroupId: mlsGroupId);

  /// Returns the user's relays for one category, ordered by insertion time.
  Future<List<String>> listUserRelays({required RelayTypeFfi relayType}) =>
      RustLib.instance.api.crateApiCircleManagerFfiListUserRelays(
        that: this,
        relayType: relayType,
      );

  /// Records that a peer's location was decrypted and persisted for a circle.
  ///
  /// `at_ms` is the caller's local RECEIPT clock, never the sender's
  /// timestamp: this answers "is anything still arriving", and a peer's own
  /// clock cannot be trusted to answer that.
  Future<void> notePeerEvent({
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiNotePeerEvent(
    that: this,
    nostrGroupId: nostrGroupId,
    atMs: atMs,
  );

  /// Records that ≥1 relay ACKed a location publish for a circle.
  ///
  /// `at_ms` is the caller's local clock in milliseconds. Call this ONLY on
  /// an affirmative relay ACK — a `PublishResult` with an empty `acceptedBy`
  /// delivered nothing, and stamping it would make a dead publish plane read
  /// as healthy, which is the exact defect this column exists to end.
  Future<void> notePublishAcked({
    required List<int> nostrGroupId,
    required PlatformInt64 atMs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiNotePublishAcked(
    that: this,
    nostrGroupId: nostrGroupId,
    atMs: atMs,
  );

  /// The NIP-40 window Haven asks relays to keep the location messages THIS
  /// device sends into a circle.
  ///
  /// The effective value, not the group's declaration: a circle created by
  /// another Marmot client may declare a much shorter window (honoured, so
  /// relays drop this device's location almost immediately), a longer one
  /// (capped to Haven's own), or none at all (Haven's own). The
  /// circle-details sheet renders it so the first case is visible instead of
  /// reading as a bug, which makes this — like `group_epoch` above — a
  /// release path rather than a test seam.
  ///
  /// Carries no key material and no group identifier, and its error surface
  /// is redacted in core.
  ///
  /// # Errors
  ///
  /// Returns an error if the circle has no live MLS group or the component
  /// cannot be read; the caller hides the segment rather than surfacing it.
  Future<BigInt> outgoingLocationExpirySecs({required List<int> mlsGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiOutgoingLocationExpirySecs(
        that: this,
        mlsGroupId: mlsGroupId,
      );

  /// Classifies the leave operation — see [`LeavePlanFfi`] for the
  /// Flutter-side state machine.
  Future<LeavePlanFfi> planLeave({
    required List<int> mlsGroupId,
    required String selfPubkeyHex,
  }) => RustLib.instance.api.crateApiCircleManagerFfiPlanLeave(
    that: this,
    mlsGroupId: mlsGroupId,
    selfPubkeyHex: selfPubkeyHex,
  );

  /// Processes a gift-wrapped Welcome event (kind 1059).
  ///
  /// This is the high-level API for processing incoming invitations.
  /// It unwraps the gift-wrapped event, extracts the sender info,
  /// and processes the invitation. Circle name and relays are
  /// extracted from the Welcome's embedded group data.
  ///
  /// # Arguments
  ///
  /// * `identity_secret_bytes` - The recipient's identity secret bytes (32 bytes)
  /// * `gift_wrap_event_json` - The kind 1059 gift-wrapped event JSON
  ///
  /// # Returns
  ///
  /// * `Ok(Some(invitation))` — a new pending invitation the caller must
  ///   accept or decline.
  /// * `Ok(None)` — the gift wrap has already been processed on a prior
  ///   poll cycle and should be silently skipped. This is the expected
  ///   outcome for NIP-59's 2-day lookback window repeatedly surfacing
  ///   the same wrapper events.
  /// * `Err(msg)` — a real failure (malformed event, MDK error, etc.).
  ///   The message is already sanitized via `redact_hex_sequences`.
  Future<InvitationFfi?> processGiftWrappedInvitation({
    required List<int> identitySecretBytes,
    required String giftWrapEventJson,
  }) =>
      RustLib.instance.api.crateApiCircleManagerFfiProcessGiftWrappedInvitation(
        that: this,
        identitySecretBytes: identitySecretBytes,
        giftWrapEventJson: giftWrapEventJson,
      );

  /// Whether the own profile has unpublished work, how far it got, and
  /// whether the persisted backoff permits another attempt.
  ///
  /// The read a resume trigger consults before calling
  /// [`Self::sync_my_profile`], so a clean install never dials a relay just
  /// to discover it had nothing to say.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<ProfilePendingStateFfi> profilePendingState() => RustLib.instance.api
      .crateApiCircleManagerFfiProfilePendingState(that: this);

  /// Health of the profile-plane relay pool, as counts only.
  ///
  /// Lets Flutter show "profile lookups are paused" instead of a silently
  /// empty roster when contamination has eaten the pool. **No relay URL ever
  /// crosses this boundary** — see [`ProfilePoolStatusFfi`].
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<ProfilePoolStatusFfi> profilePoolStatus() => RustLib.instance.api
      .crateApiCircleManagerFfiProfilePoolStatus(that: this);

  /// Step 1 of admin handoff: propose promoting `successor_hex` to admin via
  /// an `UpdateAppComponents(admin-policy.v1)` commit.
  ///
  /// Additive — the caller stays admin, so the group is admin-covered at every
  /// intermediate epoch. Returns a [`CommitToPublishFfi`]: publish
  /// `commit_event_json` to the circle's relays, then
  /// [`confirm_published`](Self::confirm_published) on a ≥1-relay ACK or
  /// [`publish_failed`](Self::publish_failed) on failure (Rule 13). Admin
  /// authorization and the "an admin must hold a member leaf" rule are both
  /// enforced by the engine against live MLS state.
  Future<CommitToPublishFfi> proposeAdminHandoff({
    required List<int> mlsGroupId,
    required String successorHex,
  }) => RustLib.instance.api.crateApiCircleManagerFfiProposeAdminHandoff(
    that: this,
    mlsGroupId: mlsGroupId,
    successorHex: successorHex,
  );

  /// Returns a `SelfRemove` proposal event JSON. Publish it, then call
  /// [`complete_leave`](Self::complete_leave).
  ///
  /// A bare `SelfRemove` proposal has NO `PendingStateRef` — a remaining
  /// member commits it later (RFC 9420 §12.1.2), so there is nothing to
  /// confirm or roll back. The returned string is the signed kind:445 event
  /// JSON to publish to the circle's relays.
  Future<String> proposeLeave({required List<int> mlsGroupId}) => RustLib
      .instance
      .api
      .crateApiCircleManagerFfiProposeLeave(that: this, mlsGroupId: mlsGroupId);

  /// Step 2 of admin handoff (or the sole step on the `AdminDemote` path):
  /// demote self from admin.
  ///
  /// Same publish-then-confirm contract as
  /// [`propose_admin_handoff`](Self::propose_admin_handoff). Fails closed when
  /// the caller is the last admin — a sole admin must promote a successor
  /// first, so a circle can never be left with an empty admin set.
  Future<CommitToPublishFfi> proposeSelfDemote({
    required List<int> mlsGroupId,
  }) => RustLib.instance.api.crateApiCircleManagerFfiProposeSelfDemote(
    that: this,
    mlsGroupId: mlsGroupId,
  );

  /// Deletes every row whose `purge_after < now_unix_secs`.
  ///
  /// Returns the number of rows removed.
  Future<int> pruneExpiredLastKnown({required PlatformInt64 nowUnixSecs}) =>
      RustLib.instance.api.crateApiCircleManagerFfiPruneExpiredLastKnown(
        that: this,
        nowUnixSecs: nowUnixSecs,
      );

  /// Prunes the gift-wrap dedup cache (`processed_gift_wraps`): drops rows
  /// past the retention window, then enforces the row cap. Returns the number
  /// of rows removed. Best-effort maintenance, safe to call on every poll
  /// cycle. `now_unix_secs` is the current Unix **seconds** clock. Errors are
  /// redacted.
  Future<BigInt> pruneProcessedGiftWraps({
    required PlatformInt64 nowUnixSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiPruneProcessedGiftWraps(
    that: this,
    nowUnixSecs: nowUnixSecs,
  );

  /// Reports that a staged publish FAILED; the engine discards the staged
  /// commit and returns the group to `Stable` at the prior epoch.
  ///
  /// The publish-failure counterpart to [`confirm_published`](Self::confirm_published);
  /// pass the same `pending` token.
  Future<void> publishFailed({required PendingStateRefFfi pending}) => RustLib
      .instance
      .api
      .crateApiCircleManagerFfiPublishFailed(that: this, pending: pending);

  /// The local member directory in picker order: current co-members first,
  /// then people who shared a circle within the retention window, each block
  /// newest-shared first.
  ///
  /// A pure local read of `circles.db` — no relay traffic, no network.
  ///
  /// Deletes everyone past the retention window before selecting, so opening
  /// the picker is enough to keep the three days honest on a device that has
  /// seen no membership change since the row was written.
  /// `now_unix_secs` is the current Unix **seconds** clock.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<List<DirectoryEntryFfi>> rankedDirectoryMembers({
    required PlatformInt64 nowUnixSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRankedDirectoryMembers(
    that: this,
    nowUnixSecs: nowUnixSecs,
  );

  /// Refreshes the member directory from the union of current co-members
  /// across every visible circle, then sweeps expired rows.
  ///
  /// Returns `false` when the refresh DEFERRED because a circle's membership
  /// commit is still in flight — nothing was written and nothing was purged,
  /// and the caller should simply read the directory as it stands.
  ///
  /// Membership changes already refresh the directory from inside Rust; this
  /// exists so a surface that OPENS the picker can bring it up to date on a
  /// device that has not seen a membership change since the feature shipped.
  /// `now_unix_secs` is the current Unix **seconds** clock.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string if a circle's roster could not be read or
  /// the directory could not be written. The directory is left untouched.
  Future<bool> reconcileMemberDirectory({required PlatformInt64 nowUnixSecs}) =>
      RustLib.instance.api.crateApiCircleManagerFfiReconcileMemberDirectory(
        that: this,
        nowUnixSecs: nowUnixSecs,
      );

  /// Records a successful publication so the unpublish path can issue a
  /// NIP-09 deletion later. Pass the `event_id_hex`, `kind`, and
  /// `published_at_secs` returned in
  /// [`BuiltRelayListEventFfi`] after a successful relay publish.
  ///
  /// `published_at_secs` MUST be the `created_at` of the signed event
  /// (not a freshly-fetched local timestamp). If a Dart caller loses
  /// the value (older bindings) it may pass a current-time fallback,
  /// but the unpublish path's clock-skew defense is then weaker.
  Future<void> recordPublishedRelayList({
    required String identityPubkeyHex,
    required int kind,
    required String eventIdHex,
    required PlatformInt64 publishedAtSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRecordPublishedRelayList(
    that: this,
    identityPubkeyHex: identityPubkeyHex,
    kind: kind,
    eventIdHex: eventIdHex,
    publishedAtSecs: publishedAtSecs,
  );

  /// Returns the deduplicated publish targets for `relay_type` — the
  /// user's own configured relays and nothing else.
  ///
  /// Two-plane model: this is exactly the user's configured list (no
  /// force-union with public defaults), so a private relay never leaks.
  /// Exposed for the relay-status UI. The publish flow uses the same
  /// computation internally; do NOT use this method to compute publish
  /// targets in Dart — call [`Self::build_relay_list_publish`] instead.
  ///
  /// # Errors
  ///
  /// Fails closed for [`RelayTypeFfi::Profile`], like every other method on
  /// this publish-plane surface. The profile category is local-only: there
  /// are no "publish targets" for it, and a Dart caller that received a list
  /// here would hold exactly the input a `publish_event` loop wants. The
  /// sentence above ("do NOT use this to compute publish targets") is a
  /// convention; this arm is the guarantee, and it also keeps the whole
  /// `RelayTypeFfi` surface uniformly fail-closed so no future reader has to
  /// wonder which member is the exception.
  Future<List<String>> relayPublishTargets({required RelayTypeFfi relayType}) =>
      RustLib.instance.api.crateApiCircleManagerFfiRelayPublishTargets(
        that: this,
        relayType: relayType,
      );

  /// Removes every last-known location row for a circle.
  ///
  /// Called when the user leaves or deletes a circle.
  Future<void> removeLastKnownCircle({required List<int> nostrGroupId}) =>
      RustLib.instance.api.crateApiCircleManagerFfiRemoveLastKnownCircle(
        that: this,
        nostrGroupId: nostrGroupId,
      );

  /// Removes the last-known location for a single sender in a circle.
  ///
  /// Called when a member is removed from the circle.
  Future<void> removeLastKnownMember({
    required List<int> nostrGroupId,
    required String senderPubkey,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRemoveLastKnownMember(
    that: this,
    nostrGroupId: nostrGroupId,
    senderPubkey: senderPubkey,
  );

  /// Removes members from a circle.
  ///
  /// Returns a [`CommitToPublishFfi`] (publish-before-apply, Rule 13):
  /// publish `commit_event_json`, then [`confirm_published`](Self::confirm_published)
  /// on a ≥1-relay ACK or [`publish_failed`](Self::publish_failed) on failure.
  Future<CommitToPublishFfi> removeMembers({
    required List<int> mlsGroupId,
    required List<String> memberPubkeys,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRemoveMembers(
    that: this,
    mlsGroupId: mlsGroupId,
    memberPubkeys: memberPubkeys,
  );

  /// Removes the local user's OWN profile picture (retraction republish).
  ///
  /// A **no-op unless a profile was published** (`has_published_profile`) — a
  /// retraction must never mint a first public event for a pubkey that never
  /// published. Otherwise it clears the `picture` field on the freshest kind-0
  /// and republishes.
  ///
  /// Either way, a photo still QUEUED for upload is cancelled: it was going
  /// to publish the picture the user is removing. A queued NAME edit is left
  /// alone — it has nothing to do with the photo.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on a secret that is not this account's
  /// identity, or on relay or database failure.
  Future<ProfileMetadataFfi> removeMyProfilePicture({
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRemoveMyProfilePicture(
    that: this,
    identitySecretBytes: identitySecretBytes,
  );

  /// Removes a relay from one category.
  ///
  /// Returns `true` when a row was removed, `false` when the URL was not
  /// in the user's list. Refuses to delete the last relay in a category
  /// (returns `Err` so the UI can show "you need at least one relay").
  Future<bool> removeUserRelay({
    required String url,
    required RelayTypeFfi relayType,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRemoveUserRelay(
    that: this,
    url: url,
    relayType: relayType,
  );

  /// Repairs a circle whose sender ratchets are exhausted, by committing a
  /// byte-identical `UpdateAppComponents(admin-policy.v1)`.
  ///
  /// This is a **ratchet reset**, not key rotation: applying the commit
  /// derives a fresh `encryption_secret`, so every sender ratchet in the group
  /// restarts at generation 0 and a peer whose messages had run past
  /// `maximum_forward_distance` becomes decryptable again. The commit carries
  /// no `UpdatePath`, so it rotates no leaf key and provides **no**
  /// post-compromise security. Never describe it as key rotation in code, in
  /// logs, or in user copy.
  ///
  /// # The caller MUST fetch first
  ///
  /// Await one relay fetch / live-sync drain for this circle and let it settle
  /// BEFORE calling this. A departure proposal sitting on a relay that has not
  /// yet reached this device turns into a same-epoch race; the same proposal
  /// ingested first turns into a clean `PendingProposal` decline. The engine's
  /// deterministic ordering makes the race survivable (one extra epoch, never
  /// a fork), but the fetch converts a race into a decline for free.
  ///
  /// # Rule 14 / gate 5
  ///
  /// Foreground service layer ONLY — never from a background isolate. Enforced
  /// by `scripts/ci/check_epoch_repair_isolation.sh`, not by this comment.
  ///
  /// # Outcomes
  ///
  /// Exactly one field of [`RepairRotationOutcomeFfi`] is `Some`:
  /// `rotated` → publish then confirm on a ≥1-relay ACK (or roll back);
  /// `deferred_send` → run that same ladder over its `commits`;
  /// `skipped` → a normal, user-visible answer, NOT a failure.
  ///
  /// `now_secs` is the wall clock in Unix seconds.
  ///
  /// # Errors
  ///
  /// Returns an error string only for a real failure — no circle row, an
  /// unreadable store, or an engine rejection that is not about epoch state
  /// (a non-`Stable` epoch is a `skipped` outcome, not an error).
  Future<RepairRotationOutcomeFfi> repairEpochRotation({
    required List<int> mlsGroupId,
    required BigInt nowSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiRepairEpochRotation(
    that: this,
    mlsGroupId: mlsGroupId,
    nowSecs: nowSecs,
  );

  /// Resets ALL sync cursors (bulk) for the wipe-on-logout path, so a
  /// returning identity re-seeds cleanly instead of resuming at a stale
  /// floor. Errors are redacted.
  Future<void> resetAllSyncCursors() => RustLib.instance.api
      .crateApiCircleManagerFfiResetAllSyncCursors(that: this);

  /// Resolves ONE typed stranger's published profile — the member-picker
  /// "typed-stranger resolve" (plan §10 D2).
  ///
  /// A THIN wrapper over [`Self::fetch_member_profiles`] for exactly one
  /// pubkey: salted single-relay assignment, pool selection, pacing, the
  /// contamination ledger and the staleness/negative-cache machinery are all
  /// inherited verbatim by delegating to it, rather than re-implemented
  /// here. This function adds no relay selection of its own — two
  /// alternatives were evaluated and rejected: fetching from a relay outside
  /// the pubkey's salted assignment would add a THIRD relay to that
  /// pubkey's disclosure set (the design bounds it to two for the life of
  /// the install), and decoy padding is theatre, since each author routes to
  /// its own salted relay independently, so a decoy only helps if it
  /// happens to hash to the SAME relay.
  ///
  /// The one thing this adds over `fetch_member_profiles`: `pubkey` accepts
  /// EITHER hex or bech32 (`npub1…`), via `PublicKey::parse` (the pattern
  /// already used to resolve another user's pubkey in
  /// `relay/manager.rs`). `fetch_member_profiles` itself stays hex-only
  /// (`PublicKey::from_hex`) — Dart has no npub-to-hex decoder, so that
  /// translation has to happen on this side of the boundary.
  ///
  /// A `pubkey` that parses (bech32 checksum included) but resolves to no
  /// known kind-0 still returns `Ok(Some(..))`, with `is_known: false` —
  /// the caller renders that as "nothing found", exactly like any other
  /// unresolved pubkey. Only a `pubkey` that FAILS to parse returns
  /// `Ok(None)`: Dart's `NpubValidator` checks only the `npub1` prefix,
  /// length and bech32 charset — no checksum — so a charset-valid npub can
  /// still fail to parse here, and that must read as "nothing found", never
  /// as an error.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on relay or database failure (never on
  /// a malformed `pubkey`, which yields `Ok(None)` instead).
  Future<ProfileMetadataFfi?> resolveStrangerProfile({
    required String pubkey,
    required PlatformInt64 maxAgeSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiResolveStrangerProfile(
    that: this,
    pubkey: pubkey,
    maxAgeSecs: maxAgeSecs,
  );

  /// Restores the curated profile pool non-destructively (adds back any
  /// missing curated entry; keeps the user's own additions).
  ///
  /// The recovery action for a pool that has underflowed.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<void> restoreDefaultProfileRelays() => RustLib.instance.api
      .crateApiCircleManagerFfiRestoreDefaultProfileRelays(that: this);

  /// Restores defaults for a category **non-destructively**.
  ///
  /// Adds any missing default relays via `INSERT OR IGNORE`. Existing
  /// user-added custom relays are preserved. Use
  /// [`Self::wipe_and_reset_defaults_for`] for the destructive variant.
  Future<void> restoreDefaultsFor({required RelayTypeFfi relayType}) =>
      RustLib.instance.api.crateApiCircleManagerFfiRestoreDefaultsFor(
        that: this,
        relayType: relayType,
      );

  /// Saves a display-name / bio edit LOCALLY and queues it for publication.
  ///
  /// Returns immediately with the row the UI should render: no relay is
  /// dialed and no signing key is needed, because nothing is published here.
  /// [`Self::sync_my_profile`] is what makes the network agree, and
  /// [`Self::profile_pending_state`] is what says whether it has.
  ///
  /// `display_name`/`about` follow `ProfileEdits` semantics (`None` =
  /// untouched, `Some("")` = clear) and ACCUMULATE across saves, so renaming
  /// and then editing the bio publishes both.
  ///
  /// The profile is the LOCAL USER's — the pubkey comes from the manager's
  /// construction keys, never from the caller.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  Future<ProfileMetadataFfi> saveMyProfileLocal({
    String? displayName,
    String? about,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSaveMyProfileLocal(
    that: this,
    displayName: displayName,
    about: about,
  );

  /// Sanitizes the chosen photo and saves it LOCALLY, queued for upload.
  ///
  /// The whole sanitizer runs here — EXIF/GPS stripped, re-encoded, resized
  /// (`process_own_avatar`) — so what is staged is already what may be
  /// uploaded, and the raw camera bytes never outlive this call. Nothing
  /// touches Blossom or a relay: [`Self::sync_my_profile`] uploads the staged
  /// bytes and publishes the URL.
  ///
  /// The returned hash is the sanitized bytes' content hash, which is also
  /// the Blossom address the eventual upload resolves to.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string if the image cannot be decoded within
  /// the avatar limits, or on database failure.
  Future<ProfilePictureRefFfi> saveMyProfilePictureLocal({
    required List<int> raw,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSaveMyProfilePictureLocal(
    that: this,
    raw: raw,
  );

  /// Seeds the user's relay lists with the default relay list returned by
  /// [`haven_core::circle::default_relays`] on first launch.
  ///
  /// Idempotent: short-circuits via the `relay_prefs_seeded_v1` sentinel
  /// in `user_settings`. Crucially, the sentinel is the signal — never
  /// row presence in `user_relays`. A user who removes a default relay
  /// must not have it re-added by the next defensive seed.
  ///
  /// # Returns
  ///
  /// `true` if seeding actually wrote rows; `false` if the sentinel was
  /// already set.
  Future<bool> seedRelayDefaultsIfUnseeded() => RustLib.instance.api
      .crateApiCircleManagerFfiSeedRelayDefaultsIfUnseeded(that: this);

  /// Sets or updates a contact.
  ///
  /// Contact information is stored locally only and never synced to relays.
  Future<ContactFfi> setContact({
    required String pubkey,
    String? displayName,
    String? notes,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSetContact(
    that: this,
    pubkey: pubkey,
    displayName: displayName,
    notes: notes,
  );

  /// Sets or clears a member's local petname (contact `display_name`).
  ///
  /// A purely local override (plan D6): `Some(name)` sets it, `None` clears it.
  /// Existing contact `notes` are preserved. Never leaves the device.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on database failure.
  void setLocalNickname({required String pubkeyHex, String? nickname}) =>
      RustLib.instance.api.crateApiCircleManagerFfiSetLocalNickname(
        that: this,
        pubkeyHex: pubkeyHex,
        nickname: nickname,
      );

  /// Sets whether this user wants to publish their relay list for the
  /// given category.
  ///
  /// # Errors
  ///
  /// Fails closed for [`RelayTypeFfi::Profile`] — see
  /// [`Self::get_publish_relay_list`].
  Future<void> setPublishRelayList({
    required RelayTypeFfi relayType,
    required bool value,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSetPublishRelayList(
    that: this,
    relayType: relayType,
    value: value,
  );

  /// Signs a NIP-09 event deletion event.
  ///
  /// Creates a kind 5 deletion event referencing the given event IDs,
  /// signed with the provided identity key. Used to delete consumed
  /// `KeyPackage` events from relays after rotation.
  String signDeletionEvent({
    required List<int> identitySecretBytes,
    required List<String> eventIds,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSignDeletionEvent(
    that: this,
    identitySecretBytes: identitySecretBytes,
    eventIds: eventIds,
  );

  /// Returns all non-purged last-known locations for a circle.
  Future<List<LastKnownLocationFfi>> snapshotLastKnownForCircle({
    required List<int> nostrGroupId,
    required PlatformInt64 nowUnixSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSnapshotLastKnownForCircle(
    that: this,
    nostrGroupId: nostrGroupId,
    nowUnixSecs: nowUnixSecs,
  );

  /// Returns whether `pubkey_hex` is still in the circle's current MLS
  /// roster — the REV-1 leaver-backstop liveness predicate.
  ///
  /// The Dart leave flow polls this with the leaver's OWN pubkey after
  /// publishing a `SelfRemove` (see [`propose_leave`](Self::propose_leave)):
  /// while it returns `true` the leaver re-issues a fresh `propose_leave` on
  /// each epoch advance; once it returns `false` the eviction has landed and
  /// [`complete_leave`](Self::complete_leave) can wipe local state. Fails
  /// SAFE to `false` when the group is gone or the caller has been evicted,
  /// so a removed leaver stops re-issuing. Error strings are hex-redacted by
  /// the core method (Security Rule 4/8).
  Future<bool> stillAMember({
    required List<int> mlsGroupId,
    required String pubkeyHex,
  }) => RustLib.instance.api.crateApiCircleManagerFfiStillAMember(
    that: this,
    mlsGroupId: mlsGroupId,
    pubkeyHex: pubkeyHex,
  );

  /// Gives a terminal disposition to stored convergence inputs the relay can
  /// no longer redeliver, across every circle, and reports what still gates
  /// outbound sends.
  ///
  /// The "repair sharing" entry point: a future-epoch application row is kept
  /// `Retryable` by the engine so a late commit can still resolve it, and
  /// when that commit never arrives the row gates every send for its circle
  /// — through restarts, because hydration keeps it for the same reason. This
  /// is the only thing that clears one. `now_secs` is the wall clock in Unix
  /// seconds.
  ///
  /// # Errors
  ///
  /// Returns an error if the MLS message store cannot be read or written.
  Future<ConvergenceSweepFfi> sweepUnresolvableInputs({
    required BigInt nowSecs,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSweepUnresolvableInputs(
    that: this,
    nowSecs: nowSecs,
  );

  /// Publishes whatever the own-profile outbox is holding.
  ///
  /// Idempotent and safe to call from any resume/foreground trigger: with
  /// nothing pending it returns [`ProfileSyncOutcomeFfi::NothingPending`],
  /// and with an unusable relay pool [`ProfileSyncOutcomeFfi::PoolUnderflow`],
  /// in both cases without touching the network OR the supplied secret.
  /// Network failure is an OUTCOME, not an error — the save stays queued with
  /// its retry ladder advanced, and only a local fault produces `Err`.
  ///
  /// # Errors
  ///
  /// Returns a redacted error string on a malformed secret, a secret that is
  /// not this account's identity, or a local database/signing fault.
  Future<ProfileSyncResultFfi> syncMyProfile({
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiCircleManagerFfiSyncMyProfile(
    that: this,
    identitySecretBytes: identitySecretBytes,
  );

  /// Admin: replace this circle's group relay list (MIP-01) via an
  /// `UpdateAppComponents(nostr-routing.v1)` commit.
  ///
  /// Returns a [`CommitToPublishFfi`]. Publish `commit_event_json` to the
  /// **union of the circle's current relays and `new_relays`** (so a member
  /// only listening on a relay being removed still receives the commit), then
  /// call [`finalize_relay_update`](Self::finalize_relay_update) on a ≥1-relay
  /// ACK or [`publish_failed`](Self::publish_failed) on failure. `new_relays`
  /// MUST be non-empty, `wss://` (or the debug loopback test seam),
  /// credential-free, and at most 20 entries; admin authorization is enforced
  /// by the engine against live MLS state.
  Future<CommitToPublishFfi> updateCircleRelays({
    required List<int> mlsGroupId,
    required List<String> newRelays,
  }) => RustLib.instance.api.crateApiCircleManagerFfiUpdateCircleRelays(
    that: this,
    mlsGroupId: mlsGroupId,
    newRelays: newRelays,
  );

  /// Persists a last-known location row.
  ///
  /// Input is validated at the FFI boundary; the core manager is the
  /// authoritative enforcement point for `purge_after` derivation. The
  /// `purge_after` value supplied by the caller is advisory only — the
  /// core recomputes it as `timestamp + LOCATION_RETENTION_SECS`.
  Future<void> upsertLastKnownLocation({
    required LastKnownLocationFfi location,
  }) => RustLib.instance.api.crateApiCircleManagerFfiUpsertLastKnownLocation(
    that: this,
    location: location,
  );

  /// Wipes every last-known location row.
  ///
  /// Called from the identity-deletion path so no stale location data
  /// survives a full account wipe.
  Future<void> wipeAllLastKnownLocations() => RustLib.instance.api
      .crateApiCircleManagerFfiWipeAllLastKnownLocations(that: this);

  /// Destructively resets a category to exactly the default relay list
  /// returned by [`haven_core::circle::default_relays`].
  ///
  /// Wipes all rows for the category and re-inserts defaults in one
  /// transaction. The caller MUST gate this behind a confirmation
  /// dialog; the function name is deliberately verbose.
  Future<void> wipeAndResetDefaultsFor({required RelayTypeFfi relayType}) =>
      RustLib.instance.api.crateApiCircleManagerFfiWipeAndResetDefaultsFor(
        that: this,
        relayType: relayType,
      );
}

@sealed
class HavenCoreImpl extends RustOpaque implements HavenCore {
  // Not to be used by end users
  HavenCoreImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  HavenCoreImpl.frbInternalSseDecode(BigInt ptr, int externalSizeOnNative)
    : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_HavenCore,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_HavenCore,
    rustArcDecrementStrongCountPtr:
        RustLib.instance.api.rust_arc_decrement_strong_count_HavenCorePtr,
  );

  /// Gets the current location settings.
  LocationSettings getLocationSettings() =>
      RustLib.instance.api.crateApiHavenCoreGetLocationSettings(that: this);

  /// Initializes the core.
  Future<void> initialize() =>
      RustLib.instance.api.crateApiHavenCoreInitialize(that: this);

  /// Returns whether the core has been initialized.
  bool isInitialized() =>
      RustLib.instance.api.crateApiHavenCoreIsInitialized(that: this);

  /// Updates the location settings.
  void setLocationSettings({required LocationSettings settings}) => RustLib
      .instance
      .api
      .crateApiHavenCoreSetLocationSettings(that: this, settings: settings);

  /// Processes raw location data and returns a `LocationMessage` with
  /// exact GPS coordinates.
  LocationMessage updateLocation({
    required double latitude,
    required double longitude,
  }) => RustLib.instance.api.crateApiHavenCoreUpdateLocation(
    that: this,
    latitude: latitude,
    longitude: longitude,
  );
}

@sealed
class LiveSyncFfiImpl extends RustOpaque implements LiveSyncFfi {
  // Not to be used by end users
  LiveSyncFfiImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  LiveSyncFfiImpl.frbInternalSseDecode(BigInt ptr, int externalSizeOnNative)
    : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_LiveSyncFfi,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_LiveSyncFfi,
    rustArcDecrementStrongCountPtr:
        RustLib.instance.api.rust_arc_decrement_strong_count_LiveSyncFfiPtr,
  );

  /// How many publishes this session has between SEND and OK right now.
  ///
  /// The other half of the same promise: presence on a relay is permitted
  /// exactly while a publish is outstanding, so a burst that is quiet on both
  /// counters is holding nothing. It is also the Rule-13 gauge
  /// [`Self::pause_subscriptions`] blocks on, so a test can observe the state
  /// the rule forbids cutting — "a commit is on the wire" — instead of
  /// inferring it from a duration.
  ///
  /// Presence-only: a COUNT. Never an event id, a relay url or a group id.
  /// Test and diagnostic surface.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session or the lock is poisoned.
  Future<int> inFlightPublishes() =>
      RustLib.instance.api.crateApiLiveSyncFfiInFlightPublishes(that: this);

  /// Whether the session is PAUSED between background bursts.
  ///
  /// Orthogonal to [`Self::is_running`], which stays `true` across a pause:
  /// the session is alive, it simply holds no REQ and no socket. A caller
  /// deciding whether to re-anchor must read THIS — a paused engine has no
  /// subscription to repair, and re-anchoring one would silently re-open
  /// standing REQs in the background.
  bool isPaused() =>
      RustLib.instance.api.crateApiLiveSyncFfiIsPaused(that: this);

  /// Whether a live session is currently running.
  bool isRunning() =>
      RustLib.instance.api.crateApiLiveSyncFfiIsRunning(that: this);

  /// Streams decrypted live events to Dart for the lifetime of the session.
  ///
  /// Returns `Err` immediately if no session is active (a cold-start race, not
  /// a hung stream). The loop ends when the Dart side closes the sink
  /// (`sink.add` returns `Err`) OR the bus closes on `stop_session` (the last
  /// `Arc<LiveSyncCore>` drops → `RecvError::Closed`); a lag is skipped, never
  /// fatal (the cursor + catch-up replay anything dropped).
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session or the lock is poisoned.
  Stream<FfiRelayEvent> liveEvents() =>
      RustLib.instance.api.crateApiLiveSyncFfiLiveEvents(that: this);

  /// Opens ONE background burst: the same re-anchor as
  /// [`Self::resume_after_background`], plus the inbox fold — the inbox REQ
  /// rides only every `INBOX_BURSTS_PER_REQ`-th burst, and this call advances
  /// the counter that decides it.
  ///
  /// The BACKGROUND entry point, and the only one the burst coordinator may
  /// use. Calling `resume_after_background` for a burst instead would leave
  /// the fold permanently un-applied — every burst would re-request the
  /// bounded inbox window, which is the metadata cost the fold exists to cut.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session, the lock is poisoned, or
  /// a re-subscription fails.
  Future<void> openBackgroundBurst() =>
      RustLib.instance.api.crateApiLiveSyncFfiOpenBackgroundBurst(that: this);

  /// Closes whatever is open: drops every standing REQ and disconnects the
  /// engine's sockets, leaving the session alive and re-openable.
  ///
  /// Usually that is a burst's own REQs, but not necessarily — a background
  /// pause that drove no burst tears the FOREGROUND session's down through
  /// this same call, which is what makes "no socket while backgrounded" true
  /// of the gap before the first burst too.
  ///
  /// Call it from a `finally`, and only after [`Self::settle_before_pause`] —
  /// the pause itself waits for the in-flight publish gauge, but settling
  /// first is what gives a commit the socket time its convergence traffic
  /// needs (Security Rule 13).
  ///
  /// # Errors
  ///
  /// Returns an error if the session slot is empty, the lock is poisoned, or
  /// the core refused because it is already shutting down — its ONLY error
  /// path, and a pre-flight one: it runs before the first `unsubscribe`, so
  /// an error means nothing was torn down, never that something was torn
  /// halfway. None of the three leaves a RUNNING session behind either:
  /// [`Self::is_running`] is `!shutdown && !wedged`, and it answers `false`
  /// for an absent slot, an unreadable one and a shutting-down core alike.
  /// Past that pre-flight check the pause is best-effort and reports `Ok`.
  Future<void> pauseSubscriptions() =>
      RustLib.instance.api.crateApiLiveSyncFfiPauseSubscriptions(that: this);

  /// How many long-lived subscriptions the engine pool holds right now,
  /// across every relay.
  ///
  /// The DIRECT read of the background-burst promise "no standing REQ between
  /// publish ticks". [`Self::is_paused`] cannot stand in for it: the core sets
  /// that flag as the FIRST statement of its pause — before `unsubscribe_all`,
  /// before the drain marker, before the uncapped Rule-13 publish gauge and
  /// before `disconnect()` — so it reports that the pause was ENTERED, not
  /// that the REQs and the sockets are gone. An oracle built on the flag
  /// asserts an intent; this one asserts the state.
  ///
  /// Presence-only: a COUNT. Never a subscription id, a relay url or a group
  /// id (Security Rules 4 and 6). Test and diagnostic surface — no
  /// user-facing path reads it.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session or the lock is poisoned,
  /// so a caller can never read "no session" as "no standing REQ".
  Future<int> poolSubscriptionCount() =>
      RustLib.instance.api.crateApiLiveSyncFfiPoolSubscriptionCount(that: this);

  /// Re-anchors the session after a background period / reconnect.
  ///
  /// The FOREGROUND entry point: app-resume and the subscription-health
  /// tick's whole-session repair. It always carries the inbox REQ, so an
  /// app that is simply open can always receive an invitation.
  ///
  /// A background burst must use [`Self::open_background_burst`] instead —
  /// the two are separate in the core precisely because only the burst may
  /// consume a position in the inbox fold, and Rust cannot tell the callers
  /// apart through one entry point.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session, the lock is poisoned, or
  /// a re-subscription fails.
  Future<void> resumeAfterBackground() =>
      RustLib.instance.api.crateApiLiveSyncFfiResumeAfterBackground(that: this);

  /// Holds the sockets open until the engine's commit traffic has quiesced,
  /// so the pause that follows cannot cut a commit between SEND and OK
  /// (Security Rule 13).
  ///
  /// The window spans everything since the last re-anchor: a burst's own
  /// traffic after a burst, and the foreground session's on a teardown no
  /// burst preceded. Either way it returns immediately when nothing has
  /// committed inside it — the common case costs nothing. The core call is
  /// infallible; the `Result` is the session gate.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session or the lock is poisoned.
  Future<void> settleBeforePause() =>
      RustLib.instance.api.crateApiLiveSyncFfiSettleBeforePause(that: this);

  /// Starts the live session over `groups` + `inbox_relays`. Idempotent — a
  /// second call while a session is live returns `Ok` without rebuilding.
  ///
  /// # Errors
  ///
  /// Returns an error if a group spec is malformed, the lock is poisoned, or a
  /// subscription fails (on which the reserved session slot is cleared).
  Future<void> startSession({
    required List<FfiGroupSpec> groups,
    required List<String> inboxRelays,
  }) => RustLib.instance.api.crateApiLiveSyncFfiStartSession(
    that: this,
    groups: groups,
    inboxRelays: inboxRelays,
  );

  /// Stops + drops the live session (logout/teardown). Idempotent.
  ///
  /// # Errors
  ///
  /// Returns an error only if the session lock is poisoned.
  Future<void> stopSession() =>
      RustLib.instance.api.crateApiLiveSyncFfiStopSession(that: this);

  /// Subscribes the running session to ONE additional circle (delta only), at
  /// its OWN cursor/seed, without re-anchoring any other circle's subscription
  /// (the M3-deferred incremental subscribe). Idempotent for an already-
  /// subscribed circle.
  ///
  /// # Errors
  ///
  /// Returns an error if `spec.nostr_group_id` is malformed, there is no active
  /// session, the lock is poisoned, a relay fails the WSS gate, or the
  /// subscription fails. The Dart caller falls back to a full restart on error.
  Future<void> subscribeCircle({required FfiGroupSpec spec}) => RustLib
      .instance
      .api
      .crateApiLiveSyncFfiSubscribeCircle(that: this, spec: spec);

  /// Unsubscribes the running session from ONE circle (delta only), dropping
  /// only its receive subscription. Idempotent: an unknown circle or no active
  /// session is a no-op (`Ok`).
  ///
  /// # Errors
  ///
  /// Returns an error if `nostr_group_id` is malformed, the lock is poisoned, or
  /// a multiplexed-bucket re-issue fails (the Dart caller then full-restarts).
  Future<void> unsubscribeCircle({required List<int> nostrGroupId}) =>
      RustLib.instance.api.crateApiLiveSyncFfiUnsubscribeCircle(
        that: this,
        nostrGroupId: nostrGroupId,
      );

  /// Waits for every endpoint this burst opened to finish its stored replay,
  /// so a peer commit that landed while paused is applied BEFORE the burst
  /// encrypts its location.
  ///
  /// The core call is infallible — a slow relay yields
  /// [`BacklogOutcomeFfi::TimedOut`] and the burst publishes anyway — so the
  /// `Result` here is purely the session gate.
  ///
  /// # Errors
  ///
  /// Returns an error if there is no active session or the lock is poisoned.
  Future<BacklogOutcomeFfi> waitBacklogSettled() =>
      RustLib.instance.api.crateApiLiveSyncFfiWaitBacklogSettled(that: this);
}

@sealed
class LocationEventServiceImpl extends RustOpaque
    implements LocationEventService {
  // Not to be used by end users
  LocationEventServiceImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  LocationEventServiceImpl.frbInternalSseDecode(
    BigInt ptr,
    int externalSizeOnNative,
  ) : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount: RustLib
        .instance
        .api
        .rust_arc_increment_strong_count_LocationEventService,
    rustArcDecrementStrongCount: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_LocationEventService,
    rustArcDecrementStrongCountPtr: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_LocationEventServicePtr,
  );

  /// Returns a jittered publish interval in seconds using the Rust-side
  /// CSPRNG (`OsRng`), sampled uniformly in
  /// `[nominal_secs * 0.6, nominal_secs * 1.4]` (40% spread).
  ///
  /// Callers pass the nominal interval (typically 120 s) and rearm their
  /// timer with the returned value. This is NOT the TTL window — the
  /// `expiration` tag sampled inside `encrypt_location` is independent
  /// and intentionally decoupled (see `haven-core/SECURITY.md`).
  ///
  /// On `nominal_secs == 0` the call falls back to `nominal_secs` rather
  /// than panicking, so a zero at the boundary does not silently
  /// reschedule at 0 s. Dart callers should pass a positive nominal.
  BigInt jitteredPublishIntervalSecs({required BigInt nominalSecs}) => RustLib
      .instance
      .api
      .crateApiLocationEventServiceJitteredPublishIntervalSecs(
        that: this,
        nominalSecs: nominalSecs,
      );

  /// Verifies the signature of a signed event.
  ///
  /// Returns `true` if the signature is valid, `false` otherwise.
  bool verifySignature({required SignedLocationEventFfi event}) => RustLib
      .instance
      .api
      .crateApiLocationEventServiceVerifySignature(that: this, event: event);
}

@sealed
class LocationMessageImpl extends RustOpaque implements LocationMessage {
  // Not to be used by end users
  LocationMessageImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  LocationMessageImpl.frbInternalSseDecode(BigInt ptr, int externalSizeOnNative)
    : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_LocationMessage,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_LocationMessage,
    rustArcDecrementStrongCountPtr:
        RustLib.instance.api.rust_arc_decrement_strong_count_LocationMessagePtr,
  );

  /// Gets the expiration time as Unix timestamp (seconds since epoch).
  PlatformInt64 expiresAt() =>
      RustLib.instance.api.crateApiLocationMessageExpiresAt(that: this);

  /// Gets the geohash representation.
  String geohash() =>
      RustLib.instance.api.crateApiLocationMessageGeohash(that: this);

  /// Checks if the location has expired.
  bool isExpired() =>
      RustLib.instance.api.crateApiLocationMessageIsExpired(that: this);

  /// Gets the latitude.
  double latitude() =>
      RustLib.instance.api.crateApiLocationMessageLatitude(that: this);

  /// Gets the longitude.
  double longitude() =>
      RustLib.instance.api.crateApiLocationMessageLongitude(that: this);

  /// Gets the timestamp as Unix timestamp (seconds since epoch).
  PlatformInt64 timestamp() =>
      RustLib.instance.api.crateApiLocationMessageTimestamp(that: this);
}

@sealed
class LocationSettingsImpl extends RustOpaque implements LocationSettings {
  // Not to be used by end users
  LocationSettingsImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  LocationSettingsImpl.frbInternalSseDecode(
    BigInt ptr,
    int externalSizeOnNative,
  ) : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_LocationSettings,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_LocationSettings,
    rustArcDecrementStrongCountPtr: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_LocationSettingsPtr,
  );

  /// Gets the update interval in minutes.
  int updateIntervalMinutes() => RustLib.instance.api
      .crateApiLocationSettingsUpdateIntervalMinutes(that: this);
}

@sealed
class NostrIdentityManagerImpl extends RustOpaque
    implements NostrIdentityManager {
  // Not to be used by end users
  NostrIdentityManagerImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  NostrIdentityManagerImpl.frbInternalSseDecode(
    BigInt ptr,
    int externalSizeOnNative,
  ) : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount: RustLib
        .instance
        .api
        .rust_arc_increment_strong_count_NostrIdentityManager,
    rustArcDecrementStrongCount: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_NostrIdentityManager,
    rustArcDecrementStrongCountPtr: RustLib
        .instance
        .api
        .rust_arc_decrement_strong_count_NostrIdentityManagerPtr,
  );

  /// Clears the in-memory cache.
  ///
  /// Call this when the app goes to background.
  Future<void> clearCache() =>
      RustLib.instance.api.crateApiNostrIdentityManagerClearCache(that: this);

  /// Creates a new random identity.
  ///
  /// After calling this, use `get_secret_bytes()` to persist the secret.
  Future<PublicIdentity> createIdentity() => RustLib.instance.api
      .crateApiNostrIdentityManagerCreateIdentity(that: this);

  /// Deletes the identity.
  ///
  /// Re-mints the log-alias salt on the way out: every handle this process
  /// already emitted must stop being linkable to the identity that produced
  /// it (Security Rule 15). Unconditional, because a failed delete can still
  /// have destroyed the stored secret.
  Future<void> deleteIdentity() => RustLib.instance.api
      .crateApiNostrIdentityManagerDeleteIdentity(that: this);

  /// Exports the identity as nsec for backup.
  ///
  /// # Security Warning
  ///
  /// This exposes the secret key. Only use for user-initiated backup.
  Future<String> exportNsec() =>
      RustLib.instance.api.crateApiNostrIdentityManagerExportNsec(that: this);

  /// Gets the current public identity.
  PublicIdentity? getIdentity() =>
      RustLib.instance.api.crateApiNostrIdentityManagerGetIdentity(that: this);

  /// Gets the secret bytes for persistence in Flutter secure storage.
  ///
  /// # Security Warning
  ///
  /// Handle these bytes with extreme care. They should only be stored
  /// in platform secure storage (iOS Keychain, Android Keystore, etc.).
  /// The bytes are automatically zeroized in Rust memory after this call.
  Future<Uint8List> getSecretBytes() => RustLib.instance.api
      .crateApiNostrIdentityManagerGetSecretBytes(that: this);

  /// Checks if an identity is loaded.
  bool hasIdentity() =>
      RustLib.instance.api.crateApiNostrIdentityManagerHasIdentity(that: this);

  /// Imports an identity from an nsec string.
  ///
  /// After calling this, use `get_secret_bytes()` to persist the secret.
  Future<PublicIdentity> importFromNsec({required String nsec}) => RustLib
      .instance
      .api
      .crateApiNostrIdentityManagerImportFromNsec(that: this, nsec: nsec);

  /// Loads an identity from raw secret bytes (retrieved from Flutter secure storage).
  ///
  /// Call this on app startup if you have persisted secret bytes.
  Future<PublicIdentity> loadFromBytes({required List<int> secretBytes}) =>
      RustLib.instance.api.crateApiNostrIdentityManagerLoadFromBytes(
        that: this,
        secretBytes: secretBytes,
      );

  /// Gets the public key as hex string (for MDK operations).
  String pubkeyHex() =>
      RustLib.instance.api.crateApiNostrIdentityManagerPubkeyHex(that: this);

  /// Signs a 32-byte message hash.
  ///
  /// Returns the signature as a 128-character hex string.
  Future<String> sign({required List<int> messageHash}) => RustLib.instance.api
      .crateApiNostrIdentityManagerSign(that: this, messageHash: messageHash);
}

@sealed
class RelayManagerFfiImpl extends RustOpaque implements RelayManagerFfi {
  // Not to be used by end users
  RelayManagerFfiImpl.frbInternalDcoDecode(List<dynamic> wire)
    : super.frbInternalDcoDecode(wire, _kStaticData);

  // Not to be used by end users
  RelayManagerFfiImpl.frbInternalSseDecode(BigInt ptr, int externalSizeOnNative)
    : super.frbInternalSseDecode(ptr, externalSizeOnNative, _kStaticData);

  static final _kStaticData = RustArcStaticData(
    rustArcIncrementStrongCount:
        RustLib.instance.api.rust_arc_increment_strong_count_RelayManagerFfi,
    rustArcDecrementStrongCount:
        RustLib.instance.api.rust_arc_decrement_strong_count_RelayManagerFfi,
    rustArcDecrementStrongCountPtr:
        RustLib.instance.api.rust_arc_decrement_strong_count_RelayManagerFfiPtr,
  );

  /// Checks whether events of a given kind by an author exist on a relay.
  ///
  /// Queries a single relay for events matching the given kind and author.
  /// Used to verify that KeyPackage (30443) and relay list (10002 / 10050)
  /// events are published.
  Future<RelayEventCheckFfi> checkEventOnRelay({
    required String relayUrl,
    required String authorPubkey,
    required int eventKind,
  }) => RustLib.instance.api.crateApiRelayManagerFfiCheckEventOnRelay(
    that: this,
    relayUrl: relayUrl,
    authorPubkey: authorPubkey,
    eventKind: eventKind,
  );

  /// Removes a single relay from the persistent connection pool by URL
  /// and tears down its WebSocket.
  ///
  /// Used by the relay-preferences UI when the user explicitly removes
  /// a relay so we do not keep leaking metadata via an idle WebSocket
  /// to a relay the user no longer trusts. Routed through the same
  /// `nostr_sdk::Client` that powers `publish_event`, `fetch_events`,
  /// and `subscribe` so removal is symmetric with addition.
  ///
  /// Returns `Ok(())` even when the relay was never connected — the
  /// caller's intent ("stop talking to this relay") is satisfied either
  /// way.
  Future<void> disconnectRelay({required String url}) => RustLib.instance.api
      .crateApiRelayManagerFfiDisconnectRelay(that: this, url: url);

  /// Fetches gift-wrapped events (kind 1059) addressed to a recipient.
  ///
  /// Queries the given relays for NIP-59 gift wrap events tagged with the
  /// recipient's public key. An optional `since` timestamp restricts results
  /// to events created after that point.
  ///
  /// # Arguments
  ///
  /// * `recipient_pubkey` - The recipient's public key (hex or npub format)
  /// * `relays` - Relay URLs to query
  /// * `since` - Optional Unix timestamp (seconds); only events after this time are returned
  ///
  /// # Returns
  ///
  /// A list of gift-wrap events serialized as JSON strings.
  Future<List<String>> fetchGiftWraps({
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  }) => RustLib.instance.api.crateApiRelayManagerFfiFetchGiftWraps(
    that: this,
    recipientPubkey: recipientPubkey,
    relays: relays,
    since: since,
  );

  /// Fetches gift-wrapped events (kind 1059) per relay, reporting which
  /// relays answered.
  ///
  /// Like [`fetch_gift_wraps`](Self::fetch_gift_wraps), but instead of one
  /// merged list it queries each relay independently and returns a per-relay
  /// outcome: whether the relay answered (`responded`) and the events it
  /// returned. This lets the caller show an accurate "N of M inboxes
  /// answered" tally. A relay that answers with zero events is
  /// `responded == true` with an empty `events` list — distinct from an
  /// unreachable relay (`responded == false`).
  ///
  /// # Arguments
  ///
  /// * `recipient_pubkey` - The recipient's public key (hex or npub format)
  /// * `relays` - Relay URLs to query, each independently
  /// * `since` - Optional Unix timestamp (seconds); only events after this time
  ///
  /// # Returns
  ///
  /// One [`RelayGiftWrapFetchFfi`] per relay, in input order.
  Future<List<RelayGiftWrapFetchFfi>> fetchGiftWrapsPerRelay({
    required String recipientPubkey,
    required List<String> relays,
    PlatformInt64? since,
  }) => RustLib.instance.api.crateApiRelayManagerFfiFetchGiftWrapsPerRelay(
    that: this,
    recipientPubkey: recipientPubkey,
    relays: relays,
    since: since,
  );

  /// Fetches MLS group messages (kind 445) from relays.
  ///
  /// Queries relays for encrypted group messages using the h-tag
  /// for routing.
  ///
  /// # Arguments
  ///
  /// * `nostr_group_id` - 32-byte Nostr group ID (h-tag value)
  /// * `relays` - Relay URLs to query
  /// * `since` - Optional Unix timestamp (seconds); only events after this time
  /// * `limit` - Maximum number of events to return
  Future<List<String>> fetchGroupMessages({
    required List<int> nostrGroupId,
    required List<String> relays,
    PlatformInt64? since,
    int? limit,
  }) => RustLib.instance.api.crateApiRelayManagerFfiFetchGroupMessages(
    that: this,
    nostrGroupId: nostrGroupId,
    relays: relays,
    since: since,
    limit: limit,
  );

  /// Fetches a user's key package (kind 30443; legacy 443 is detected but
  /// never returned).
  ///
  /// Resolves the user's relay lists (legacy kind 10051, then NIP-65 kind
  /// 10002), then fetches the most recent key package from those relays,
  /// falling back to the read-only discovery plane.
  ///
  /// # Arguments
  ///
  /// * `pubkey` - The user's public key (hex or npub format)
  ///
  /// # Returns
  ///
  /// The KeyPackage event as JSON, or `None` if not found.
  /// Returns the event JSON so Flutter can cache and use it for circle creation.
  Future<String?> fetchKeypackage({required String pubkey}) => RustLib
      .instance
      .api
      .crateApiRelayManagerFfiFetchKeypackage(that: this, pubkey: pubkey);

  /// Fetches a user's `KeyPackage` relay list (kind 10051).
  ///
  /// Returns the relay URLs where the user publishes their KeyPackages.
  /// Used to discover where to fetch a user's KeyPackage for inviting them.
  ///
  /// # Arguments
  ///
  /// * `pubkey` - The user's public key (hex or npub format)
  ///
  /// # Returns
  ///
  /// List of relay URLs, or empty if no relay list is published.
  Future<List<String>> fetchKeypackageRelays({required String pubkey}) =>
      RustLib.instance.api.crateApiRelayManagerFfiFetchKeypackageRelays(
        that: this,
        pubkey: pubkey,
      );

  /// Fetches a user's `KeyPackage` with their relay lists.
  ///
  /// Concurrently fetches three replaceable relay-list events:
  /// - kind 10051 for `KeyPackage` discovery
  /// - kind 10050 for Welcome delivery (NIP-17 inbox)
  /// - kind 10002 for Welcome delivery fallback (NIP-65)
  ///
  /// Then runs the shared `KeyPackage` discovery cascade
  /// (`fetch_keypackage_with_cascade`): 10051 → NIP-65 → defaults.
  ///
  /// Each relay-list fetch is tolerated independently: a transient failure
  /// on one list does not abort the whole call. The returned
  /// `MemberKeyPackageFfi` carries the 10050 and 10002 lists so the caller
  /// can run the Welcome delivery cascade (10050 → 10002 → defaults).
  ///
  /// # Arguments
  ///
  /// * `pubkey` - The user's public key (hex or npub format)
  ///
  /// # Returns
  ///
  /// A `MemberKeyPackageFfi` with the key package, inbox relays,
  /// and NIP-65 relays, or `None` if no `KeyPackage` was found.
  Future<MemberKeyPackageFfi?> fetchMemberKeypackage({
    required String pubkey,
  }) => RustLib.instance.api.crateApiRelayManagerFfiFetchMemberKeypackage(
    that: this,
    pubkey: pubkey,
  );

  /// Fetches a user's NIP-65 relay list (kind 10002).
  ///
  /// Returns the relay URLs from the user's general-purpose relay list.
  /// Used as a fallback when inbox relays (kind 10050) are not available.
  ///
  /// # Arguments
  ///
  /// * `pubkey` - The user's public key (hex or npub format)
  ///
  /// # Returns
  ///
  /// List of relay URLs from "r" tags, or empty if no relay list is published.
  Future<List<String>> fetchNip65Relays({required String pubkey}) => RustLib
      .instance
      .api
      .crateApiRelayManagerFfiFetchNip65Relays(that: this, pubkey: pubkey);

  /// Gets the connection status of all relays.
  Future<List<RelayConnectionStatusFfi>> getRelayStatus() =>
      RustLib.instance.api.crateApiRelayManagerFfiGetRelayStatus(that: this);

  /// `KeyPackage` maintenance (Dark Matter DM-2b) — republish-if-missing into
  /// a stable NIP-33 `d` slot on the user's own NIP-65 relays. Also the
  /// FIRST-publish path (onboarding / login): a responding relay serving
  /// nothing + no tracked slot mints a fresh package.
  ///
  /// Dart-timer-driven (the identity secret lives only in Dart, Security Rule
  /// 9): the secret bytes are consumed per-call and zeroized. Fail-soft and
  /// idempotent — one tick of the periodic maintenance loop.
  ///
  /// Steps:
  /// 1. Derive `Keys`/pubkey from the secret bytes (zeroized after).
  /// 2. Probe the user's OWN NIP-65 relays (dedup'd, own-relays-only — never a
  ///    default union) for kind-30443 events authored by self.
  /// 3. Build the presence snapshot (`(d, event_id)` per responder) — under
  ///    Dark Matter a published 30443 is a last-resort package that never dies
  ///    on join, so the presence gate is pure relay presence of the tracked
  ///    stable slot (the M8-2 live-material gate is gone).
  /// 4. Read the tracked package's OWN MLS `Lifetime` and honour the spec's
  ///    `not_after` init-key deletion bound (step 4a below) — unconditionally,
  ///    before any relay decision, because that bound does not depend on the
  ///    transport.
  /// 5. Decide via [`decide_kp_maintenance`]; on `SeedD` record the seed row;
  ///    on `Republish`/`Rotate` reuse-or-mint the single kind-30443 event,
  ///    publish to OWN relays only (publish-first), record the row, and delete
  ///    minted material on a failed publish (mdk#160).
  ///
  /// # Lifetime-aware rotation
  ///
  /// A `KeyPackage` carries an MLS `Lifetime` and stops validating at
  /// `not_after` (84 days for the OpenMLS default Haven inherits, which is
  /// also the Marmot maximum). Past that instant the 30443 is still on the
  /// relay and still fetchable, but every `Add` referencing it fails
  /// validation — the account is **silently uninvitable**. So the tick asks
  /// the package itself, never a Nostr timestamp, whether it is past
  /// `KP_ROTATE_AT_LIFETIME_FRACTION` of its own window. Timing this off
  /// `event.created_at` would be wrong here even though the reference app
  /// does it: Haven's heal path re-publishes CACHED bytes under a FRESH
  /// `created_at`, so an event-age timer would reset on every relay heal
  /// while the real `not_after` kept ticking.
  ///
  /// # 4a. The `not_after` deletion bound
  ///
  /// `foundation/key-packages.md`: a last-resort `KeyPackage` MUST delete its
  /// private `init_key` at the earlier of (a) confirmed publication of a
  /// replacement — handled by the publish-then-delete step in
  /// [`Self::republish_key_package`] — or (b) `Lifetime.not_after`. Bound (b)
  /// is transport-independent, so it runs here even when no relay responded
  /// and nothing can be published. Retaining it would mean a compromise of
  /// that one key decrypts *every recorded Welcome* ever sent to it.
  ///
  /// Returns a presence-only [`KpMaintenanceOutcomeFfi`] (counters + enum).
  ///
  /// [`decide_kp_maintenance`]: haven_core::relay::maintenance::decide_kp_maintenance
  Future<KpMaintenanceOutcomeFfi> maintainKeyPackage({
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiRelayManagerFfiMaintainKeyPackage(
    that: this,
    circle: circle,
    identitySecretBytes: identitySecretBytes,
  );

  /// M8-1 relay-list maintenance — republish-if-missing/drifted for the
  /// user's kind 10050 (inbox) + 10051 (`KeyPackage`) relay lists, honoring
  /// the per-category privacy toggle.
  ///
  /// Dart-timer-driven; the secret bytes are consumed per-call and zeroized
  /// (Security Rule 9). Fail-soft + idempotent.
  ///
  /// Per category: reads the user's OWN configured relays, NETWORK-PROBES the
  /// user's OWN relays for a currently-reachable list (never a local-timestamp
  /// check — a relay-side drop must be detected), and if missing/drifted
  /// republishes via [`build_relay_list_event`] + [`dedup_relay_targets`]
  /// (own relays only, no default union). When the toggle is off it skips (no
  /// publish). Never NIP-65/kind-10002 (Haven posture). `record_published_event`
  /// after a successful publish.
  ///
  /// Returns a presence-only [`RelayListMaintenanceOutcomeFfi`].
  ///
  /// [`build_relay_list_event`]: haven_core::relay::build_relay_list_event
  /// [`dedup_relay_targets`]: haven_core::relay::dedup_relay_targets
  Future<RelayListMaintenanceOutcomeFfi> maintainRelayList({
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiRelayManagerFfiMaintainRelayList(
    that: this,
    circle: circle,
    identitySecretBytes: identitySecretBytes,
  );

  /// Publishes a signed event to the specified relays.
  ///
  /// # Arguments
  ///
  /// * `event_json` - JSON-serialized signed Nostr event
  /// * `relays` - List of relay URLs (must be wss://)
  Future<PublishResultFfi> publishEvent({
    required String eventJson,
    required List<String> relays,
  }) => RustLib.instance.api.crateApiRelayManagerFfiPublishEvent(
    that: this,
    eventJson: eventJson,
    relays: relays,
  );

  /// Publishes an event in the background without waiting for relay acknowledgment.
  ///
  /// Spawns a background task. Suitable for location updates and key package
  /// re-publishes where periodic timers ensure eventual delivery.
  Future<void> publishEventFireAndForget({
    required String eventJson,
    required List<String> relays,
  }) => RustLib.instance.api.crateApiRelayManagerFfiPublishEventFireAndForget(
    that: this,
    eventJson: eventJson,
    relays: relays,
  );

  /// Publishes a kind-445 LOCATION event: one bounded fan-out, no retry.
  ///
  /// The location plane's publish, and the only one that may take it. One
  /// connect and one 5 s per-relay window — 10 s worst case against the
  /// ~49 s [`RelayManagerFfi::publish_event`] ladder — succeeding as soon as
  /// one relay returns `OK`. A location that misses is superseded by the next
  /// tick; a commit is not, so commits, welcomes, proposals, key packages,
  /// relay lists and profiles keep `publish_event` (Security Rule 13). Never
  /// call this for anything whose outcome resolves a `PendingStateRef`.
  ///
  /// # Arguments
  ///
  /// * `event_json` - JSON-serialized signed kind-445 event
  /// * `relays` - List of relay URLs (must be wss://)
  ///
  /// # Errors
  ///
  /// The error string keeps `RelayError`'s `Display`, so the
  /// `haven.clock.device_clock_rejected:` token a fast device clock produces
  /// still reaches `nostr_relay_service.dart` and still becomes a
  /// `RelayClockRejectionException` — the one publish failure the user can
  /// act on. No relay prose and no key material is ever in it (Rule 8).
  Future<PublishResultFfi> publishLocationEvent({
    required String eventJson,
    required List<String> relays,
  }) => RustLib.instance.api.crateApiRelayManagerFfiPublishLocationEvent(
    that: this,
    eventJson: eventJson,
    relays: relays,
  );

  /// Once-only legacy relay hygiene (Dark Matter §6 step 5 / F10a): retracts
  /// this account's stale pre-migration KeyPackage advertisements so an
  /// old-stack client cannot mint a Welcome the new stack can't process.
  ///
  /// NON-OPTIONAL cutover cleanup, guarded by a persisted sentinel
  /// (`legacy_kp_retraction_done`) so it fires at most once:
  /// 1. If the sentinel is already set → no-op (`already_done = true`).
  /// 2. Probe the user's OWN NIP-65 (KeyPackage-discovery) relays for their
  ///    own kind-443 KeyPackages AND their kind-10051 relay list.
  /// 3. For each 443, publish a self-authored NIP-09 (kind-5) id-only deletion
  ///    ([`build_legacy_key_package_retraction`]). For a present 10051, publish
  ///    an empty-replacement retraction ([`build_key_package_relay_list_retraction`])
  ///    plus a best-effort NIP-09 coordinate deletion.
  /// 4. On ≥1-relay ACK of ANY retraction, set the sentinel
  ///    ([`mark_legacy_kp_retraction_done`]) so it never re-runs.
  ///
  /// Dart-driven (the identity secret lives only in Dart, Rule 9); the secret
  /// bytes are consumed per-call and zeroized. Fail-soft: relay errors are
  /// tallied, never fatal. Returns a presence-only [`LegacyRetractionOutcomeFfi`].
  ///
  /// [`build_legacy_key_package_retraction`]: haven_core::relay::maintenance::build_legacy_key_package_retraction
  /// [`build_key_package_relay_list_retraction`]: haven_core::relay::maintenance::build_key_package_relay_list_retraction
  Future<LegacyRetractionOutcomeFfi> retractLegacyKeyMaterial({
    required CircleManagerFfi circle,
    required List<int> identitySecretBytes,
  }) => RustLib.instance.api.crateApiRelayManagerFfiRetractLegacyKeyMaterial(
    that: this,
    circle: circle,
    identitySecretBytes: identitySecretBytes,
  );

  /// Runs an M7 receive-only catch-up sweep over every visible circle.
  ///
  /// Best-effort + deadline-bounded; NEVER authors/merges/converges a commit.
  /// The Dark Matter engine owns convergence + publish-before-apply, so
  /// catch-up ingests through the single process-global session (Rule 14) and
  /// the old `MlsWriteGate` hand-off is gone. Returns a presence-only
  /// [`CatchupResultFfi`] (counters).
  Future<CatchupResultFfi> runCatchupAllCircles({
    required CircleManagerFfi circle,
    required String ownPubkeyHex,
    required BigInt maxDurationSecs,
  }) => RustLib.instance.api.crateApiRelayManagerFfiRunCatchupAllCircles(
    that: this,
    circle: circle,
    ownPubkeyHex: ownPubkeyHex,
    maxDurationSecs: maxDurationSecs,
  );

  /// Disconnects from all relays.
  Future<void> shutdown() =>
      RustLib.instance.api.crateApiRelayManagerFfiShutdown(that: this);
}
