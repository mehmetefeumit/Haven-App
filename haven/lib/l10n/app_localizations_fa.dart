...
class AppLocalizationsFa extends AppLocalizations {
  ...
  String get privacyHubSummary =>
      'Haven موقعیت مکانی‌ات را فقط با همان گروه‌های کوچکی که خودت انتخاب می‌کنی به اشتراک می‌گذارد؛ به آن‌ها حلقه می‌گوییم. پیش از خروج از گوشی هم رمزگذاری‌اش می‌کند. نه ثبت‌نامی هست و نه سروری از آنِ Haven. نام نمایشی و عکسی که تعیین می‌کنی عمومی‌اند، و در کنارشان کلیدها و فهرست‌های رله‌ای که دیگران برای دعوت کردن تو لازم دارند. صفحه‌های زیر این را، و هر چیز دیگر را، با زبان ساده توضیح می‌دهند.';
  ...
  String get privacyWhatHavenIsMeansForYou =>
      'هیچ گردانندهٔ رله‌ای را نمی‌شود وادار کرد موقعیت مکانی‌ات را تحویل بدهد، چون سرورهایی که آن را جابه‌جا می‌کنند نمی‌توانند بخوانندش. بهایش این است که هیچ‌کس هم نمی‌تواند هویتت را برایت بازیابی کند، و هر چه پیش‌تر منتشر کرده‌ای منتشر می‌ماند. برای اینکه بدانی از چه چیزی باید پشتیبان بگیری، «دو کلید تو» را ببین.';
  ...
  String get privacyRelaysMeansForYou =>
      'یک رله هرگز موقعیت مکانی‌ات را نمی‌بیند، چون پیش از خروج از گوشی رمزگذاری شده است. اما آنچه را باید عمومی باشد تا دیگران به تو برسند می‌بیند: کلیدهایی که دیگران برای دعوت کردنت لازم دارند، و فهرست‌هایی که می‌گویند کدام رله‌ها به کار می‌روند. نام و عکست هم عمومی‌اند، اما به مجموعهٔ جداگانه‌ای از رله‌ها می‌روند. Haven کار را با رله‌هایی که کار می‌کنند برایت شروع می‌کند، پس چیزی نیست که مجبور باشی عوضش کنی.';

  @override
  String get privacyRelaysDetailIndexers =>
      'Haven جدا از فهرست‌های خودت، از دو دستهٔ ثابت از سرورهای عمومی استفاده می‌کند که تو انتخابشان نمی‌کنی: هشت سرور برای پیدا کردن نام و عکس دیگران، و شش سرور برای پیدا کردن کلیدهای لازم برای دعوت کردنشان. این دو دسته هرگز هم‌پوشانی ندارند. می‌توانی سرورهای خودت را به دستهٔ نمایه اضافه کنی، اما نمی‌توانی این هشت سرور را از آن بیرون بیاوری. Haven با هر دو دسته تماس می‌گیرد، حتی اگر همهٔ رله‌هایی که برای خودت انتخاب کرده‌ای خصوصی باشند، و هر کدامشان آدرس شبکه‌ات را می‌بیند.';

  @override
  String get privacyRelaysDetailProfileLookups =>
      'برای نام‌ها و عکس‌ها، Haven هر بار سراغ یک نفر را می‌گیرد. برای هر شخص روی دستگاهت یک سرور تعیین می‌شود، و اگر آن سرور چیزی دربارهٔ آن شخص نداشته باشد، Haven فقط یک سرور دیگر را امتحان می‌کند. پس جست‌وجوی یک نفر، آن شخص را دست‌بالا به دو سرور از آن هشت سرور نشان می‌دهد، نه به همهٔ آن‌ها. Haven این تخصیص‌ها را خودبه‌خود جابه‌جا نمی‌کند، هرچند اگر مجموعهٔ سرورها عوض شود تغییر می‌کنند. این کار پرس‌وجوهایت را پخش می‌کند، به‌جای اینکه همه‌شان به یک سرور برود. با این حال، هر سروری که به کسی اختصاص یافته باز هم کم‌کم سابقه‌ای ماندگار از این پیدا می‌کند که گوشی تو مدام سراغ همان شخص را می‌گیرد، و وقتی آدم‌ها انگشت‌شمارند، ممکن است چندتاشان به یک سرور اختصاص پیدا کنند. وقتی نام یا عکس خودت را ذخیره می‌کنی، آنچه ذخیره کرده‌ای به همهٔ رله‌های نمایه‌ای می‌رود که واقعاً از آن‌ها استفاده می‌کنی، یعنی همان هشت سرور منهای هر کدام که Haven کنار گذاشته، به‌علاوهٔ هر کدام که خودت افزوده‌ای، پس هر یک از آن‌ها کلید عمومی‌ات را می‌شناسد.';
  ...
  String get privacyWhatOthersSeeCannotPause =>
      'تا وقتی Haven باز است و در حلقه‌ای عضوی، موقعیتت هر دو سه دقیقه خودبه‌خود بیرون می‌رود. دکمهٔ توقف موقت وجود ندارد: گزینهٔ صفحهٔ «موقعیت مکانی» فقط تعیین می‌کند که آیا این کار وقتی Haven در پس‌زمینه و همچنان در حال اجراست ادامه پیدا کند یا نه. اگر سیستم Haven را ببندد و اجرایش را پایان دهد، اشتراک‌گذاری متوقف می‌شود. روی Android، اشتراک‌گذاری حتی بعد از کنار زدن Haven از فهرست برنامه‌های باز ادامه پیدا می‌کند و پس از راه‌اندازی دوبارهٔ دستگاه خودبه‌خود از سر گرفته می‌شود؛ روی iPhone تا وقتی Haven را باز نکنی متوقف می‌ماند. برای آنکه اشتراک‌گذاری با یک حلقه را متوقف کنی، آن را باز کن و «ترک حلقه» را انتخاب کن.';
  ...
  String get privacyWhatOthersSeeDetailTag =>
      'برچسب یک حلقه تا پایان عمر آن حلقه همان می‌ماند، پس یک رله می‌تواند همهٔ پیام‌هایش را تا هر وقت که بخواهد به هم پیوند بزند. پروتکل اجازه می‌دهد که یک حلقه به برچسب تازه‌ای برود، اما Haven امروز این کار را نمی‌کند؛ و رفتن به برچسب تازه هم آنچه را رله پیش‌تر به هم پیوند زده پس نمی‌گیرد.';
  ...
  String get privacyWhatOthersSeeDetailOnDevice =>
      'گوشی هر عضو، آخرین موقعیتی را که از تو گرفته پس از یک روز دیگر نشان نمی‌دهد، و دفعهٔ بعد که آن عضو Haven را باز کند پاکش می‌کند. این جدا از هر چیزی است که خودِ آن عضو تصمیم گرفته ذخیره کند یا از صفحه‌نمایش عکس بگیرد، که Haven هیچ اختیاری بر آن ندارد.';

  @override
  String get privacyWhatOthersSeeDetailOneConnection =>
      'Haven به هر رله یک اتصال باز می‌کند و هر کاری روی آن رله را از همان اتصال پیش می‌برد. جایی که یک رله هم دعوت‌های تو را حمل می‌کند و هم پیام‌های یک حلقه را، همان یک اتصال هم‌زمان هم دعوت‌هایی را می‌خواهد که به کلید عمومی خودِ تو نشانی شده‌اند و هم پیام‌های آن حلقه را با برچسبش، و همین است که به رله امکان می‌دهد این دو را به هم گره بزند. در یک نصب تازه، برای هر دو از همان رله‌ها استفاده می‌شود.';
  ...
  String get privacyInferenceActivityPattern =>
      'از همان الگو، رله‌ای که از آن استفاده می‌کنی می‌تواند تقریباً بفهمد کِی فعالی و هر چند وقت یک‌بار. تا وقتی Haven روی صفحه است، و روی iPhone تا وقتی اشتراک‌گذاری در پس‌زمینه آن را در حال اجرا نگه می‌دارد، این برنامه هر بار که حدود صد متر جابه‌جا می‌شوی یک به‌روزرسانی اضافی هم می‌فرستد، آن هم دست‌بالا دقیقه‌ای یک بار. در طول چند ساعت، همین می‌تواند به یک رله نشان بدهد که در حال حرکت بوده‌ای یا یک جا مانده‌ای. اما این الگو هرگز نشان نمی‌دهد کجا هستی.';
