...
class AppLocalizationsFa extends AppLocalizations {
  ...
  String get relaySettingsKeyPackageSubtitle =>
      'kind 10002، جایی که دعوت‌شدگان کلیدهای رمزگذاری تو را پیدا می‌کنند';
  ...
  String get privacyRelaysWhyMany =>
      'استفاده از چند رله یعنی هیچ‌کدام به‌تنهایی نمی‌تواند تو را قطع کند. اگر یکی از کار بیفتد، بقیه به کار خود ادامه می‌دهند. خودت انتخاب می‌کنی از کدام رله‌ها استفاده کنی و هر وقت بخواهی می‌توانی عوضشان کنی.';
  ...
  String get privacyRelaysMeansForYou =>
      'اینجا چیز خطرناکی نیست که اشتباه کنی: یک رله همیشه فقط دادهٔ رمزگذاری‌شده می‌بیند. اما دست‌کم به یک رله نیاز داری که در دسترس باشد، وگرنه کسی نمی‌تواند تو را دعوت کند و حلقه‌هایت ساکت می‌شوند.';
  ...
  String get privacyRelaysDetailKeyListIsPublic =>
      'فهرست رله‌های KeyPackage تو خودش هم روی شبکه منتشر می‌شود و با کلید هویتت امضا شده است تا دیگران بتوانند پیدایش کنند. اگر رلهٔ خصوصی‌ای به آن فهرست اضافه کنی، آدرسش عمومی می‌شود و به تو گره می‌خورد. رله‌ای که فقط داخل فهرست خودِ یک حلقه به کار می‌رود، این‌گونه اعلام نمی‌شود.';
  ...
  String get privacyEncryptionSubtitle =>
      'کلیدهای جدا برای هر حلقه، و آنچه با پیوستن یا رفتن افراد تغییر می‌کند';

  @override
  String get privacyEncryptionPerCircle =>
      'موقعیت مکانی‌ات پیش از خروج از گوشی، با یک استاندارد باز به نام MLS رمزگذاری می‌شود. هر حلقه گروه رمزگذاری‌شدهٔ خودش با کلیدهای خودش است. هیچ‌کس از بیرونِ یک حلقه نمی‌تواند آنچه درون آن فرستاده می‌شود را بخواند — نه یک رله، نه خودِ Haven و نه حلقهٔ دیگری که در آن هستی.';
  ...
  String get privacyEncryptionWhenSomeoneLeaves =>
      'وقتی کسی می‌رود یا حذف می‌شود، حلقه به کلید تازهٔ دیگری می‌رود. حداکثر در طول چند دقیقه، او دیگر نمی‌تواند چیز تازه‌ای بخواند.';

  @override
  String get privacyEncryptionKeysChangeOnMembership =>
      'کلیدها وقتی عوض می‌شوند که اعضای حلقه عوض شوند — نه بر پایهٔ زمان‌بندی. میان دو تغییر عضویت، یک کلید همهٔ آنچه فرستاده می‌شود را پوشش می‌دهد. پس اگر کسی را از حلقه‌ای دیرپا حذف کنی، او باز هم می‌تواند هر پیامی را که در دوران عضویتش ذخیره کرده باز کند، حتی پیام‌های هفته‌ها پیش‌تر. او نمی‌تواند هیچ چیزی را که پس از رفتنش فرستاده شده باز کند.';

  @override
  String get privacyEncryptionMeansForYou =>
      'رمزگذاری تو را از همهٔ کسانی که بیرون حلقه‌هایت هستند محافظت می‌کند. اما نمی‌تواند تو را از کسی که روزی درون یکی از آن‌ها بوده محافظت کند. اگر می‌خواهی رابطه‌ات با کسی را یک‌سره قطع کنی، ساختن یک حلقهٔ تازه ایمن‌تر از حذف کردن او از حلقهٔ قدیمی است.';
  ...
  String get privacyEncryptionDetailEpochs =>
      'به هر بازه‌ای که یک کلید در آن کار می‌کند، یک «دوره» می‌گویند. دستگاهت فقط آن‌قدر مواد رمزنگاری نگه می‌دارد که بتواند پیام‌های در راه ماندهٔ چند دورهٔ آخر را بازگشایی کند، و بعد آن را دور می‌ریزد — و همین است که Haven حتی در اصل هم نمی‌تواند پیام‌های قدیمی‌ترت را برایت بازیابی کند.';
  ...
  String get privacyWhatOthersSeeCannotPause =>
      'تا وقتی Haven باز است و در حلقه‌ای عضوی، موقعیتت هر دو سه دقیقه خودبه‌خود بیرون می‌رود. دکمهٔ توقف موقت وجود ندارد. تنها گزینه‌ای که در دست تو است این است که آیا اشتراک‌گذاری پس از بیرون رفتنت از برنامه ادامه پیدا کند یا نه.';
  ...
  String get privacyWhatOthersSeeCoMemberIp =>
      'عکس نمایهٔ هر عضو از سروری گرفته می‌شود که خودش انتخاب کرده است. بارگیری آن عکس، آدرس شبکه‌ات — و در نتیجه اینکه تقریباً کجای دنیا هستی — را به گردانندهٔ آن سرور لو می‌دهد. این کار خودبه‌خود انجام می‌شود، بدون آنکه روی چیزی بزنی.';
  ...
  String get privacyWhatOthersSeeRelaysCannot =>
      'گردانندگان رله نمی‌توانند موقعیت مکانی‌ات، پیام‌هایت، نام حلقه‌ات یا فهرست اعضایش را بخوانند. هر پیام از یک آدرس فرستندهٔ تازه و یک‌بارمصرف به آن‌ها می‌رسد، پس نمی‌توانند بگویند کدام عضو آن را فرستاده است.';

  @override
  String get privacyWhatOthersSeeRelaysCan =>
      'آن‌ها آدرس شبکه‌ات، زمان‌هایی که آنلاینی، و اندازهٔ ترافیکت و اینکه هر چند وقت یک‌بار رفت‌وآمد داری را می‌بینند. هر حلقه یک برچسب تصادفی با خود دارد، پس یک رله می‌تواند پیام‌های یک حلقه را کنار هم بگذارد و حدس بزند چند نفر در آن هستند — بی‌آنکه بداند آن‌ها چه کسانی‌اند.';

  @override
  String get privacyWhatOthersSeeMeansForYou =>
      'با یک حلقه مثل کسانی رفتار کن که نشانی خانه‌ات را به آن‌ها می‌دهی. در برابر رله‌ها و بیرونی‌ها رمزگذاری پایدار است، و یک VPN شکافِ آدرس شبکه را می‌بندد. در برابر عضوی که عکس صفحه می‌گیرد، هیچ چیزی پایدار نیست.';
  ...
  String get relaySettingsBackendCaption =>
      'Haven سرور خودش را ندارد. به‌روزرسانی‌های رمزگذاری‌شده‌ات از سرورهای مستقلی می‌گذرند که به آن‌ها رله می‌گویند؛ این رله‌ها هرگز نه موقعیت مکانی‌ات را می‌بینند و نه اینکه چه کسانی در حلقه‌هایت هستند.';

  @override
  String get privacyWhatOthersSeeScreenshots =>
      'عکس گرفتن از صفحه به سیستم‌عامل بستگی دارد و این دو سیستم‌عامل با هم فرق دارند. Haven روی Android عکس صفحه و ضبط صفحه را در سراسر برنامه مسدود می‌کند. روی iPhone این کار شدنی نیست: Haven پیش‌نمایش برنامه را در فهرست برنامه‌های باز محو می‌کند، اما هر عضوی همچنان می‌تواند از آنچه روی صفحه است عکس بگیرد.';
  ...
  String get privacyInferenceWhatIsMetadata =>
      'رمزگذاری آنچه می‌فرستی را پنهان می‌کند. اما نمی‌تواند پنهان کند که چیزی فرستاده‌ای. الگویی که گرد یک پیام شکل می‌گیرد — اینکه کِی بیرون رفته، چه اندازه بوده و به کدام سرورها وصل بوده‌ای — فراداده نام دارد، و حتی وقتی محتوا پنهان است، این الگو پیدا می‌ماند.';

  @override
  String get privacyInferenceActivityPattern =>
      'از همان الگو، رله‌ای که از آن استفاده می‌کنی می‌تواند تقریباً بفهمد کِی فعالی و هر چند وقت یک‌بار. Haven وقتی بیش از حدود صد متر جابه‌جا می‌شوی یک به‌روزرسانی اضافی هم می‌فرستد. پس سابقه‌ای که به‌قدر کافی بلند باشد می‌تواند نشانه بدهد که در حال سفر بوده‌ای یا یک جا مانده‌ای — بی‌آنکه هرگز نشان بدهد کجا.';

  @override
  String get privacyInferencePresence =>
      'تا وقتی Haven در حال اجراست، یک اتصال باز به رله‌هایت نگه می‌دارد، پس آن‌ها می‌توانند ببینند که آنلاینی و کدام حلقه‌ها را دنبال می‌کنی. این بهای آن است که به‌روزرسانی‌ها در چند ثانیه برسند، نه چند دقیقه.';
  ...
  String get privacyInferenceIpAddress =>
      'هر سروری که Haven با آن حرف می‌زند، آدرس شبکه‌ای را می‌بیند که گوشی‌ات از آن وصل می‌شود: رله‌هایت، ارائه‌دهندهٔ نقشه، و هر کسی که عکس نمایهٔ یک عضو را میزبانی می‌کند. ارائه‌دهندهٔ اینترنتت می‌بیند به کدام‌یک از این سرورها رسیده‌ای و کِی. Haven خودش هیچ کاری برای پنهان کردن هیچ‌یک از این دو نمی‌کند.';
  ...
  String get privacyInferenceMeansForYou =>
      'اگر نگرانی‌ات این است که یک گردانندهٔ شبکه — و نه یک آدم — تو را روی نقشه بگذارد، یک VPN مفیدترین چیزی است که می‌توانی اضافه کنی. اگر نگرانی‌ات افرادِ حلقه‌هایت هستند، هیچ کمکی نمی‌کند.';
  ...
  String get privacyInferenceDetailOutOfScope =>
      'بعضی چیزها به‌سادگی بیرون از حوزهٔ کاری‌اند که Haven می‌تواند انجام دهد. کسی که بتواند چندین رله را هم‌زمان زیر نظر بگیرد، یا گوشی خودت وقتی کس دیگری کنترلش را در دست بگیرد، فراتر از چیزی است که هیچ انتخاب طراحی‌ای در اینجا بتواند درستش کند.';
  ...
  String get qrCodeExplainerUsername =>
      'کلید عمومی تو مانند یک نام کاربری کار می‌کند که به اشتراک گذاشتنش امن است. مردم این کد را اسکن می‌کنند، یا کلید عمومی تو را جای‌گذاری می‌کنند، تا تو را به یک حلقه دعوت کنند. به اشتراک گذاشتن آن نمی‌تواند موقعیت مکانی‌ات را آشکار کند؛ موقعیت مکانی‌ات تنها برای اعضای حلقه‌هایی که به آن‌ها پیوسته‌ای دیده می‌شود. نام نمایشی و عکست از پیش عمومی‌اند، پس به اشتراک گذاشتن این کد چیز تازه‌ای دربارهٔ آن‌ها آشکار نمی‌کند.';
