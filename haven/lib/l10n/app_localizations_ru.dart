...
class AppLocalizationsRu extends AppLocalizations {
  ...
  String get relaySettingsKeyPackageSubtitle =>
      'kind 10002, где приглашающие находят ваши ключи шифрования';
  ...
  String get privacyRelaysWhyMany =>
      'Когда реле несколько, ни одно из них не может отрезать вас от связи. Если одно отключится, остальные продолжат работать. Вы сами выбираете, какими реле пользоваться, и можете поменять их в любой момент.';

  @override
  String get privacyRelaysTwoLists =>
      'Haven ведёт для вас два списка. На ваши входящие реле приходят приглашения. С ваших реле KeyPackage другие берут ключи, которые нужны, чтобы вас пригласить. У каждого круга есть ещё и свой список — по нему идут зашифрованные обновления этого круга.';

  @override
  String get privacyRelaysMeansForYou =>
      'Здесь нечего испортить: реле всё равно видит только зашифрованные данные. Но хотя бы одно доступное реле вам нужно — иначе никто не сможет вас пригласить, а ваши круги замолчат.';
  ...
  String get privacyRelaysDetailKeyListIsPublic =>
      'Список ваших реле KeyPackage сам публикуется в сети и подписывается вашим ключом личности, чтобы другие могли его найти. Если добавить в этот список приватное реле, его адрес становится публичным и связанным с вами. Реле, которое используется только в списке одного круга, так не раскрывается.';
  ...
  String get privacyEncryptionSubtitle =>
      'У каждого круга свои ключи; что меняется, когда кто-то входит или уходит';

  @override
  String get privacyEncryptionPerCircle =>
      'Ваше местоположение шифруется на телефоне ещё до отправки — по открытому стандарту MLS. Каждый круг — это отдельная зашифрованная группа со своими ключами. Никто за пределами круга не может прочитать то, что отправлено внутри него: ни реле, ни Haven, ни другой круг, в котором вы состоите.';
  ...
  String get privacyEncryptionWhenSomeoneLeaves =>
      'Когда кто-то уходит или его удаляют, круг переходит на очередной новый ключ. Максимум через несколько минут этот человек уже не сможет прочитать ничего нового.';

  @override
  String get privacyEncryptionKeysChangeOnMembership =>
      'Ключи меняются только тогда, когда меняется состав круга; сами по себе, просто со временем, они не меняются никогда. Между такими изменениями всё отправленное закрыто одним и тем же ключом. Поэтому человек, которого вы удалили из давно неизменного круга, по-прежнему сможет открыть любые сообщения, сохранённые им, пока он был участником, — даже те, которым уже недели. Но ничего из отправленного после его ухода он открыть не сможет.';

  @override
  String get privacyEncryptionMeansForYou =>
      'Шифрование защищает вас от всех, кто вне ваших кругов. Оно не защитит вас от того, кто когда-то был внутри. Если вы хотите полностью разойтись с человеком, надёжнее создать новый круг, чем удалять этого человека из старого.';
  ...
  String get privacyEncryptionDetailEpochs =>
      'Каждый период действия ключа называется эпохой. Ваше устройство хранит ровно столько материала, сколько нужно, чтобы расшифровать сообщения последних эпох, ещё находящиеся в пути, а затем удаляет его — поэтому Haven в принципе не может восстановить для вас старые сообщения.';
  ...
  String get privacyWhatOthersSeeCannotPause =>
      'Пока Haven открыт и вы состоите в круге, ваше местоположение само отправляется каждые пару минут. Кнопки «Пауза» нет. Единственный переключатель — продолжать ли делиться, когда вы выходите из приложения.';
  ...
  String get privacyWhatOthersSeeCoMemberIp =>
      'Фото профиля участника загружается с сервера, который выбрал он сам. При загрузке ваш сетевой адрес — а значит, и примерно то, в какой части мира вы находитесь, — становится известен тому, кто управляет этим сервером. Это происходит само собой, без единого нажатия с вашей стороны.';
  ...
  String get privacyWhatOthersSeeRelaysCannot =>
      'Операторы реле не могут прочитать ни ваше местоположение, ни ваши сообщения, ни название круга, ни список его участников. Каждое сообщение приходит к ним с нового одноразового адреса отправки, поэтому они не могут определить, кто из участников его отправил.';

  @override
  String get privacyWhatOthersSeeRelaysCan =>
      'Зато они видят ваш сетевой адрес, время, когда вы в сети, а также размер и частоту вашего трафика. У каждого круга есть случайная метка, поэтому реле может собрать сообщения одного круга вместе и оценить, сколько в нём людей, — не узнавая при этом, кто именно.';

  @override
  String get privacyWhatOthersSeeMeansForYou =>
      'Пускайте в круг только тех, кому дали бы свой домашний адрес. От реле и посторонних шифрование защищает, а VPN закрывает вопрос с сетевым адресом. От участника, который сохранит снимок экрана, не защищает ничто.';
  ...
  String get relaySettingsBackendCaption =>
      'У Haven нет собственного сервера. Ваши зашифрованные обновления идут через независимые серверы, которые называют реле: они никогда не видят ни ваше местоположение, ни то, кто состоит в ваших кругах.';

  @override
  String get privacyWhatOthersSeeScreenshots =>
      'Со снимками экрана всё зависит от платформы, и две платформы ведут себя по-разному. На Android Haven блокирует снимки экрана и запись экрана во всём приложении. На iPhone Haven так сделать не может — там размывается только предпросмотр в переключателе приложений, а участник всё равно может сделать снимок того, что видно на экране.';
  ...
  String get privacyInferenceWhatIsMetadata =>
      'Шифрование скрывает то, что вы отправляете. Оно не может скрыть сам факт отправки. То, что окружает сообщение — когда оно ушло, какого было размера, к каким серверам вы подключались — называется метаданными и остаётся видимым, даже когда содержимое скрыто.';

  @override
  String get privacyInferenceActivityPattern =>
      'По этим метаданным реле, которым вы пользуетесь, может примерно определить, когда вы активны и как часто. Haven отправляет ещё и дополнительное обновление, когда вы отходите примерно на сотню метров или больше, поэтому достаточно длинная запись может подсказать, были ли вы в пути или оставались на месте, — при этом никогда не показывая, где именно.';

  @override
  String get privacyInferencePresence =>
      'Пока Haven работает, он держит открытое соединение с вашими реле, поэтому они видят ещё и то, что вы в сети, и за какими кругами вы следите. Это цена того, что обновления приходят за секунды, а не за минуты.';
  ...
  String get privacyInferenceIpAddress =>
      'Каждый сервер, с которым связывается Haven, видит адрес, с которого подключается ваш телефон: это ваши реле, поставщик карт и тот, кто размещает фото профиля участника. Ваш интернет-провайдер видит, к каким из этих серверов вы обращались и когда. Сам по себе Haven не скрывает ни того, ни другого.';
  ...
  String get privacyInferenceMeansForYou =>
      'Если вас беспокоит, что ваше место на карте узнает тот, кто управляет сетью, а не конкретный человек, VPN — самое полезное, что вы можете добавить. Если же вас беспокоят люди в ваших кругах, он совсем не поможет.';
  ...
  String get privacyInferenceDetailOutOfScope =>
      'С чем-то Haven просто не может ничего сделать. Наблюдатель, способный следить сразу за многими реле, или ваш собственный телефон, попавший под чужой контроль, — всё это лежит за пределами того, что можно исправить любыми решениями в самом приложении.';
  ...
  String get qrCodeExplainerUsername =>
      'Ваш публичный ключ работает как имя пользователя, которым безопасно делиться. Люди сканируют этот код или вставляют ваш публичный ключ, чтобы пригласить вас в круг. По нему нельзя узнать ваше местоположение — оно видно только участникам кругов, в которые вы вступили. Ваше отображаемое имя и фото уже публичны, поэтому, поделившись этим кодом, вы не раскроете о них ничего нового.';
