...
void main() {
  ...
  group('LocationSharingService.fetchMemberLocations — IgnoredProposal: '
      'pendingDepartureReason', () {
    test('ignored result threads ignoredReason into '
        'LocationFetchResult.pendingDepartureReason', () async {
    ...
    test('non-ignored result leaves pendingDepartureReason null', () async {
      ...
      expect(
        result.pendingDepartureReason,
        isNull,
        reason: 'A normal group-update must NOT set pendingDepartureReason',
      );
    });
  });
  ...
  group('LocationSharingService.fetchMemberLocations — IgnoredProposal: '
      'seen-set exclusion', () {
    test('ignored result does NOT add the event id to _seenEventIds', () async {
    ...
    test(
      ...
      () async {
        ...
        final mockRelay = MockRelayService(
          groupMessages: ['{"id":"normal-evt","kind":445,"content":"commit"}'],
        );
        final mockCircle = MockCircleService()
          ..decryptLocationResults = [const DecryptResult(groupUpdated: true)];
  ...
  group('LocationSharingService.pollEvolutionEvents — IgnoredProposal: '
    ...
    test('_runEvolutionPoll does NOT mark an ignored event as seen', () async {
