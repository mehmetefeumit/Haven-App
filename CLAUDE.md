...
13. **Publish-Before-Apply**: NEVER call `confirm_published` before at least one relay has returned an OK-ack ("acked" means acked, never merely "sent"); call `publish_failed` on failure; treat `PendingCommitRecovered` as a mandatory resync; an `Err` from `confirm_published` is NOT a publish failure — the engine has already applied the commit and only its replay failed — so never roll it back and never retry it: drain and fold what it delivered, then return the original error (pinned in `haven-core/tests/security_rule_gates.rs`)
